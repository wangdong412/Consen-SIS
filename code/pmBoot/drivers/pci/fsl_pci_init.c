...
#else
#ifndef CONFIG_SYS_PCI_MEMORY_BUS
#define CONFIG_SYS_PCI_MEMORY_BUS CONFIG_SYS_PCI_MEMORY_SPI_FLASH_BUS
#endif

#ifndef CONFIG_SYS_PCI_MEMORY_PHYS
#define CONFIG_SYS_PCI_MEMORY_PHYS CONFIG_SYS_PCI_MEMORY_SPI_FLASH_PHYS
#endif
#endif
					 ...
					 volatile pit_t *pi)
{
	...
	pci_size_t pci_sz;
#else
	u64 sz = CONFIG_SYS_PCI_MEMORY_SPI_FLASH_SIZE ;//min((u64)gd->ram_size, (1ull << 32));

	phys_addr_t phys_start ;
	pci_addr_t bus_start ;
	pci_size_t pci_sz;

    if (hose->cfg_addr == (unsigned int *)CONFIG_SYS_PCIE2_ADDR) { 
        phys_start = CONFIG_SYS_PCI_MEMORY_SPI_FLASH_PHYS;
        bus_start = CONFIG_SYS_PCI_MEMORY_SPI_FLASH_BUS;
    }
    else if (hose->cfg_addr == (unsigned int *)CONFIG_SYS_PCIE1_ADDR) { 
        phys_start = CONFIG_SYS_PCI_MEMORY_IO_PHYS;
        bus_start = CONFIG_SYS_PCI_MEMORY_IO_BUS;
    }
    else {
    }
#endif
...
void fsl_pci_init(struct pci_controller *hose, struct fsl_pci_info *pci_info)
{
	...
	volatile ccsr_fsl_pci_t *pci = (ccsr_fsl_pci_t *)cfg_addr; //pcie controller
