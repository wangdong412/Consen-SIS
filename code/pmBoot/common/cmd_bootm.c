...
int do_burnub(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    return run_command(getenv("burnub"),flag);
}

U_BOOT_CMD(
    burnub, 1,  1,  do_burnub,
    "burn uboot to flash",
    ""
);

int do_upub(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    return run_command(getenv("ubcmd"),flag);
}
...
U_BOOT_CMD(
    nor0, 1,  1,  do_bootcagos0,
    "boot cagos 0 from flash",
    ""
);
