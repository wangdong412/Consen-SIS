...
//清理pm接收寄存器
void clear_precvstat_status(INT8 bus_no);
void clear_precvdata_status(INT8 bus_no);
...
void os_update_mno();
void  OSTaskCreate (void *p_tcb,void *p_name,void *p_task,void *p_arg,int prio,
    void *p_stk_base,int stk_limit,int stk_size,int q_size,int time_quanta,void *p_ext,
    int opt,void *p_err);
...
/*
 *
 * 可操作地址 : 0xec000000 ~ 0xeff20000
 * uboot地址  : 0xeff20000 ~ 0xf0000000
 * 擦除块大小 : 128KB, 共 0~511 个块
 * 
 */
int lld_write ( unsigned int addr, unsigned char *src, unsigned int cnt);//返回实际写入的字节数
int lld_read ( unsigned int addr , unsigned char *buf, unsigned int cnt);//返回实际读出的字节数
int lld_erase(int no, int num); //从第no块开始擦除num块
...
typedef void (*exception_handler)(char *);
int registerException(enum EXCEPTION_TYPE exception_type,exception_handler handler,char * private_data,const char * name);

/*
 * flag = 0x0,打印关闭。
 * flag = 0x1 打印打开。
 */
void printCfg(unsigned long flag);

void printe (const char *fmt,...);

/*
 * Get the SPE Float status control reg
 *
 * Returns:
 *  0: No SPE float Error
 *  1: invalid input error, ep, NaN, Inf
 *  2: divide-by-zero error
 *  3: floating-point underflow error
 *  4: floating-point overflow error
 */

UINT32 os_get_spescr(void);
