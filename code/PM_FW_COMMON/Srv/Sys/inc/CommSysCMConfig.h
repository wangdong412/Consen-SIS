...
#define PRG_SECTION_ID_TASKDATAEXCHANGE     (0x0010)          /*任务间数据交换段*/
...
#define PRG_SECTION_ID_IMP_FUNC_PTR_LOAD_RELOC (0x0015)       /*导入函数指针加载时重定位区*/
#define PRG_SECTION_ID_PMCONFIG             (0x0016)          /*PM配置区*/
...
/*工程没有被配置过，不存在配置信息*/
#define NOT_HAS_BE_CONFIGURED               (0x00000000U)
/*工程被配置过，存在配置信息*/
#define HAS_BE_CONFIGURED                   (0x00000001U)
...
typedef struct NTPTCPConfigInfoTag
{
    ...
    uint8_t ucPulseSrc;                 /*取值0：None，1：GPS，2：System Internal，3：Local*/
    uint8_t uiReserved[3];              /*保留*/
}NTPTCPConfigInfo_t;
...
typedef struct TaskConfigInfoTag
{
    ...
    uint32_t uiChangeGlobalInitEntry;/*任务的数据区增量初始化函数入口，相对于PRG文件的Program段的Machine Code的偏移。*/
    uint32_t uiGOffset;             /*G区的起始地址，相对于数据区首地址的偏移*/
    uint32_t uiGUsedSize;           /*G区的已使用大小*/
    uint32_t uiIOffset;             /*I区的起始地址，相对于数据区首地址的偏移*/
    uint32_t uiIUsedSize;           /*I区的已使用大小*/
    uint32_t uiOOffset;             /*Q区的起始地址，相对于数据区首地址的偏移*/
    uint32_t uiOUsedSize;           /*Q区的已使用大小*/
...
typedef struct StationConfigInfoTag
{
    ...
    uint32_t uiObjPasswordLen;                   /*工程密码长度*/
    char_t ucObjPassword[MAX_PASSWOED_NUM];      /*工程密码*/
    uint8_t ucCommTolerate;                      /*通讯容忍， 取值0,1,2,3*/
    uint8_t ucVoteTolerate;                      /*表决容忍， 取值0,1,2,3*/
    uint8_t ucSlaveConfirm;
