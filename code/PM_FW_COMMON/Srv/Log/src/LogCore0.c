...
void LogInitCore0(void)
{
    ...
    if(0==get_cpu_no())
    {
        ...
        s_pstQueueInfo = (LogQueue_t *)LOG_SRAM_QUEUE_INFO_SEC5_START_ADDR;
        ...
        s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC5_START_ADDR;
        //s_puiLockFlag = (puint32_t)LOG_SRAM_LOCK_FLAG_START_ADDR;
        //*s_puiLockFlag = (uint32_t)LOG_QUEUE_UNLOCK;

        s_uiFlagC0 = (uint32_t)LOG_QUEUE_UNLOCK;
        s_puiLockFlag = &s_uiFlagC0;

        printf("Log: %x %x %d %d %d %d\n",s_stQueueInfo.usMagic1, s_stQueueInfo.usMagic2, s_stQueueInfo.usSize,s_stQueueInfo.usHeader,s_stQueueInfo.usTail, s_stQueueInfo.usQueueState);
    }
...
***********************************************************************************************************************
*/
void LogWriteCore0(uint32_t uiLogID)
{
    ...
    do
    {
        ...
        if(true == bAtomicFlg)
        {
            ...
            //printf("tail=%d\n", usTail);
            if(usTail <= LOG_INFO_SEC5_MAX_INDEX)
            {
                s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC5_START_ADDR;
            ...
            {
                s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC6_START_ADDR;
                s_pstLogInfo = &s_pstBaseLogInfo[usTail-LOG_INFO_SEC6_START_INDEX];
            }
...
***********************************************************************************************************************
*/
int32_t LogReadCore0(LogInfo_t *pstLogInfo, uint16_t usLogicPos, puint32_t puiNextIndex, SysTime_t* pstTimestamp)
{
            ...
            if(true == bAtomicFlg)
            {
                ...
                if(LOG_QUEUE_EMPTY != s_stQueueInfo.usQueueState)
                {
                    ...
                    else
                    {
                        ...
                        if(iLogIndex <= LOG_INFO_SEC5_MAX_INDEX)
                        {
                            s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC5_START_ADDR;
                        ...
                        {
                            s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC6_START_ADDR;
                            pstLogInfoTmp = &s_pstBaseLogInfo[iLogIndex-LOG_INFO_SEC6_START_INDEX];
                        }

                        if(usPreIndex <= LOG_INFO_SEC5_MAX_INDEX)
                        {
                            s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC5_START_ADDR;
                        ...
                        {
                            s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC6_START_ADDR;
                            pstLogInfoTmpPre = &s_pstBaseLogInfo[usPreIndex-LOG_INFO_SEC6_START_INDEX];
                        }
                        ...
                        if(usLogicPos != READ_LOG_FROM_BEGINNING)
                        {
                            ...
                            else
                            {
                                ...
                                *puiNextIndex = (usLogicPos + 1) % MAX_LOG_NUM;
                                ...
                                if(usLogicPos <= LOG_INFO_SEC5_MAX_INDEX)
                                {
                                    s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC5_START_ADDR;
                                ...
                                {
                                    s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC6_START_ADDR;
                                    pstLogInfoTmp = &s_pstBaseLogInfo[usLogicPos-LOG_INFO_SEC6_START_INDEX];
                                }
                                /* 读取日志 */
                                memcpy((puint8_t)pstLogInfo, (puint8_t)pstLogInfoTmp, sizeof(LogInfo_t));
                            }
