...
static void UpdateLogAlarm(void);
...
void LogInitUser(void)
{
    ...
    if(1==get_cpu_no())
    {
        ...
        if((s_pstQueueInfo->usMagic1 != LOG_MAGIC_1) && (s_pstQueueInfo->usMagic2 != LOG_MAGIC_2))
        {
            ...
            s_stQueueInfo.usAlarm = LOG_CLR_ALARM;
            s_stQueueInfo.iReadIndex = 0;
    ...
    do
    {
        ...
        if(true == bAtomicFlg)
        {
            ...
            {/* 队列满 */
                ...
                s_stQueueInfo.usHeader = s_stQueueInfo.usTail;
            }
            else
            {/* 队列未满 */
                s_stQueueInfo.usQueueState = LOG_QUEUE_NOT_EMPTY_NOT_FULL;
...
***********************************************************************************************************************
*/
int32_t LogReadUser(LogInfo_t *pstLogInfo, uint16_t usLogicPos, puint32_t puiNextIndex, SysTime_t* pstTimestamp)
{
    ...
    uint16_t usPreIndex = 0;
    LogInfo_t *pstLogInfoTmpPre = NULL;
    LogInfo_t stLogPre;
            ...
            if(true == bAtomicFlg)
            {
                ...
                if(LOG_QUEUE_EMPTY != s_stQueueInfo.usQueueState)
                {
                    ...
                    else
                    {
                        ...
                        if(usLogicPos != READ_LOG_FROM_BEGINNING)
                        {
                            if(usLogicPos > 0)
                            {
                                usPreIndex = usLogicPos - 1;
                            }
                            else
                            {
                                usPreIndex = s_stQueueInfo.usSize -1;
                            }
                        }
                        ...
                        if(usPreIndex <= LOG_INFO_SEC9_MAX_INDEX)
                        {
                            s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC9_START_ADDR;
                            pstLogInfoTmpPre = &s_pstBaseLogInfo[usPreIndex];
                        }
                        else
                        {
                            s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SECA_START_ADDR;
                            pstLogInfoTmpPre = &s_pstBaseLogInfo[usPreIndex-LOG_INFO_SECA_START_INDEX];
                        }
                        ...
                        if(usLogicPos != READ_LOG_FROM_BEGINNING)
                        {
                            ...
                            /* 读取日志 */
                            memcpy((puint8_t)&stLogPre, (puint8_t)pstLogInfoTmpPre, sizeof(LogInfo_t));

                            /* 未循环覆盖过一圈 */
                            if((stLogPre.uiSec == pstTimestamp->uiSec) && \
                                    (stLogPre.uiMicroSec == pstTimestamp->uiMicSec))
                            {
                                /* 没有新Log产生 */
                                if(usLogicPos == s_stQueueInfo.usTail)
                                {
                                    iLogIndex = READ_LOG_QUEUE_NO_NEW_RECORD;
                                }
                                else
                                {
                                    /* 有新Log产生 */
                            ...
                            else
                            {
                                ...
                                *puiNextIndex = (usLogicPos + 1) % MAX_LOG_NUM;
                                ...
                                if(usLogicPos <= LOG_INFO_SEC9_MAX_INDEX)
                                {
                                    s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC9_START_ADDR;
                                    pstLogInfoTmp = &s_pstBaseLogInfo[usLogicPos];
                                }
                                else
                                {
                                    s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SECA_START_ADDR;
                                    pstLogInfoTmp = &s_pstBaseLogInfo[usLogicPos-LOG_INFO_SECA_START_INDEX];
                                }
                                /* 读取日志 */
                                memcpy((puint8_t)pstLogInfo, (puint8_t)pstLogInfoTmp, sizeof(LogInfo_t));
    ...
    /* 记录读取位置 */
    if((iLogIndex == THE_LAST_LOG_IN_QUEUE) || (iLogIndex == LOG_QUEUE_IS_EMPTY_NOW))
    {
        s_stQueueInfo.iReadIndex = iLogIndex;
    }
    else
    {
        if((iLogIndex != READ_LOG_QUEUE_FAILED) && (iLogIndex != READ_LOG_QUEUE_LOCKED))
        {
            if(iLogIndex > s_stQueueInfo.iReadIndex)
            {
                s_stQueueInfo.iReadIndex = iLogIndex;
            }
        }
...
{
    s_stQueueInfo.usAlarm = LOG_SET_ALARM;
    s_pstQueueInfo->usAlarm = LOG_SET_ALARM;
    return;
}
...
{
    s_stQueueInfo.usAlarm = LOG_CLR_ALARM;
    s_pstQueueInfo->usAlarm = LOG_CLR_ALARM;
    return;
}
/*
***********************************************************************************************************************
...
{
    uint16_t usAlarm = 0;
    usAlarm = s_pstQueueInfo->usAlarm;
    return usAlarm;
}
...
static void UpdateLogAlarm(void)
{
    if(s_stQueueInfo.usQueueState == LOG_QUEUE_NOT_EMPTY_NOT_FULL)
    {
        if((s_stQueueInfo.iReadIndex == THE_LAST_LOG_IN_QUEUE) || (s_stQueueInfo.iReadIndex == LOG_QUEUE_IS_EMPTY_NOW))
        {
            ClearLogAlarm();
        }
        else
        {
            if((MAX_LOG_NUM - s_stQueueInfo.usTail + s_stQueueInfo.iReadIndex) > LOG_THRESHOLD_VAL)
            {
                ClearLogAlarm();
            }
            else
    ...
    else if(s_stQueueInfo.usQueueState == LOG_QUEUE_EMPTY)
    {
        ClearLogAlarm();
    }
    else //if(s_stQueueInfo.usQueueState == LOG_QUEUE_FULL)
    {
        if((s_stQueueInfo.iReadIndex == THE_LAST_LOG_IN_QUEUE) || (s_stQueueInfo.iReadIndex == LOG_QUEUE_IS_EMPTY_NOW))
        {
            ClearLogAlarm();
        ...
        {
            if(s_stQueueInfo.iReadIndex >= s_stQueueInfo.usTail)
            {
                if(s_stQueueInfo.iReadIndex - s_stQueueInfo.usTail > LOG_THRESHOLD_VAL)
                {
                    ClearLogAlarm();
                }
                else
...
static void WriteQueueInfoToSRAM(void)
{
    ...
    s_pstQueueInfo->usAlarm = s_stQueueInfo.usAlarm;
    s_pstQueueInfo->iReadIndex = s_stQueueInfo.iReadIndex;
...
static void ReadQueueInfoFromSRAM(void)
{
    ...
    s_stQueueInfo.usAlarm = s_pstQueueInfo->usAlarm;
    s_stQueueInfo.iReadIndex = s_pstQueueInfo->iReadIndex;
