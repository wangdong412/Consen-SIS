...
/* File Updata Task Info. */
static FileUPTaskInfo_t s_stFUpTaskInfo;

/* Command */
static int32_t s_iShmID;
static uint8_t *s_pucShmCmd;
static SysCmdInfo_t s_stNetCmdInfo[NUM_OF_NETWORK_PORT];
...
/* Network Adapter */
static bool_t OpenNetAdapter( NetworkPortType_t emNet, SysCmdInfo_t *pstCmdInfo );
static bool_t CloseNetAdapter( NetworkPortType_t emNet, SysCmdInfo_t *pstCmdInfo );
static bool_t SetLocalIPAddr( NetworkPortType_t emNet, uint32_t uiIPAddr, SysCmdInfo_t *pstCmdInfo );
static bool_t SetLocalNetMask( NetworkPortType_t emNet, uint32_t uiNetMask, SysCmdInfo_t *pstCmdInfo );
static bool_t SetLocalGateWay( NetworkPortType_t emNet, uint32_t uiNetMask, SysCmdInfo_t *pstCmdInfo );
static bool_t HandleIPRoute( NetworkPortType_t emNet, uint32_t uiIPAddr, SysCmdInfo_t *pstCmdInfo );
...
static void ClearNetConfigInfo( bool_t *pbClrDone );
static void ClearLocalConfigInfo( void );
static void ClearNetworkResource( NetworkPortType_t emNet );
...
static void CreateModbusTCPListenSocket( NetworkPortType_t emNet, uint32_t uiIPAddr, uint16_t usPort,
                                         bool_t bNonBlock, bool_t bCloExec, SOCKET *phSocket );
...
/* File Update Task */
static void InitFileUpTask( void );
...
void ExternalCommInit( void )
{
    ...
    for( emNet = NET1; emNet < NUM_OF_NETWORK_PORT; emNet++ )
    {
        ...
        if( SetLocalIPAddr( emNet, uiIPAddr, &s_stNetCmdInfo[emNet]) &&
            SetLocalNetMask( emNet, (uint32_t)(DEF_SUBNET_MASK), &s_stNetCmdInfo[emNet]))
        {
            /* Wait command finished */
            while((int32_t)(ESC_FINISH_NOT) == ExecSysCmd( &s_stNetCmdInfo[emNet], s_pucShmCmd, s_iShmID ))
            {
                usleep( 10000 );  /* 10ms */
            }

            /* Wait network ready: 10s */
            usleep( 10000000 );
            DEBUG_RTS( "Set Net[%d] Success\n", emNet+1 );

            memset( &s_stNetCmdInfo[emNet], 0, sizeof(SysCmdInfo_t));
            if( !HandleIPRoute( emNet, uiIPAddr, &s_stNetCmdInfo[emNet]))
            {
            ...
            else
            {
                ...
                CreateNetworkSocket( emNet, uiIPAddr );
            }
        }
        else
        {
...
{
    static bool_t s_bClrLocalDone = false;  /* Clear Local Info. except network */
    static bool_t s_bClrNetDone = false;    /* Clear Network Info. */
    ...
    if( s_bEth1Close )
    {
        ...
        if( 1 == s_uiEth1CycCnt )
        {
            ...
            /* Down */
            if( !CloseNetAdapter( NET1, &s_stNetCmdInfo[NET1]))
            {
        ...
        if( s_uiEth1CycCnt >= (uint32_t)(ETH_CHK_INTL_CYC_CNT))
        {
            ...
            /* UP */
            if( !OpenNetAdapter( NET1, &s_stNetCmdInfo[NET1]))
            {
                DEBUG_RTS( "Eth1 Up Error!\n" );
            }

            /* IP Route */
            if( !HandleIPRoute( NET1, GetIPAddr(NET1), &s_stNetCmdInfo[NET1]))
            {
    ...
    if( s_bEth2Close )
    {
        ...
        if( 1 == s_uiEth2CycCnt )
        {
            ...
            /* Down */
            if( !CloseNetAdapter( NET2, &s_stNetCmdInfo[NET2]))
            {
        ...
        if( s_uiEth2CycCnt >= (uint32_t)(ETH_CHK_INTL_CYC_CNT))
        {
            ...
            /* UP */
            if( !OpenNetAdapter( NET2, &s_stNetCmdInfo[NET2]))
            {
                DEBUG_RTS( "Eth2 Up Error!\n" );
            }

            /* IP Route */
            if( !HandleIPRoute( NET2, GetIPAddr(NET2), &s_stNetCmdInfo[NET2]))
            {
    ...
    else
    {
        ...
        {
            ClearLocalInfo();
            s_bClrLocalDone = true;
        }

        if( !s_bClrNetDone )
        {
            ClearNetConfigInfo( &s_bClrNetDone );
        }
    }
    ...
    /* System Command */
    if( s_stNetCmdInfo[NET1].ucCmdCnt > 0U )
    {
        emNet = NET1;
    }
    else if( s_stNetCmdInfo[NET2].ucCmdCnt > 0U )
    {
        emNet = NET2;
    }
    else
    {
        emNet = INVALID_NETWORK_PORT;
    }

    if( emNet != INVALID_NETWORK_PORT )
    {
        /* Network Configuration */
        if((int32_t)(ESC_FINISH_OK) == ExecSysCmd( &s_stNetCmdInfo[emNet], s_pucShmCmd, s_iShmID ))
        {
            DEBUG_RTS( "ExecSysCmd fin ok\n" );
            ...
            if( pstNetInfo->bIPChange )
            {
                ...
                s_stNetCmdInfo[emNet].bIPChange = true;
                s_stNetCmdInfo[emNet].uiWaitNetCycCnt = 0U;
            }
        }
    }
    ...
    for( emNet = NET1; emNet < NUM_OF_NETWORK_PORT; emNet++ )
    {
        if( s_stNetCmdInfo[emNet].bIPChange )
        {
            if( s_stNetCmdInfo[emNet].uiWaitNetCycCnt >= (uint32_t)(ETH_CREATE_WAIT_CYC_CNT))
            {
                s_stNetCmdInfo[emNet].bIPChange = false;
                s_stNetCmdInfo[emNet].uiWaitNetCycCnt = 0U;
                pstNetInfo = &s_stNetInfo[emNet];
                ...
                /* Handle connect with MBTCPSlave */
                if( pstNetInfo->bConnMBSlave )
                {
                    pstNetInfo->bConnMBSlave = false;
            ...
            {
                s_stNetCmdInfo[emNet].uiWaitNetCycCnt++;
            }
        }
    }
...
void GetFileUpTaskInfo( FileUPTaskInfo_t *pstInfo )
{
    if( pstInfo != NULL )
    {
        memcpy( pstInfo, &s_stFUpTaskInfo, sizeof(FileUPTaskInfo_t));
    }
...
{
    if((uiShmOffset < (uint32_t)(SHM_FUP_SIZE)) && (pucData != NULL))
    {
        memcpy( &s_stFUpTaskInfo.pucShmAddr[uiShmOffset], &pucData[0], uiDataLen );
    }
}
...
static bool_t OpenNetAdapter( NetworkPortType_t emNet, SysCmdInfo_t *pstCmdInfo )
{
        ...
        (pstCmdInfo != NULL))
    {
        ...
        /* Pathname */
        strcpy( &stCmd.cPrgPath[0], "/sbin/ifconfig" );

        /* Argv: "ifconfig eth1/2 up" */
        strcpy( &stCmd.cArgv[0][0], "ifconfig" );
        sprintf( &stCmd.cArgv[1][0], "eth%d", emNet+1 );
        strcpy( &stCmd.cArgv[2][0], "up");

        /* Add */
        bResult = AddSysCmd( &stCmd, pstCmdInfo );
    }
...
static bool_t CloseNetAdapter( NetworkPortType_t emNet, SysCmdInfo_t *pstCmdInfo )
{
        ...
        (pstCmdInfo != NULL))
    {
        ...
        /* Argv: "ifconfig eth1/2 down" */
        strcpy( &stCmd.cArgv[0][0], "ifconfig" );
        sprintf( &stCmd.cArgv[1][0], "eth%d", emNet+1 );
        strcpy( &stCmd.cArgv[2][0], "down");
...
static bool_t SetLocalIPAddr( NetworkPortType_t emNet, uint32_t uiIPAddr, SysCmdInfo_t *pstCmdInfo )
{
        ...
        (pstCmdInfo != NULL))
    {
        ...
        /* Argv: "ifconfig eth1/2 x.x.x.x" */
        strcpy( &stCmd.cArgv[0][0], "ifconfig" );
        sprintf( &stCmd.cArgv[1][0], "eth%d", emNet+1 );
        strcpy( &stCmd.cArgv[2][0], inet_ntoa( stAddr ));
...
static bool_t SetLocalNetMask( NetworkPortType_t emNet, uint32_t uiNetMask, SysCmdInfo_t *pstCmdInfo )
{
       ...
       (pstCmdInfo != NULL))
    {
        ...
        /* Argv: "ifconfig eth1/2 netmask x.x.x.x" */
        strcpy( &stCmd.cArgv[0][0], "ifconfig" );
        sprintf( &stCmd.cArgv[1][0], "eth%d", emNet+1 );
        strcpy( &stCmd.cArgv[2][0], "netmask" );
        strcpy( &stCmd.cArgv[3][0], inet_ntoa( stAddr ));
...
static bool_t SetLocalGateWay( NetworkPortType_t emNet, uint32_t uiGateWay, SysCmdInfo_t *pstCmdInfo )
{
    bool_t bResult = false;
    struct in_addr stAddr;   /* GateWay */
    SysCmd_t stCmd;
       ...
       (pstCmdInfo != NULL))
    {
        ...
        /* Delete default gw */
        /* Pathname */
        strcpy( &stCmd.cPrgPath[0], "/sbin/route" );

        /* Argv: "route del default dev %s", NetName */
        strcpy( &stCmd.cArgv[0][0], "route" );
        strcpy( &stCmd.cArgv[1][0], "del" );
        strcpy( &stCmd.cArgv[2][0], "default" );
        strcpy( &stCmd.cArgv[3][0], "dev" );
        sprintf( &stCmd.cArgv[4][0], "eth%d", emNet+1 );
        ...
        if( bResult )
        {
            memset( &stCmd, 0, sizeof(SysCmd_t));
            memset( &stAddr, 0, sizeof(struct in_addr));
            stAddr.s_addr = htonl( uiGateWay );

            /* Pathname */
            strcpy( &stCmd.cPrgPath[0], "/sbin/route" );

            /* Argv: "route add default gw %s %s", inet_ntoa( stAddr ), cNetName */
            strcpy( &stCmd.cArgv[0][0], "route" );
            strcpy( &stCmd.cArgv[1][0], "add" );
            strcpy( &stCmd.cArgv[2][0], "default" );
            strcpy( &stCmd.cArgv[3][0], "gw" );
            strcpy( &stCmd.cArgv[4][0], inet_ntoa( stAddr ));
            sprintf( &stCmd.cArgv[5][0], "eth%d", emNet+1 );
...
static bool_t HandleIPRoute( NetworkPortType_t emNet, uint32_t uiIPAddr, SysCmdInfo_t *pstCmdInfo )
{
       ...
       (pstCmdInfo != NULL))
    {
        ...
        /* Pathname */
        strcpy( &stCmd.cPrgPath[0], "/sbin/ip" );

        /* Argv: "ip route flush table %d", ucTableNum */
        strcpy( &stCmd.cArgv[0][0], "ip" );
        strcpy( &stCmd.cArgv[1][0], "route" );
        strcpy( &stCmd.cArgv[2][0], "flush" );
        strcpy( &stCmd.cArgv[3][0], "table" );
        sprintf( &stCmd.cArgv[4][0], "%d", ucTableNum );
        ...
        if( bResult )
        {
            ...
            /* Argv: "ip route add default dev %s table %d", cNetName, ucTableNum */
            strcpy( &stCmd.cArgv[0][0], "ip" );
            strcpy( &stCmd.cArgv[1][0], "route" );
            strcpy( &stCmd.cArgv[2][0], "add" );
            strcpy( &stCmd.cArgv[3][0], "default" );
            strcpy( &stCmd.cArgv[4][0], "dev" );
            sprintf( &stCmd.cArgv[5][0], "eth%d", emNet+1 );
            strcpy( &stCmd.cArgv[6][0], "table" );
            sprintf( &stCmd.cArgv[7][0], "%d", ucTableNum );
        ...
        if( bResult )
        {
            ...
            /* Argv: "ip rule add from %s table %d", inet_ntoa( stAddr ), ucTableNum */
            strcpy( &stCmd.cArgv[0][0], "ip" );
            strcpy( &stCmd.cArgv[1][0], "rule" );
            strcpy( &stCmd.cArgv[2][0], "add" );
            strcpy( &stCmd.cArgv[3][0], "from" );
            strcpy( &stCmd.cArgv[4][0], inet_ntoa( stAddr ));
            strcpy( &stCmd.cArgv[5][0], "table" );
            sprintf( &stCmd.cArgv[6][0], "%d", ucTableNum );
        ...
        if( bResult )
        {
            ...
            /* Argv: "ip route flush cache" */
            strcpy( &stCmd.cArgv[0][0], "ip" );
            strcpy( &stCmd.cArgv[1][0], "route" );
            strcpy( &stCmd.cArgv[2][0], "flush" );
            strcpy( &stCmd.cArgv[3][0], "cache" );
...
/*
***********************************************************************************************************************
* 函数名称: ClearNetConfigInfo
*
* 功能描述: 清除本地保存的网口配置信息
*
* 输入参数: pbClrDone: 清除完成标志
*
* 输出参数: 无
*
* 返 回 值: 无
...
static void ClearNetConfigInfo( bool_t *pbClrDone )
{
    NetworkPortType_t emNet = NET1;
    ExtCommNetworkInfo_t *pstNetInfo = NULL;
    ExtCommNetConfigInfo_t *pstNetCfg = NULL;
    uint32_t uiIPAddr = 0U;
    ProjVerInfo_t stPMPrjVerInfo;
    DWStatus_t emDWStatus = DWS_NON;
    bool_t bClrNetCfg = false;

    if( pbClrDone != NULL )
    {
        /* Clear Net Cfg only when PM no project & not downloading */
        if( SysGetPMPrjVerInfo( &stPMPrjVerInfo ))
        {
            if((0 == stPMPrjVerInfo.uiMainVer) &&
               (0 == stPMPrjVerInfo.uiMinorVer) &&
               (0 == stPMPrjVerInfo.uiProjID) &&
               (0 == stPMPrjVerInfo.uiProjCRC))
            {
                emDWStatus = (DWStatus_t)SysGetPMDownloadStatus();
                if( emDWStatus != DWS_DWING )
                {
                    bClrNetCfg = true;
                }
            }
        ...
        {
            bClrNetCfg = true;
            DEBUG_RTS( "##ClrFlag %d: SysGetPMPrjVerInfo error(InterComm error)\n", bClrNetCfg );
        }

        DEBUG_RTS( "##PMPrjCRC %x, DWSts %d, ClrFlag %d\n", stPMPrjVerInfo.uiProjCRC, emDWStatus, bClrNetCfg );
        ...
        if( bClrNetCfg )
        {
            for( emNet = NET1; emNet < NUM_OF_NETWORK_PORT; emNet++ )
            {
                /* Network Port Configuration: Compare */
                pstNetInfo = &s_stNetInfo[emNet];
                pstNetInfo->ucIndex = (pstNetInfo->ucIndex + 1) % (uint8_t)(LOCAL_CFG_VER_CNT);
                pstNetInfo->uiCMCfgCRC = 0U;
                pstNetInfo->uiCMCfgVer = 0U;
                ...
                /* Reset Net Cfg */
                pstNetCfg = &( pstNetInfo->stNetConfig[pstNetInfo->ucIndex]);
                pstNetCfg->emActive = ACTIVE;
                pstNetCfg->bUpdate = true;
                pstNetCfg->uiIPAddr = uiIPAddr;
                pstNetCfg->uiSubnetMask = (uint32_t)(DEF_SUBNET_MASK);
                pstNetCfg->uiGateway = 0U;
...
static void ClearLocalConfigInfo( void )
{
    ...
    ExtCommModbusTCPConfigInfo_t *pstMTCfg = NULL;
    ...
    for( emNet = NET1; emNet < NUM_OF_NETWORK_PORT; emNet++ )
    {
        ...
        pstMBTCPInfo->ucIndex = (pstMBTCPInfo->ucIndex + 1) % (uint8_t)(LOCAL_CFG_VER_CNT);
        pstMBTCPInfo->uiCMCfgCRC = 0U;
        pstMBTCPInfo->uiCMCfgVer = 0U;
        memset( &pstMBTCPInfo->stRTDataVer, 0, sizeof(RTDataVerInfo_t));

        /* Reset Modbus TCP Cfg */
        pstMTCfg = &( pstMBTCPInfo->stMBTCPConfig[pstMBTCPInfo->ucIndex]);
        pstMTCfg->emActive = ACTIVE;
        pstMTCfg->bUpdate = true;
        pstMTCfg->ucProtocol = (uint8_t)(MODBUS_TCP_NONE);
        pstMTCfg->uiIPAddr = 0U;
...
static void HandleConfiguration( void )
{
    ...
    for( emNet = NET1; emNet < NUM_OF_NETWORK_PORT; emNet++ )
    {
        ...
        if( HandleNetworkConfig( emNet, pstNetInfo ))
        {
            ...
            LogWrite( (uint32_t)(LOGID_EXCOMM_CFG_SWITCH_NET));
        ...
        if( HandleModbusTCPConfig( emNet, pstMBTCPInfo ))
        {
            ...
            LogWrite( (uint32_t)(LOGID_EXCOMM_CFG_SWITCH_MBTCP));
    ...
    if( HandleModbusMstrInfo( &s_stMBMstrCfgInfo, &s_uiMBMstrCfgOffset, s_ucLocalCMSlotID ))
    {
        ...
        LogWrite( (uint32_t)(LOGID_EXCOMM_CFG_SWITCH_MBMSTR));
                       ...
                       ( ACTIVE == stNetCfgInfo.emActive ))
                    {
                            ...
                            ((0U == pstNetInfo->uiCMCfgCRC ) && (0U == pstNetInfo->uiCMCfgVer)))
                        {
                            ...
                            else
                            {
                                ...
                                LogWrite( (uint32_t)(LOGID_EXCOMM_E_CFG_NET));
                            }
                        }
                    }
                }
       ...
       (pstNetInfo != NULL))
    {
        ...
        if( ucCfgIndex < (uint8_t)(LOCAL_CFG_VER_CNT))
        {
            ...
            if( pstNewCfg->uiIPAddr != pstOldCfg->uiIPAddr )
            {
                ...
                /* Clear old resource */
                ClearNetworkResource( emNet );

                /* IP Address */
                if( !SetLocalIPAddr( emNet, pstNewCfg->uiIPAddr, &s_stNetCmdInfo[emNet]))
                {
            ...
            if( pstNewCfg->uiSubnetMask != pstOldCfg->uiSubnetMask )
            {
                ...
                /* Sub-NetMask */
                if( !SetLocalNetMask( emNet, pstNewCfg->uiSubnetMask, &s_stNetCmdInfo[emNet]))
                {
            ...
            if( pstNewCfg->uiGateway != pstOldCfg->uiGateway )
            {
                ...
                /* GateWay */
                if( !SetLocalGateWay( emNet, pstNewCfg->uiGateway, &s_stNetCmdInfo[emNet]))
                {
            ...
            if( bSetIPRoute )
            {
                if( !HandleIPRoute( emNet, pstNewCfg->uiIPAddr, &s_stNetCmdInfo[emNet]))
                {
        ...
        if( ucCfgIndex < (uint8_t)(LOCAL_CFG_VER_CNT))
        {
            ...
            if( pstNewCfg->ucProtocol != pstOldCfg->ucProtocol )
            {
                ...
                else /* SLAVE / MASTER_SLAVE */
                {
                    ...
                    CreateListenSocket( uiIPAddr, s_stSockInfo[emNet].stLisMBSock.usPort, true, true,
...
/*
***********************************************************************************************************************
* 函数名称: ClearNetworkResource
*
* 功能描述: 网口IP地址变化时，清除与之相关的资源
*
* 输入参数: emNet：网口对应的端口号
*
* 输出参数: 无
...
static void ClearNetworkResource( NetworkPortType_t emNet )
{
    DEBUG_RTS( "ClearNetworkResource: Net %d\n", emNet+1 );
    uint8_t ucIndex = 0U;

    if(( emNet >= NET1 ) && ( emNet < NUM_OF_NETWORK_PORT ))
    {
        /* Close Network Socket */
        CloseNetworkSocket( emNet );
                                         ...
                                         bool_t bNonBlock, bool_t bCloExec, SOCKET *phSocket )
{
       ...
       (phSocket != NULL))
    {
        ...
        if( ucCfgIndex < (uint8_t)(LOCAL_CFG_VER_CNT))
        {
            ...
            {
                CreateListenSocket( uiIPAddr, usPort, bNonBlock, bCloExec, phSocket );
            }
        }
    }
...
static void InitFileUpTask( void )
{
    ...
    memset( &s_stFUpTaskInfo, 0, sizeof(FileUPTaskInfo_t));
    s_stFUpTaskInfo.iMsgQID = -1;

    /* Shared Memory */
    /* Create if not exist; If exists, return fail; Not block; User/Group RW */
    iShmID = shmget( IPC_PRIVATE, (uint32_t)(SHM_FUP_SIZE), \
                     IPC_CREAT|IPC_EXCL|S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP );
    if( -1 == iShmID )
    {
        DEBUG_RTS( "shmget failed, errno: %d!\n", errno );
        bInitRet = false;
    }
    ...
    {
        /* Attach, Read/Write */
        s_stFUpTaskInfo.pucShmAddr = (uint8_t *)shmat( iShmID, NULL, 0 );
        if((uint8_t *)-1 == s_stFUpTaskInfo.pucShmAddr )
        {
            DEBUG_RTS( "shmat failed, errno: %d!\n", errno );
            s_stFUpTaskInfo.pucShmAddr = NULL;
            bInitRet = false;
        }
    }
    ...
    {
        /* IPC Message Queue */
        /* Create if not exist; If exists, return fail; User/Group RW */
        s_stFUpTaskInfo.iMsgQID = msgget( IPC_PRIVATE, IPC_CREAT|IPC_EXCL|S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP );
        if( -1 == s_stFUpTaskInfo.iMsgQID )
        {
            DEBUG_RTS( "msgget failed, errno: %d!\n", errno );
            bInitRet = false;
        }
    }
    ...
    if( bInitRet )
    {
        s_stFUpTaskInfo.bFileUpTask = true;

        /* Fork write file process */
        iPID = fork();
        if( -1 == iPID )
        {
            DEBUG_RTS( "Fork write file process failed, errno %d\n", errno );
            bInitRet = false;
            s_stFUpTaskInfo.bFileUpTask = false;
            LogWrite( (uint32_t)(LOGID_EXCOMM_E_INIT_FUTASK));
        }
        else if( 0 == iPID )
        {
            /* Child Process */
            DEBUG_RTS( "Enter write file child process\n" );
            ...
            {
                DEBUG_RTS( "Bind FU to CPU0 failed: %d!\n", errno );
            }

            /* Set scheduler */
            stParam.sched_priority = 0;  /* Must 0 for non-real-time process */
            if( -1 == sched_setscheduler( 0, (int32_t)(POLICY_FU), &stParam ))
            {
                DEBUG_RTS( "Set FU scheduler failed: %d!\n", errno );
            }

            /* Set Priority */
            if( -1 == setpriority( PRIO_PROCESS, 0, (int32_t)(PRIORITY_FU)))
            {
                DEBUG_RTS( "Fork write file process: setpriority failed %d\n", errno );
            }

            FWUPTask( iShmID, s_stFUpTaskInfo.iMsgQID );
        }
        else
        {
            /* Parent Process */
            usleep( 1000 );  /* 1ms */
            DEBUG_RTS( "Fork write file process: Parent process, child PID %d\n", iPID );
        }
    }
...
static void InitCommand( void )
{
    ...
    /* NetCmdInfo */
    memset( &s_stNetCmdInfo[0], 0, sizeof(SysCmdInfo_t)*(NUM_OF_NETWORK_PORT));
    ...
    if( -1 == s_iShmID )
    {
        DEBUG_RTS( "InitCommand: shmget failed, errno %d\n", errno );
        bInitRet = false;
    }
    else
    {
        DEBUG_RTS( "InitCommand: ShmID %d\n", s_iShmID );
    }
    ...
    if( bInitRet )
    {
        ...
        {
            DEBUG_RTS( "InitCommand: shmat failed, errno %d\n", errno );
            s_pucShmCmd = NULL;
            bInitRet = false;
        }
    }
