...
bool_t RecvOprRetMsg( int32_t iMsgQID, MsgType_t emRetMsgType, FileUPSts_t *pemFUSts )
{
       ...
       ((MSGT_FWUP_OPR_RET == emRetMsgType) || (MSGT_SRCUP_OPR_RET == emRetMsgType)))
    {
        ...
        else
        {
            ...
            {
                if( OPRT_UPINDEX == stRetMsg.stOprRetInfo.ucOprType )
                {
                    if( pemFUSts != NULL )
                    {
                        *pemFUSts = FUS_OK;
...
static void FWUPInit( int32_t iShmID )
{
    ...
    /**** FPGA ****/
    /* "FPGAFlag" file: R/W, Create if not exist, flush after write */
    s_iFPGAInfoFD = open( "/home/root/FPGAFlag", O_RDWR|O_CREAT|O_SYNC, S_IRWXU|S_IRUSR|S_IWUSR|S_IXUSR );
    if((int32_t)(INVALID_FD) == s_iFPGAInfoFD )
    {
        DEBUG_FU( "Open FPGAInfo file failed, errno: %d!\n", errno );
    }

    /* ".._v01" file: R/W, Create if not exist, flush after write */
    s_iFPGA1FD = open( "/home/root/cm01_u103_v01.rbf", O_RDWR|O_CREAT|O_SYNC, S_IRWXU|S_IRUSR|S_IWUSR|S_IXUSR );
    if((int32_t)(INVALID_FD) == s_iFPGA1FD )
    {
        DEBUG_FU( "Open FPGA1 file failed, errno: %d!\n", errno );
    }

    /* ".._v02" file: R/W, Create if not exist, flush after write */
    s_iFPGA2FD = open( "/home/root/cm01_u103_v02.rbf", O_RDWR|O_CREAT|O_SYNC, S_IRWXU|S_IRUSR|S_IWUSR|S_IXUSR );
...
static bool_t FPGAUpdateStart( void )
{
    ...
    /* Read FPGA index: binary */
    iLen = ReadFile( s_iFPGAInfoFD, 0U, &uiFPGAIndex, sizeof(uint32_t));
    if( iLen >= 0 )
    {
        if( iLen > 0 )
        {
            DEBUG_FU( "FPGAUpdateStart: FPGAIndex %d\n", uiFPGAIndex );
        }
        else
        {
            uiFPGAIndex = 1U;
...
static bool_t UpdateFPGAIndex( void )
{
    ...
    /* Write index to its file: binary */
    if( WriteFile( s_iFPGAInfoFD, 0U, &s_uiUPFPGAIndex, sizeof(uint32_t)))
    {
        bResult = true;
