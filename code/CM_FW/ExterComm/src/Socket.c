...
void CloseListenSocket( SOCKET *phSocket )
{
    ...
    DEBUG_SOCKET( "CloseListenSocket\n" );
       ...
       (phSocket != NULL))
    {
        ...
        if((int32_t)(INVALID_SOCKET) == hLisSock )
        {
            ...
            if( hLisSock != (int32_t)(INVALID_SOCKET))
            {
                ...
                if( bCloExec )
                {
                    if( !SetCloExec( hLisSock ))
                    {
                        DEBUG_SOCKET( "CreateListenSocket: SetCloExec failed!\n" );
                        CloseSocket( &hLisSock );
                    }
                }
       ...
       (phSocket != NULL))
    {
        ...
        if((int32_t)(INVALID_SOCKET) == hUDPSerSock )
        {
            ...
            if( hUDPSerSock != (int32_t)(INVALID_SOCKET))
            {
                ...
                if( bCloExec )
                {
                    if( !SetCloExec( hUDPSerSock ))
                    {
                        DEBUG_SOCKET( "CreateUDPServerSocket: SetCloExec failed!\n" );
                        CloseSocket( &hUDPSerSock );
                    }
                }
