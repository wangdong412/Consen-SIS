    ...
    if( iFD != -1 )
    {
        ...
        if( iFlags != -1 )
        {
            ...
            else
            {
                ...
                DEBUG_RTS( "SetNonBlockMode: fcntl(set) failed-%d!\n", errno );
                bResult = false;
            }
        }
        else
        {
            /* Record Error Info: fcntl, errno */
            DEBUG_RTS( "SetNonBlockMode: fcntl(get) failed-%d\n", errno );
            bResult = false;
        }
    }
...
bool_t SetCloExec( int32_t iFD )
{
    ...
    if( iFD != -1 )
    {
        ...
        if( iFlags != -1 )
        {
            if( fcntl( iFD, F_SETFL, iFlags | FD_CLOEXEC ) != -1 )
            {
                bResult = true;
            }
            else
            {
                /* Record Error Info: fcntl, errno */
                DEBUG_RTS( "SetCloExec: fcntl(set) failed-%d\n", errno );
                bResult = false;
            }
        }
        else
        {
            /* Record Error Info: fcntl, errno */
            DEBUG_RTS( "SetCloExec: fcntl(get) failed-%d\n", errno );
            bResult = false;
        }
    }
    ...
    if( pstTSyncCfg != NULL )
    {
        ...
        if( bValid && (ACTIVE == pstTSyncCfg->emActive))
        {
            ...
            if(((pstTSyncCfg->ucTimingSrc >= (uint8_t)(TSRC_NTP)) && (pstTSyncCfg->ucTimingSrc <= (uint8_t)(TSRC_NONE))) &&
               ((pstTSyncCfg->ucPulseSrc >= (uint8_t)(PSRC_NONE)) && (pstTSyncCfg->ucTimingSrc <= (uint8_t)(PSRC_LOCAL))) &&
...
static int32_t ExecSingleCmd( int32_t iShmID )
{
    ...
    if( iShmID != -1 )
    {
        ...
        else if( 0 == iPID )
        {
            ...
            if( -1 == sched_setscheduler( 0, (int32_t)(POLICY_CMD), &stParam ))
            {
                DEBUG_RTS( "Set cmd scheduler failed: %d!\n", errno );
            }
...
static void ChdExecCmd( int32_t iShmID )
{
    ...
    if( iShmID != -1 )
    {
        ...
        if( -1 == setpriority( PRIO_PROCESS, 0, (int32_t)(PRIORITY_CMD)))
        {
