...
static FileUPSts_t s_emFWUPSts;
...
static void DecodeReadFWUPStatus( ClientInfo_t *pstSWInfo, uint8_t const pucReq[], RespMsg_t *pstResp );
...
static uint16_t AddFWUPStatusData( uint8_t const pucReq[], uint8_t pucRespData[]);
...
void ClientCycle( void )
{
    ...
    if( RecvOprRetMsg( s_stFUPOprInfo.iMsgQID, MSGT_FWUP_OPR_RET, &s_emFWUPSts ))
    {
        LogWrite( (uint32_t)(LOGID_CLI_E_FWUP));
    }

    if( s_emFWUPSts != FUS_UPING )
    {
        SysSetFWUpdate( false );
    }
...
static bool_t CheckPMHandle( uint8_t const pucReq[])
{
    ...
    if( pucReq != NULL )
    {
        ...
        if(((uint16_t)(CLI_LOGIN) == usAType) ||
           ...
           ((uint16_t)(CLI_READ_FWUP_STS) == usAType))
        {
            bPMHandle = false;
        }
       ...
       (pstResp != NULL))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else if((uint16_t)(CLI_READ_FWUP_STS) == usAType )
                {
                    usDataLen = AddFWUPStatusData( &pucReq[0], &pucResp[usIndex]);
                    if( usDataLen > 0U )
                    {
                        usLength = (uint16_t)(APP_MSG_HEADER_LEN) + usDataLen;
                    }
                    ...
                    {
                        ucSubReason = (uint8_t)(ERR_ADD_PMFWUP_STATUS);
                        DEBUG_CLI( "AddFWUPStatusData Failed\n" );
                        //LogWrite( (uint32_t)(LOGID_CLI_E_ADD_PMFWUP_STATUS));
                    }
                }
...
static uint16_t AddDiagData( uint8_t const pucReq[], uint8_t pucRespData[])
{
    ...
    if((pucReq != NULL) && (pucRespData != NULL))
    {
        ...
        if( pstRDReq->ucPMID < (uint8_t)(NUM_OF_PM))
        {
            ...
            if( pstAppend != NULL )
            {
                uiDataIndex = sizeof( DiagInfoHead_t );  /* Index of First Section Data */
                ...
                /* Diag Info. Head */
                pstDIHead = (DiagInfoHead_t *)&pucRespData[0];
                pstDIHead->ucPMID = pstRDReq->ucPMID;  /* PMID */
                pstDIHead->ucSecCnt = ucSecCnt;        /* Number Of Sections */
                pstDIHead->usICStatus = SysGetLocalInterCommState((PMController_t)pstRDReq->ucPMID );
    ...
    if((pucReq != NULL) && (pucRespData != NULL))
    {
        ...
        if((uint8_t)(MID_CM) == pstREInfo->ucModuleID )
        {
                ...
                (pstREInfo->usGetIndex < (uint16_t)(MAX_LOG_NUM)))
            {
                ...
                pstResp->uiSec = pstREInfo->uiSec;
                pstResp->uiMicroSec = pstREInfo->uiMicroSec;
                ...
                else
                {
                    ...
                    uiSec = pstREInfo->uiSec;
                    uiMicroSec = pstREInfo->uiMicroSec;
                    while( usEventCnt < (uint32_t)(MAX_PER_READ_EVENT_CNT))
                    {
                        iResult = LogRead( &stLogInfo, usLogicPos, &usNextIndex, uiSec, uiMicroSec );
                        if( iResult >= 0 )
                        {
                            ...
                            /* Update time stamp */
                            pstResp->uiSec = stLogInfo.uiSec;
                            pstResp->uiMicroSec = stLogInfo.uiMicroSec;
                            uiSec = stLogInfo.uiSec;
                            uiMicroSec = stLogInfo.uiMicroSec;
                        }
                        else
                        {
...
static uint16_t AddFWUPStatusData( uint8_t const pucReq[], uint8_t pucRespData[])
{
    ...
    if((pucReq != NULL) && (pucRespData != NULL))
    {
        ...
        /* CM */
        pucRespData[uiDataIndex] = (uint8_t)s_emFWUPSts;
        uiDataIndex++;

        /* PMA */
        pstAppend = (SharedAppend_t const *)SysGetAppendInfoAddr( PMA );
        if( pstAppend != NULL )
        {
            pucRespData[uiDataIndex] = pstAppend->ucFWUpdateFlg;
            uiDataIndex++;
        }

        /* PMB */
        pstAppend = (SharedAppend_t const *)SysGetAppendInfoAddr( PMB );
        if( pstAppend != NULL )
        {
            pucRespData[uiDataIndex] = pstAppend->ucFWUpdateFlg;
            uiDataIndex++;
        ...
        /* PMC */
        pstAppend = (SharedAppend_t const *)SysGetAppendInfoAddr( PMC );
        if( pstAppend != NULL )
        {
            pucRespData[uiDataIndex] = pstAppend->ucFWUpdateFlg;
            uiDataIndex++;
       ...
       ( pstResp != NULL ))
    {
        ...
        else
        {
            ...
            else if((uint8_t)(AL_READ_ONLY) == pstSWInfo->ucAccAuth )
            {
                ...
                if((uint16_t)(CLI_CLR_DIAG) == usAType )
                {
        ...
        if((uint8_t)(ERR_SUCCESS) == ucSubReason )
        {
            ...
            else
            {
                ...
                if( SysSetMsg( SwitchPMID( emSelPMID ), CLIENT_REQ, pstReqMsg->ucBuff, pstReqMsg->stNetHead.usLen ))
                {
                    ...
                    {
                        DEBUG_CLI( "PMHandle: MsgLen %d, Data(Max Top 50B): \n", pstReqMsg->stNetHead.usLen );
...
static void DecodeCmd( ClientInfo_t *pstSWInfo, uint8_t const pucReq[], RespMsg_t *pstResp )
{
       ...
       (pstResp != NULL))
    {
        ...
        switch( usAType )
        {
            ...
            case (uint16_t)(CLI_READ_FWUP_STS):
            {
                DecodeReadFWUPStatus( pstSWInfo, pucReq, pstResp );
            }
            break;
                   ...
                   ((pstFWInfo->ucPrgType >= (uint8_t)(FW_CMFPGA)) && (pstFWInfo->ucPrgType <= (uint8_t)(FW_CMRTSCRC))))
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            /* Set updating only if its operation message was write successfully */
                            if((uint8_t)(ERR_SUCCESS) == ucSubReason )
                            {
                                s_emFWUPSts = FUS_UPING;
                                SysSetFWUpdate( true );
                            }
                        }
                    }
...
static void DecodeReadFWUPStatus( ClientInfo_t *pstSWInfo, uint8_t const pucReq[], RespMsg_t *pstResp )
{
    ...
    {
        usAType = GetReqActualType( pucReq );
        if((uint16_t)(CLI_READ_FWUP_STS) == usAType )
        {
            if( LOGIN == pstSWInfo->emLog )
            {
                DEBUG_CLI( "CliReadPMFWUPStatus OK\n" );
                ucSubReason = (uint8_t)(ERR_SUCCESS);
            }
            else
            {
       ...
       (s_ucCurIndex < (uint8_t)(APP_SW_CLIENT_NUM)))
    {
        ...
        if( PM_RESP == s_emResp )
        {
            ...
            if( 0U == s_stRespMsg.usLen )
            {
                ...
                if( s_uiWaitPMCycleCnt >= (uint32_t)(CLIENT_PM_TIMEOUT_CYCLE_CNT))
                {
                    ...
                    /* Clear Req */
                    if( !SysClearMsg( SwitchPMID(pstSWInfo->emSelPMID), CLIENT_REQ ))
                    {
                        //DEBUG_CLI( "PMHandle: SysClearMsg Failed, SelPMID-%d\n", emSelPMID );
                    }
        ...
        else
        {
            ...
            if( ucResult != (uint8_t)(SEND_SUCCESS))
            {
                ...
                /* Reset FWUpdate Flag if send failed */
                if( CM_RESP == s_emResp )
                {
                    usAType = GetReqActualType( &s_stRespMsg.ucBuff[0]);
                    if((usAType >= (uint16_t)(CLI_FWUP_START)) &&
                       (usAType <= (uint16_t)(CLI_FWUP_END)))
                    {
                        SysSetFWUpdate( false );
                    }
                }
