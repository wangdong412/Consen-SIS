...
#define CS1131_PM_TIMEOUT_CYCLE_CNT  (5*1000*1000/(MAIN_TASK_PERIOD))   /* PM应答CS1131命令的超时拍数：5s */
#define CS1131_SW_TIMEOUT_CYCLE_CNT  (30*1000*1000/(MAIN_TASK_PERIOD))  /* CS1131软件超时拍数: 30s */
...
#define CS1131_TIMING          (1042)  /* 1131校时服务 */
...
/* Timing Info. 校时时间结构体定义 */
typedef struct TimingInfoTag
{
    uint16_t usYear;     /* Year    年 */
    uint8_t ucMonth;     /* Month   月 */
    uint8_t ucDay;       /* Day     日 */
    uint8_t ucHour;      /* Hour    小时 */
    uint8_t ucMinute;    /* Minute  分钟 */
    uint8_t ucSecond;    /* Second  秒 */
    uint8_t ucReserve;   /* Reserve 预留 */
    uint32_t uiMilliSec; /* ms      毫秒 */
    uint32_t microsec;   /* us      微秒 */
}TimingInfo_t;
