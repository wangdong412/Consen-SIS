...
#define MBTCP_RESP_TO_CYCLE_CNT   (1*1000*1000/(MAIN_TASK_PERIOD))  /* Modbus TCP从站应答超时拍数，时间1S */
#define MBSERL_RESP_TO_CYCLE_CNT  (1*1000*1000/(MAIN_TASK_PERIOD))  /* Modbus Serial从站应答超时拍数，时间1S */
#define LOOPBLK_INVL_CYCLE_CNTS   (304*1000/(MAIN_TASK_PERIOD))     /* 单MB主站端口轮询块的间隔拍数，时间304ms */
...
/* Port Parameters Info.  端口参数信息结构体定义 */
typedef struct PortParaInfoTag
{
    bool_t bRTUProt;
    uint8_t ucReserve[3];
    CommPort_t emPort;
...
/* Master Make Message Info.  主站构造消息信息结构体定义 */
typedef struct MstrMakeMsgInfoTag
{
    uint8_t ucFPOrder;
    uint8_t ucBlkID;
    uint8_t ucReserve[2];
    CommPort_t emPort;
    RTDataVerInfo_t const *pstRTDataVer;
