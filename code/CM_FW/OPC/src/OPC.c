...
static void ClearOPCSysInfo( uint8_t ucSWIndex )
{
    if( ucSWIndex < (uint8_t)(APP_SW_OPC_NUM))
    {
        ...
        if( !SysResetAppSwLockedFlag( APP_OPC, ucSWIndex ))
        {
            ...
            LogWrite( (uint32_t)(LOGID_OPC_E_SYS_RST_LOCK));
        }
        ...
        if( !SysResetAppSwConnectFlag( APP_OPC, ucSWIndex ))
        {
            ...
            LogWrite( (uint32_t)(LOGID_OPC_E_SYS_RST_CONN));
        }
        ...
        if( !SysSetAppSwClearFlag( APP_OPC, ucSWIndex ))
        {
            ...
            LogWrite( (uint32_t)(LOGID_OPC_E_SYS_SET_CLR));
        }
...
static void HandleConfiguration( void )
{
    ...
    if( HandleAccessCtrlInfo( &s_stAccCtrlInfo ))
    {
        ...
        LogWrite( (uint32_t)(LOGID_OPC_CFG_SWITCH_ACCESS_CTRL));
    ...
    for( emNet = NET1; emNet < NUM_OF_NETWORK_PORT; emNet++ )
    {
        ...
        if( HandleNetworkInfo( emNet, pstNetInfo ))
        {
            ...
            LogWrite( (uint32_t)(LOGID_OPC_CFG_SWITCH_NET));
        }
    }
    ...
    if( HandleOPCVarTblInfo())
    {
        ...
        LogWrite( (uint32_t)(LOGID_OPC_CFG_SWITCH_VAR_TABLE));
...
static bool_t HandleOPCVarTblInfo( void )
{
    ...
    if( s_uiOPCCfgOffset > 0U )
    {
        ...
        if( !GetOPCConfigInfo( &bComplete ))
        {
            ...
            LogWrite( (uint32_t)(LOGID_OPC_E_COPY_OPC_VAR_TABLE));
            ClearCurOPCCfg();
        }
        else
        {
    ...
    if( bComplete )
    {
        if( !CheckOPCVarCfgInfo())
        {
            ...
            LogWrite( (uint32_t)(LOGID_OPC_E_CFG_VAR_TABLE));
        }
        else
        {
            /* Valid */
                       ...
                       ( ACTIVE == stNetCfgInfo.emActive ))
                    {
                            ...
                            ((0U == pstNetInfo->uiCMCfgCRC ) && (0U == pstNetInfo->uiCMCfgVer)))
                        {
                            ...
                            else
                            {
                                ...
                                LogWrite( (uint32_t)(LOGID_OPC_E_CFG_NET));
                            }
                        }
                    }
                }
    ...
    if( ucSWIndex < (uint8_t)(APP_SW_OPC_NUM))
    {
        if( SysGetAppSwConnectFlag( APP_OPC, ucSWIndex))  /* Connect */
        {
            ...
            if( s_stOPCInfo[ucSWIndex].uiWaitReqCycleCnt >= (uint32_t)(OPC_SW_TIMEOUT_CYCLE_CNT))
            {
                ...
                LogWrite( (uint32_t)(LOGID_OPC_E_SWTO));
    ...
    while( ucCheckCnt < (uint8_t)(APP_SW_OPC_NUM))
    {
        ...
        if((usLen > 0U) && (s_stReqMsg.stNetHead.usLen > 0U))
        {
            ...
            else
            {
                ...
                //LogWrite( (uint32_t)(LOGID_OPC_E_CMD));
                ...
                if( !MakeRespMsg( &s_stOPCInfo[s_ucCurIndex], ucSubReason, s_stReqMsg.ucBuff, &s_stRespMsg ))  /* Make Response Message */
                {
                    ...
                    LogWrite( (uint32_t)(LOGID_OPC_E_MAKE_RESP));
                }
            ...
            if( !SysSetAppSwLockedFlag( APP_OPC, s_ucCurIndex ))
            {
                ...
                LogWrite( (uint32_t)(LOGID_OPC_E_SYS_SET_LOCK));
            }
       ...
       (pstSWInfo != NULL))
    {
        ...
        if( pstSWInfo->uiIPAddr != 0U )
        {
               ...
               (pstSWInfo->emNetwork != pstReqMsg->stNetHead.emNetwork))
            {
                ...
                //LogWrite( (uint32_t)(LOGID_OPC_E_SW_SRCADDR));
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    //LogWrite( (uint32_t)(LOGID_OPC_E_ADD_ONLINE_DATA));
                }
...
static uint16_t AddOPCOnlineData( OPCInfo_t *pstSWInfo, uint8_t pucRespData[], bool_t *pbCMBus )
{
    ...
    OPCReadRespHead_t *pstRespHead = NULL;  /* Response Head */
    OPCVarInfo_t const *pstOPCVarInfo = NULL;
    OPCVar_t const *pstVar = NULL;
            ...
            if( SelectRTData( &s_emLastPMID, pbCMBus ))
            {
                   ...
                   (pucQAddr != NULL))
                {
                    ...
                    if( usDataIndex > 0U )
                    {
                        ...
                        pstRespHead = (OPCReadRespHead_t *)&pucRespData[0U];
                        pstRespHead->uiSec = stTime.uiSecond;            /* Second */
                        pstRespHead->usMilliSec = stTime.usMiliSecond;   /* Millisecond */
                        pstRespHead->usSeq = pstSWInfo->usSequence++;    /* Sequence */
                        pstRespHead->usTotalNum = s_usTotalMsgNum;       /* Total Message Number */
                        pstRespHead->usMsgIndex = pstSWInfo->usMsgIndex; /* Message Index */
                        pstRespHead->uiTblCRC = pstOPCVarInfo->uiCRC;    /* OPC Variable Table CRC */
                        pstRespHead->usVarCnts = usAddVarCnt;            /* Variable Count */
                        pstRespHead->usFirIndex = pstSWInfo->usVarIndex; /* Variable Index */

                        DEBUG_OPC( "OnlineData: Sq %d, TolNum %d, MsgIndex %d, CRC 0x%x, VarCnts %d, FirIndex %d\n", \
                                   pstRespHead->usSeq, pstRespHead->usTotalNum, pstRespHead->usMsgIndex, \
                                   pstRespHead->uiTblCRC, pstRespHead->usVarCnts, pstRespHead->usFirIndex );
                ...
                else
                {
                    ...
                    LogWrite( (uint32_t)(LOGID_OPC_E_SYS_GET_RTDATA_ADDR));
                }
            }
        }
    }
...
static void PMHandle( OPCInfo_t *pstSWInfo, NetMsg_t *pstReqMsg, RespMsg_t *pstResp, bool_t bCMHasCfg )
{
       ...
       (pstResp != NULL))
    {
        if( pstSWInfo->emLog != LOGIN )
        {
            ...
            //LogWrite( (uint32_t)(LOGID_OPC_E_NOTLOGIN));
        ...
        else
        {
            if( bCMHasCfg )
            {
                if((uint8_t)(AL_DENY) == pstSWInfo->ucAccAuth )
                {
                    ...
                    //LogWrite( (uint32_t)(LOGID_OPC_E_ACCESS_DENY));
                ...
                else if((uint8_t)(AL_READ_ONLY) == pstSWInfo->ucAccAuth )
                {
                    ...
                    //LogWrite( (uint32_t)(LOGID_OPC_E_ACCESS_RO));
                    ucSubReason = (uint8_t)(ERR_ACCESS);  /* No Access Authority */
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        //LogWrite( (uint32_t)(LOGID_OPC_E_NET_NOT_WRITEABLE));
        ...
        if((uint8_t)(ERR_SUCCESS) == ucSubReason )
        {
            ...
            if( SELPM_NONE == emSelPMID )
            {
                ...
                //LogWrite( (uint32_t)(LOGID_OPC_E_CMBUS));
            ...
            else
            {
                ...
                if( SysSetMsg( SwitchPMID( emSelPMID ), OPC_REQ, pstReqMsg->ucBuff, pstReqMsg->stNetHead.usLen ))
                {
                    ...
                    if( !SysClearMsg( SwitchPMID(pstSWInfo->emSelPMID), OPC_RESP ))
                    {
                        //DEBUG_OPC( "PMHandle: SysClearMsg Failed, SelPMID-%d\n", emSelPMID );
                    }
                }
                else
                {
                    DEBUG_OPC( "PMHandle: SysSetMsg Failed, SelPMID-%d\n", emSelPMID );
                    LogWrite( (uint32_t)(LOGID_OPC_E_SYS_SET_MSG));
       ...
       (pstResp != NULL))
    {
        ...
        switch( usAType )
        {
            ...
            default:
            {
                ...
                LogWrite( (uint32_t)(LOGID_OPC_E_CMD_ACTUAL_TYPE));
            }
            break;
        }/*switch*/
    }
       ...
       (pstResp != NULL))
    {
        ...
        if((uint16_t)(OPC_LOGIN) == usAType )
        {
            if( bCMHasCfg )
            {
                ...
                else
                {
                    ...
                    else if( ucPSDLen == pucReq[usDataIndex])  /* Length */
                    {
                        ...
                        for( ucIndex = 0U; ucIndex < ucPSDLen; ucIndex++ )
                        {
                            if( s_ucPassword[ucIndex] != pucReq[(uint8_t)(PSD_INDEX)+ucIndex])
                            {
                                bPSDValid = false;
                                DEBUG_OPC( "PWD Err\n" );
                                //LogWrite( (uint32_t)(LOGID_OPC_E_CMD_PASSWORD_CONTENT));
                                break;
                            }
                        }
                    }
                    else
                    {
                        ...
                        //LogWrite( (uint32_t)(LOGID_OPC_E_CMD_PASSWORD_LEN));
                    }
            ...
            else
            {
                ...
                //LogWrite( (uint32_t)(LOGID_OPC_E_CM_NOCFG));
                ucSubReason = (uint8_t)(ERR_CMNOCFG);
                pstSWInfo->emLog = LOGOUT;
            }
                ...
                if( bCMHasCfg )
                {
                       ...
                       ((uint8_t)(AL_ALL) == pstSWInfo->ucAccAuth))
                    {
                        ...
                        else
                        {
                            ...
                            //LogWrite( (uint32_t)(LOGID_OPC_E_CFG_VAR_TABLE_COPYING));
                    ...
                    else
                    {
                        ...
                        //LogWrite( (uint32_t)(LOGID_OPC_E_ACCESS));
                        ucSubReason = (uint8_t)(ERR_ACCESS);  /* No access authority */
                    }
                }
                else
       ...
       (s_ucCurIndex < (uint8_t)(APP_SW_OPC_NUM)))
    {
        ...
        if( PM_RESP == s_emResp )
        {
            ...
            if( 0U == s_stRespMsg.usLen )
            {
                ...
                if( s_uiWaitPMCycleCnt >= (uint32_t)(OPC_PM_TIMEOUT_CYCLE_CNT))
                {
                    ...
                    LogWrite( (uint32_t)(LOGID_OPC_E_RESP_PM_TIMEOUT));
                    s_uiWaitPMCycleCnt = 0U;

                    /* Clear Req */
                    if( !SysClearMsg( SwitchPMID(pstSWInfo->emSelPMID), OPC_REQ ))
                    {
                        //DEBUG_OPC( "PMHandle: SysClearMsg Failed, SelPMID-%d\n", emSelPMID );
                    }
        ...
        else
        {
            ...
            if( ucResult != (uint8_t)(SEND_SUCCESS))
            {
                ...
                LogWrite( (uint32_t)(LOGID_OPC_E_RESP_EXCOMM_SEND));
            }
