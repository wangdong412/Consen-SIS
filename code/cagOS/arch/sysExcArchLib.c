...
void SpeException(void)
{
    ...
    if ((spefscr & _PPC_SPEFSCR_FINV) != 0)
        printf("Set when an input value on the high side is a NaN, Inf, or Denorm. Also set on a divide if both the dividend and divisor are zero.\n");
...
void generalException(UINT32 sp,UINT32 dear,UINT32 esr,UINT32 type)
{
    ...
    case _EXC_OFF_DATA_MISS:
        printf("B you meet the data miss exception\n");
        executeException(EXCEPTION_DMISS);
        break;
    case _EXC_OFF_INST_MISS:
        executeException(EXCEPTION_IMISS);
