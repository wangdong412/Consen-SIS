...
static char *put_dec(char *buf, UINT64 num)
{
	while (1) {
		unsigned rem;
		if (num < 100000)
...
static char *number2(char *buf, char *end, UINT64 num,
		...
		int field_width, int precision, int flags)
{
	...
	return number2(buf, end, (unsigned long)ptr, 16, field_width,
		      precision, flags);
}

int vsnprintf_internal(char *buf, size_t size, const char *fmt,
			      va_list args)
{
	UINT64 num;
	int base;
...
repeat:
		...
		str = number2(str, end, num, base, field_width, precision,
			     flags);
	}
