...
#define lld_log(log)   //record_log(log)//printf("%d\n",log) 
...
#define LLD_LOCK    //unsigned int lldlock;   \
                    int x1 = __LINE__;\
                    lldlock = lld_lock(x1); \
                    if (lldlock == 0xffffffff) return DEV_LOCK_FAIL;


#define LLD_UNLOCK // x1 = __LINE__;\
                    unsigned int lldunlock = lld_unlock(lldlock,x1); \
                    if (lldunlock == 0xffffffff) return DEV_UNLOCK_FAIL;
...
int lld_erase(int no, int num)
{
    DEVSTATUS ret;
    int i;
    if (no <0 || num <0 || no>SECTOR_NUM|| (no+num)>SECTOR_NUM) {
        lld_log(NOR_PARA_ERR1);
        return -1;
    }

    if(num < SECTOR_NUM) {
        for( i=0; i<num; i++ ) {
            ret = lld_SectorEraseOp((FLASHDATA *)FLASH_BASE_ADDR, (no+i)*LLD_SECTOR_SIZE);
            if (ret != DEV_NOT_BUSY) {
                lld_log(NOR_ERASEFAIL_ERR1);
                return -2;
            }
        }
    } else {
