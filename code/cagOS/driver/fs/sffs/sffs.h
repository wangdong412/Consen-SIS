...
//file name len
#define FF_HDR_NAMELEN          (FF_HDR_NAME+FNAME_LEN+1)
//file size
...
static __inline__ INT32 sffs_read_at(struct sffs *fs, UINT32 offset, void *data,UINT32 size)
{
    ...
    if(fs->read != NULL) {
        ...
        LOG_DEBUG("sffs_read_at 0x%x size %d name %s\n",offset,size,fs->fsname);
        LOG_DEBUG("%x %x\n",*(UINT32 *)data,*((UINT32 *)data+1));
    }
    return ret;
}
