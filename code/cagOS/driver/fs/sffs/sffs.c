...
//INT32 sffs_erase_time[MAX_SLOT][SE_NUM]= {{0}}; /*record relative sector erase time*/
#else
...
static inline INT32 sffs_write_at(struct sffs *fs, UINT32 offset, const void *data, UINT32 size)
{
    ...
    if(fs->write != NULL) {
        ...
        INT32 ret2;
        UINT8 buf[4096]= {0xaa};
        ret2 = fs->read(fs->flashId, offset, buf, size);
        if(memcmp((UINT8*)data,buf,size)) {
            printf("ret %d %d\n",ret,ret2);
            for(int i=0; i<size; i++) {
                if(buf[i] != *((UINT8 *)data+i))
                    printf("%x - %d : %x : %x\n",offset,i,buf[i],*((UINT8 *)data+i));
            }
        }
#endif
    }
...
static inline INT32 sffs_erase_at(struct sffs *fs, UINT32 no_from, UINT32 num)
{
    INT32 ret = FS_FAIL;
    int i;
    if(fs->erase != NULL && no_from <SS_NUM*SE_NUM*2 ) {
        ret = fs->erase(fs->flashId, no_from,num);
        if (ret >= 0) {
            for(i=0; i<num; i++) { /*record sector erase times*/
#ifdef SFFS_MULTI_DEVICE
//                    sffs_erase_time[fs->fsid][(no_from+i)/SS_NUM]++;
#else
                ...
                INT32 ret2;
                UINT8 buf[4096]= {0xaa};
                ret2 = fs->read(fs->flashId, (no_from+i)*4096, buf, 4096);
...
static INT32 sffs_get_se(struct sffs *fs)
{
    ...
    sffs_set_errno(fs,SFFS_NO_FREE_SE1);
    return FS_FAIL;
}
...
//            if (fs->free_ss_hdr[i] != FS_HDRS_USED && fs->free_ss_hdr[i] != FS_HDRS_BAD && fs->free_ss_hdr[i] != FS_HDRS_TMP) {
            if (fs->free_ss_hdr[i] == FS_HDRS_EMPTY || fs->free_ss_hdr[i] == FS_HDRS_EMPTY || fs->free_ss_hdr[i] == FS_S_UNUSED ) {
...
TRY_TWICE:
        if (sffs_erase_at(fs, SFFS_SB_START, ERASE_NUM_PER)<0) {
            if (sffs_erase_at(fs, SFFS_SB_START, ERASE_NUM_PER)<0) {
                sffs_set_errno(fs,SFFS_ERASE_HDRSS_ERR1);
                return FS_FAIL-1;
            }
        }
            ...
            if (fs->file[i].deled == FS_EMPTY) { /*file is not deleted */
                if (fs->file[i].comt == FS_COMMITED) { /* file is committed */
                    ...
                    if ( sffs_write_at(fs,ofs, &(fs->file[i]),
                        ...
                        sffs_set_errno(fs,SFFS_WRITE_HDRSS_ERR1);
                        fs->fileNum -- ;
...
INT32 sffs_find_filehdr(struct sffs *fs,const INT8 *fname)
{
    ...
    sffs_set_errno(fs,SFFS_FILENAME_NOT_EXIST_ERR1);
    return FS_FAIL;
}
...
static INT32 sffs_write_4k(struct sffs *fs, INT32 fd, const void *data, INT32 size)
{
    ...
    if(file->size % fs->se_size == 0) {
        ...
        if(se_no < 1) {
            ...
            sffs_set_errno(fs,SFFS_NO_FREE_SE2);
            return FS_FAIL;
        }
        ...
        if (fs->pageErase != SFFS_PAGE_ERASE) {
            ...
            if (ret <0) {
                ...
                if (ret <0) {
                    ...
                    sffs_set_errno(fs,SFFS_ERASE_DATA_ERR1);
                    return FS_FAIL;
                }
            }
        }
        ...
        /*except the last of file , every write to flash using sffs should be aligin with ss_size*/
...
static INT32 sffs_read_4k(struct sffs *fs, INT32 fd, void *data, INT32 size)
{
    ...
    if (ret < size) {
        file->ofs += ret;
        sffs_set_errno(fs,SFFS_READ_DATA_ERR1);
        return ret;
    }
    file->ofs += size;
    return ret;
...
static INT32 sffs_start(struct sffs *fs)
{
    ...
    //memset(fs->free_ss_hdr,FS_HDRS_TMP,sizeof(fs->free_ss_hdr)); //8.11 e
    memset(fs->free_ss_hdr,FS_HDRS_EMPTY,sizeof(fs->free_ss_hdr));

#ifdef SFFS_MULTI_DEVICE
    memset(&hdrsStat[fs->fsid],0,FS_SS_NUM*sizeof(INT32));
#else
...
INT32 sffs_open(struct sffs *fs,const INT8 *fname,INT32 mode)
{
    ...
    if (fname == NULL) {
        sffs_set_errno(fs,SFFS_PARA_ERR1);
        return FS_FAIL;
    }
    ...
    if (pos < 0) {
        ...
        LOG_DEBUG("file %s hdr %d pos %d\n",file->name,file->hdr_ss,pos);
...
INT32 sffs_close(struct sffs *fs, INT32 fd)
{
    ...
    if (sffs_ckstat(fs,fd) == FS_WRONLY) {
            ...
            if (fsStat == 1) {
                ...
                if (ret <0) {
                    ...
                    if (ret <0) {
                        ...
                        sffs_set_errno(fs,SFFS_ERASE_HDRSS_ERR2);
                        return FS_FAIL;
                    }
                }
#ifdef SFFS_MULTI_DEVICE
        ...
        //8.11 e
        if(fs->pageErase == SFFS_PAGE_ERASE)
            sffs_erase_at(fs,fs->file[fd].hdr_ss,1);
        ...
        if ( sffs_write_at(fs, ofs, &(fs->file[fd]), (UINT32 )(&(fs->file[fd].deled))-(UINT32)(&(fs->file[fd])))
            ...
            if (fs->pageErase == SFFS_PAGE_ERASE) {
                ...
                if ( sffs_write_at(fs, ofs, &(fs->file[fd]),(UINT32 )(&(fs->file[fd].deled))-(UINT32)(&(fs->file[fd])))
                    ...
                    fs->file[fd].seq    = 0;
                    fs->fileNum --;
                    sffs_set_errno(fs,SFFS_WRITE_HDRSS_ERR2);
                    return FS_FAIL;
                }
            } else {
                LOG_WARN("np close file %d fail name %s\n",fd,fs->file[fd].name);
                ...
                fs->file[fd].seq = 0;
                fs->fileNum --;
                sffs_set_errno(fs,SFFS_WRITE_HDRSS_ERR3);
                return FS_FAIL;
            }
        }
    }
...
INT32 sffs_write(struct sffs *fs, INT32 fd, const void *data, INT32 size)
{
    ...
    if (sffs_ck_fd(fs,fd) < 0 || size <0 || size > SFFS_MAX_FILES_SIZE) {
        sffs_set_errno(fs,SFFS_PARA_ERR2);
        return FS_FAIL;
    }
    ...
    if (file->stat != FS_WRONLY) {
        ...
        sffs_set_errno(fs,SFFS_WRITEMODE_ERR1);
        return FS_FAIL;
    }
    if (size == 0)
        return 0;
    ...
    if(file->ofs + size > SFFS_MAX_FILES_SIZE) {
        sffs_set_errno(fs,SFFS_FILE_SIZE_EXCED_ERR1);
        LOG_DEBUG("write, exceed %x\n",file->ofs + size);
        return FS_FAIL;
    }
    ...
    if (size1 != fs->ss_size) {
        ...
        if (sffs_write_4k(fs,fd,temp,size1) < (size1)) {
            /*if no space left tell caller*/
            if ( fs->errno != SFFS_NO_FREE_SE3) {
                if (fs->pageErase == SFFS_PAGE_ERASE) {
                    sffs_erase_at(fs,fs->file[fd].ofs/fs->ss_size,ERASE_NUM_PER);
                    if (sffs_write_4k(fs,fd,temp,size1) < size1) {
                        /*bad block*/
                        sffs_set_errno(fs,SFFS_WRITE_DATA_ERR1);
    ...
    if(size > fs->ss_size) {
        ...
        while(size >= fs->ss_size) {
            if (sffs_write_4k(fs,fd,temp,size1)<size1) {
                if ( fs->errno == SFFS_NO_FREE_SE4) {
                    return file->ofs-ofsb;
                }
                if (fs->pageErase == SFFS_PAGE_ERASE) {
                    sffs_erase_at(fs,fs->file[fd].ofs/fs->ss_size,ERASE_NUM_PER);
                    if (sffs_write_4k(fs,fd,temp,size1)<size1) {
                        if ( fs->errno == SFFS_NO_FREE_SE5) {
                            return file->ofs-ofsb;
                        }
                        sffs_set_errno(fs,SFFS_WRITE_DATA_ERR3);
                        return file->ofs-ofsb;
                    }
                } else {
                    sffs_set_errno(fs,SFFS_WRITE_DATA_ERR4);
                    return file->ofs-ofsb;
                }
        ...
        if (sffs_write_4k(fs,fd,temp,size) < size) {
            ...
            if (fs->pageErase == SFFS_PAGE_ERASE) {
                ...
                if (sffs_write_4k(fs,fd,temp,size) < size) {
                    ...
                    if ( fs->errno == SFFS_NO_FREE_SE7) {
                        return file->ofs-ofsb;
                    }

                    sffs_set_errno(fs,SFFS_WRITE_DATA_ERR5);
                    return file->ofs-ofsb;
                }
            } else {
                sffs_set_errno(fs,SFFS_WRITE_DATA_ERR2);
                return file->ofs-ofsb;
            }
        }
...
INT32 sffs_read(struct sffs *fs, INT32 fd, void *data, INT32 size)
{
    ...
    /*check fd is legal*/
    if (sffs_ck_fd(fs,fd) < 0 || size < 0 || size >SFFS_MAX_FILES_SIZE) {
        sffs_set_errno(fs,SFFS_PARA_ERR3);
        return FS_FAIL;
    }
    ...
    if(file->ofs + size > file->size) {
        ...
        sffs_set_errno(fs,SFFS_FILE_SIZE_EXCED_ERR2);
    }
    ...
    if (file->ofs%fs->ss_size !=0) {
        ...
        if (ret < size1) {
            sffs_set_errno(fs,SFFS_READ_DATA_ERR2);
            return ret;
        }
    ...
    if (size >= fs->ss_size) {
        while(size >= fs->ss_size) {
            if (sffs_read_4k(fs,fd,temp,fs->ss_size) < fs->ss_size) {
                sffs_set_errno(fs,SFFS_READ_DATA_ERR3);
                return file->ofs-ofsb;
            }
    ...
    if(size > 0) {
        if (sffs_read_4k(fs,fd,temp,size) < size) {
            sffs_set_errno(fs,SFFS_READ_DATA_ERR4);
            return file->ofs-ofsb;
        }
        size = 0;
    }
...
INT32 sffs_rename(struct sffs *fs,const INT8 *old_name,const INT8 *new_name)
{
    ...
    if(old_name == NULL || new_name == NULL) {
        sffs_set_errno(fs,SFFS_PARA_ERR4);
        return FS_FAIL;
    ...
    if (pos1 <0) {
        sffs_set_errno(fs,SFFS_FILENAME_NOT_EXIST_ERR2);
        return FS_FAIL;
    }
    ...
    if (file ->comt != FS_COMMITED) {
        ...
        /*new hdr_ss offset*/
        ofs_new         = SFFS_SB_START*fs->se_size + file->hdr_ss*fs->ss_size;
        ...
        if (fs->pageErase == SFFS_PAGE_ERASE) {
            //re-erase
            if (sffs_erase_at(fs,file->hdr_ss,1) != FS_OK) {
                if (sffs_erase_at(fs,file->hdr_ss,1) != FS_OK) {
                    goto fail2;
                }
            }
...
ok2:
        ...
        if ( sffs_write_at(fs,ofs_old,&data, 1) != 1 ) {
            ...
            if(fs->pageErase != SFFS_PAGE_ERASE) {
                ...
                if (sffs_erase_at(fs,old_hs,ERASE_NUM_PER)<0) {
                    sffs_set_errno(fs,SFFS_RENAME_FAIL2);
                    fs->free_ss_hdr[old_hs]=FS_HDRS_BAD;
                } else {
                    goto ok1;
...
ok1:
        ...
        if(fs->pageErase != SFFS_PAGE_ERASE)
            ...
            fs->free_ss_hdr[old_hs]=FS_HDRS_EMPTY; //8.12 e
    }
    return FS_OK;
...
INT32 sffs_lseek(struct sffs *fs, INT32 fd, INT32 pos)
{
    if (sffs_ck_fd(fs,fd) < 0 || pos < 0) {
        sffs_set_errno(fs,SFFS_PARA_ERR5);
        return FS_FAIL;
    }
    /*cannot seek in writing mode*/
    if (sffs_ckstat(fs,fd) == FS_WRONLY) {
        sffs_set_errno(fs,SFFS_WRITEMODE_ERR2);
        return FS_FAIL;
    }
...
INT32 sffs_remove_byfd(struct sffs *fs, INT32 fd)
{
    ...
    if (ret != size) {
        if (fs->pageErase == SFFS_PAGE_ERASE) {
            ...
            if (sffs_erase_at(fs,fs->file[fd].hdr_ss,ERASE_NUM_PER)<0) {
                ...
                sffs_reset_filehdr(fs, fd);
                fs->fileNum -- ;
                fs->free_ss_hdr[fs->file[fd].hdr_ss]=FS_HDRS_EMPTY;//FS_DELETED;/*FS_S_UNUSED;*/ 8.12 e
                return FS_OK;
            }
    ...
    if (fs->pageErase != SFFS_PAGE_ERASE) { //8.12 e
#ifdef SFFS_MULTI_DEVICE
        hdrsStat[fs->fsid][fs->file[fd].hdr_ss]=FS_HDRS_TMP;
#else
        hdrsStat[fs->file[fd].hdr_ss]=FS_HDRS_TMP;
#endif
        fs->free_ss_hdr[fs->file[fd].hdr_ss]=FS_HDRS_EMPTY;
    } else {
        fs->free_ss_hdr[fs->file[fd].hdr_ss]=FS_HDRS_EMPTY; //8.12 e
    }
    sffs_reset_filehdr(fs, fd);
    fs->fileNum -- ;
    return FS_OK;
...
INT32 sffs_remove_byname(struct sffs *fs, const INT8 *fname)
{
    ...
    if (pos < 0) {
        LOG_INFO("no file %s\n",fname);
        sffs_set_errno(fs,SFFS_FILENAME_NOT_EXIST_ERR3);
        return FS_FAIL;
    } else {
...
INT32 sffs_format(struct sffs *fs, INT32 type)
{
    ...
    case FS_FORMAT_HDRS:
        /*only erase hdrs*/
        ret = sffs_erase_at(fs,SFFS_SB_START,FORMAT_HDRSS);

        if(ret <0) {
            ret = sffs_erase_at(fs,SFFS_SB_START,FORMAT_HDRSS);
            if(ret <0) {
                sffs_set_errno(fs, SFFS_ERASE_HDRSS_ERR3);
        ...
        sffs_start(fs);
        fs->free_ss_hdr_cur = 0; //ezio 8.23
        break;
    case FS_FORMAT_ALL:
        /*erase whole flash*/
        for(INT32 i=0; i<fs->device_size/fs->erase_unit; i++) {
            ...
            if(ret <0) {
                ret = sffs_erase_at(fs,i,ERASE_NUM_PER);
                if(ret <0) {
                    sffs_set_errno(fs, SFFS_ERASE_DATA_ERR2);
...
INT32 sffs_mount(struct sffs *fs, INT32 type)
{
    ...
    if (type == 1) {
        ...
        LOG_INFO("fs mount to erase all %s\n",fs->fsname);
        return FS_OK;
    }
    ...
    while(offset < fs->se_size+SFFS_SB_START*fs->se_size &&
        ...
        if (header[FF_HDR_DELED] != FS_EMPTY || /*deleted*/
            header[FF_HDR_COMMIT] != FS_COMMITED || /*not committed*/
            *(UINT32 *)(header) != SFFS_MAGIC) { /*magic error*/
        ...
        if (ret != FS_FAIL) {
            ...
            if (sffs_write_at(fs,ofs0,&data,1) != 1) {
                if (fs->pageErase == SFFS_PAGE_ERASE) {
                    if (sffs_write_at(fs,ofs0,&data,1) != 1) {
                        sffs_set_errno(fs,SFFS_WRITE_HDRSS_ERR4);
                        FS_UNLOCK(fs);
                        return FS_FAIL;
                    }
                } else {
                    sffs_set_errno(fs,SFFS_WRITE_HDRSS_ERR5);
                    FS_UNLOCK(fs);
                    return FS_FAIL;
                }
            }
    ...
    if (fs->fileNum<1) {
        LOG_INFO("empty flash \n");
        fs->free_ss_hdr_cur = 0; /*empty so from 0*/
        if(fs->pageErase != SFFS_PAGE_ERASE)
#ifdef SFFS_MULTI_DEVICE
