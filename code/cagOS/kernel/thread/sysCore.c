 ...
 *
 * DESCRIPTION
 * This file contains initialize the internals
*/
...
/* init sys structure for rts*/
extern void rts_var_init(void);
UINT32 synchronization = 0x0;
UINT32 fpga_tick = 333;
...
void  OSIntExit (void)
{
    ...
    /*
        if ((synchronization == 0x0) && (OSPrioHighRdy == 0x8))
        {
           do {
           fpga_tick = get_fpga();
           } while ( fpga_tick % 200 > 0x2);
           sysUsDelay(10);
           synchronization = 0x1;
        }
    */
...
#if OS_CFG_SCHED_STRENGTHEN_EN > 0u
    if (OSPrioHighRdy == 0x1)
        OSTCBHighRdyPtr->Tconsuming = get_fpga();
#endif
...
#if OS_CFG_SCHED_STRENGTHEN_EN > 0u
    if ((synchronization == 0x1) && (OSPrioHighRdy == 0x1)) {
        OSTCBHighRdyPtr->Tconsuming = get_fpga();
        synchronization = 0x2;
    }
#endif
...
void  OS_IdleTaskInit (OS_ERR  *p_err)
{
    ...
    OSTaskCreate((OS_TCB     *)&OSIdleTaskTCB,
                 ...
                 (CPU_STK *)TASK_IDLE_STACK_HIGH,
                 (CPU_STK_SIZE)OSCfg_IdleTaskStkLimit,
                 (CPU_STK_SIZE)OSCfg_IdleTaskStkSize,
                 (OS_MSG_QTY  )0u,
                 (OS_TICK     )0u,
                 (UINT8 *)(TASK_SPESTACK_HIGH-5*(TASK_SPESTACK_SIZE+GAP_SIZE)),
                     ...
                     CPU_TS        ts)
{
    ...
    case OS_TASK_STATE_PEND_TIMEOUT:
        if (p_tcb->PendOn == OS_TASK_PEND_ON_MULTI) {
            OS_PendObjDel1(p_obj,p_tcb,ts);                           /* Indicate which object was pend aborted            */
        }
...
void  OS_SchedRoundRobin (OS_RDY_LIST  *p_rdy_list)
{
    ...
    /* if task running here, it's TimeQuantaCtr == 0 */

#if OS_CFG_SCHED_STRENGTHEN_EN > 0u
    if (OSPrioCur == 0x1) {
        UINT32 balance = get_fpga() - OSTCBCurPtr->Tconsuming;
        if (((balance * 100) < (500 * OSTCBCurPtr->TimeQuanta)) && \
            ((500 * OSTCBCurPtr->TimeQuanta - balance * 100) > 200)) {
            p_tcb->TimeQuantaCtr++;
            //printf("$$$$%d %d\n",balance, OSTCBCurPtr->TimeQuanta);
            return;
        }
    }
#endif
