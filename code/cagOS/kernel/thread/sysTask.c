...
extern CPU_INT32U fpga_tick;
extern CPU_INT32U synchronization;
extern CPU_INT32U get_fpga(void);
...
void  OS_TaskReturn (void)
{
    ...
    printf("ready schedule to RTS task\n");

    if ((synchronization == 0x0) && (OSPrioHighRdy == 0x0)) {
        do {
            fpga_tick = get_fpga();
        } while ( fpga_tick % 200 > 0x1);
        synchronization = 0x1;
    }
    sysUsDelay(50);
