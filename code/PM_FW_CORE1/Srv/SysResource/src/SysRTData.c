...
bool_t SysReadRTData(SysRtDataAreaType_t emDataArea, puint8_t pucContent, uint16_t usLen, puint16_t pusRdLen, CMController_t emCMID)
{
    ...
    if((emDataArea < MAX_RTDATA_AREA_NUM) && (pucContent != NULL) && (usLen > 0) && (pusRdLen!=NULL))
    {
        ...
        {
            if(s_bRtDataStart == true)/* 可以开始传输实时数据 */
            {
                if(emCMID==CM1) /* 开始传输 */
                {
        ...
        {
            /* 获取读偏移地址 */
            uiReadOff = s_stRTDataArea[emIndex].uiReadOffset[emDataArea][emCMID];
            ...
            if(pucSrc != NULL)
            {
                ...
                if(uiReadOff + usLen < uiBaseOffset + s_stRTDataArea[emIndex].uiLen[emDataArea])
                {
                    ...
                    s_stRTDataArea[emIndex].uiReadOffset[emDataArea][emCMID] += usLen;
                    s_stRTDataArea[emIndex].bReadFlag[emDataArea][emCMID] = RTDATA_AREA_READ_NOT_FINISHED;
                }
                else
                {
                    usLen = uiBaseOffset + s_stRTDataArea[emIndex].uiLen[emDataArea] - uiReadOff;
                    memcpy(pucContent, pucSrc, usLen);
                    s_stRTDataArea[emIndex].bReadFlag[emDataArea][emCMID] = RTDATA_AREA_READ_FINISHED;
                }
                *pusRdLen = usLen;
...
bool_t SysReadRTDataReset(void)
{
    ...
    {
        /* 复位读偏移地址 */
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP1_G_AREA][0] = s_uiG1Offset;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP2_G_AREA][0] = s_uiG2Offset;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_I_AREA][0] = 0;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_Q_AREA][0] = 0;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_APPEND_AREA][0] = 0;

        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP1_G_AREA][1] = s_uiG1Offset;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP2_G_AREA][1] = s_uiG2Offset;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_I_AREA][1] = 0;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_Q_AREA][1] = 0;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_APPEND_AREA][1] = 0;

        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP1_G_AREA][2] = s_uiG1Offset;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP2_G_AREA][2] = s_uiG2Offset;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_I_AREA][2] = 0;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_Q_AREA][2] = 0;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_APPEND_AREA][2] = 0;

        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP1_G_AREA][3] = s_uiG1Offset;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP2_G_AREA][3] = s_uiG2Offset;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_I_AREA][3] = 0;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_Q_AREA][3] = 0;
        s_stRTDataArea[emIndex].uiReadOffset[RTDATA_APPEND_AREA][3] = 0;
        bRet = true;
    }
...
bool_t SysGetRTDataReadOffset(SysRtDataAreaType_t emDataArea, puint32_t puiOffset,CMController_t emCMID)
{
    ...
    if((emDataArea < MAX_RTDATA_AREA_NUM) && (puiOffset != NULL))
    {
        ...
        {
            /* 获取读偏移地址 */
            uiReadOff = s_stRTDataArea[RTDATA_AREA_A].uiReadOffset[emDataArea][emCMID];
            *puiOffset = uiReadOff;
            /*读取成功*/
            bRet = true;
        }
...
bool_t SysGetRTDataAreaLen(SysRtDataAreaType_t emDataArea, puint32_t puiLen,CMController_t emCMID)
{
    ...
    if((emDataArea < MAX_RTDATA_AREA_NUM) && (puiLen != NULL))
    {
        /* 未加锁且为只读区域 */
        if((RTDATA_AREA_UNLOCKED == s_stRTDataArea[RTDATA_AREA_A].bLock) \
            &&(RTDATA_AREA_READ_ONLY == s_stRTDataArea[RTDATA_AREA_A].bDataAreaStatus))
        {
            /* 获取读偏移地址 */
            uiLen = s_stRTDataArea[RTDATA_AREA_A].uiLen[emDataArea];
            *puiLen = uiLen;
            /*读取成功*/
            bRet = true;
        }
...
bool_t SysGetRTDataReadFlag(SysRtDataAreaType_t emDataArea, bool_t *pbReadFlag,CMController_t emCMID)
{
    ...
    if((emDataArea < MAX_RTDATA_AREA_NUM) && (pbReadFlag != NULL))
    {
        ...
        {
            /* 获取读偏移地址 */
            bFlag = s_stRTDataArea[RTDATA_AREA_A].bReadFlag[emDataArea][emCMID];
            *pbReadFlag = bFlag;
#if 0
    ...
    if(emIndex != INVALID_RTDATA_AREA_INDEX)
    {
        switch(emDataArea)
        {
            case RTDATA_UP1_G_AREA:
            {
                ...
                s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP1_G_AREA][0] = uiOffset;
                s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP1_G_AREA][1] = uiOffset;
                s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP1_G_AREA][2] = uiOffset;
                s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP1_G_AREA][3] = uiOffset;
                s_stRTDataArea[emIndex].uiLen[RTDATA_UP1_G_AREA] = uiAreaUsedLen;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_UP1_G_AREA][0] = RTDATA_AREA_READ_NOT_FINISHED;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_UP1_G_AREA][1] = RTDATA_AREA_READ_NOT_FINISHED;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_UP1_G_AREA][2] = RTDATA_AREA_READ_NOT_FINISHED;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_UP1_G_AREA][3] = RTDATA_AREA_READ_NOT_FINISHED;
                //printf("up1 Gsize %d\n", uiSize);
            }
            break;
            ...
            case RTDATA_UP2_G_AREA:
            {
                ...
                s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP2_G_AREA][0] = uiOffset;
                s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP2_G_AREA][1] = uiOffset;
                s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP2_G_AREA][2] = uiOffset;
                s_stRTDataArea[emIndex].uiReadOffset[RTDATA_UP2_G_AREA][3] = uiOffset;
                s_stRTDataArea[emIndex].uiLen[RTDATA_UP2_G_AREA] = uiAreaUsedLen;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_UP2_G_AREA][0] = RTDATA_AREA_READ_NOT_FINISHED;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_UP2_G_AREA][1] = RTDATA_AREA_READ_NOT_FINISHED;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_UP2_G_AREA][2] = RTDATA_AREA_READ_NOT_FINISHED;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_UP2_G_AREA][3] = RTDATA_AREA_READ_NOT_FINISHED;
            }
            break;
            ...
            {
                /* 拷贝 I区数据 */
                pucSrc = (puint8_t) &(g_stSharedRtData.ucI[uiOffset]);
                pucDst = (puint8_t) &(s_stRTDataArea[emIndex].ucInputContent[uiOffset]);
                memcpy(pucDst, pucSrc, uiSize);
                s_stRTDataArea[emIndex].uiReadOffset[RTDATA_I_AREA][0] = 0;
                s_stRTDataArea[emIndex].uiReadOffset[RTDATA_I_AREA][1] = 0;
                s_stRTDataArea[emIndex].uiReadOffset[RTDATA_I_AREA][2] = 0;
                s_stRTDataArea[emIndex].uiReadOffset[RTDATA_I_AREA][3] = 0;
                s_stRTDataArea[emIndex].uiLen[RTDATA_I_AREA] = uiAreaUsedLen;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_I_AREA][0] = RTDATA_AREA_READ_NOT_FINISHED;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_I_AREA][1] = RTDATA_AREA_READ_NOT_FINISHED;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_I_AREA][2] = RTDATA_AREA_READ_NOT_FINISHED;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_I_AREA][3] = RTDATA_AREA_READ_NOT_FINISHED;
            }
            break;
            ...
            {
                /* 拷贝 Q区数据 */
                pucSrc = (puint8_t) &(g_stSharedRtData.ucQ[uiOffset]);
                pucDst = (puint8_t) &(s_stRTDataArea[emIndex].ucOutputContent[uiOffset]);
                memcpy(pucDst, pucSrc, uiSize);
                ...
                s_stRTDataArea[emIndex].uiReadOffset[RTDATA_Q_AREA][2] = 0;
                s_stRTDataArea[emIndex].uiReadOffset[RTDATA_Q_AREA][3] = 0;
                s_stRTDataArea[emIndex].uiLen[RTDATA_Q_AREA] = uiAreaUsedLen;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_Q_AREA][0] = RTDATA_AREA_READ_NOT_FINISHED;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_Q_AREA][1] = RTDATA_AREA_READ_NOT_FINISHED;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_Q_AREA][2] = RTDATA_AREA_READ_NOT_FINISHED;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_Q_AREA][3] = RTDATA_AREA_READ_NOT_FINISHED;
            }
            break;
...
//                SysGetCMVer(CM4, (puint32_t)&s_stAppendInfo.stCMVern[CM4].stCMFirmwareVern.ucRTSVern);
                ...
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_APPEND_AREA][0] = RTDATA_AREA_READ_NOT_FINISHED;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_APPEND_AREA][1] = RTDATA_AREA_READ_NOT_FINISHED;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_APPEND_AREA][2] = RTDATA_AREA_READ_NOT_FINISHED;
                s_stRTDataArea[emIndex].bReadFlag[RTDATA_APPEND_AREA][3] = RTDATA_AREA_READ_NOT_FINISHED;
            }
            break;
