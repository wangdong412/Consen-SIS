...
***********************************************************************************************************************
*/
static void ConfigInfoInit(void);
...
static void ConfigInitDownLoadInit(void);

static void ConfigPowerOnInit(void);

static void ConfigIncreDownLoadInit(void);

static void ConfigDataAreaAllInit(void);

static void ConfigDataAreaIncreInit(void);

static void ConfigDisableAreaInit(void);

static void ConfigStopInit(void);
...
//                    DEBUG_CONFIG("  pucFileDataAddr is : %x  \n",pucFileDataAddr);
                    ...
                    switch(emCfgFileType)
                    {
                        case CFG_CM:
                        {
                            ...
                            if(RET_OK != cRet)
                            {
                                LogWrite(LOG1_ID_APP_PROJECT_CONFIG_LEVEL0_CM_CFG_FAIL);
                            }
                        }
                        break;
                        ...
                        case CFG_MODBUS_MASTER:
                        {
                            ...
                            if(RET_OK != cRet)
                            {
                                LogWrite(LOG1_ID_APP_PROJECT_CONFIG_LEVEL0_MMC_CFG_FAIL);
                            }
                        }
                        break;
                        ...
                        case CFG_MODBUS_SLAVE:
                        {
                            ...
                            if(RET_OK != cRet)
                            {
                                LogWrite(LOG1_ID_APP_PROJECT_CONFIG_LEVEL0_MSC_CFG_FAIL);
                            }
...
//                            ConfigOPCFileParse(pucFileDataAddr);
                        ...
                        case CFG_CTRL_STATION:
                        {
                            ...
                            if(RET_OK != cRet)
                            {
                                LogWrite(LOG1_ID_APP_PROJECT_CONFIG_LEVEL0_SC_CFG_FAIL);
                            }
...
//                            SharedSetDownloadFlg(DOWN_LOAD_COMPLETE);
                        ...
                        case CFG_SOE:
                        {
                            cRet = ConfigSOEFileParse(pucFileDataAddr);
                            
                            if(RET_OK != cRet)
                            {
                                LogWrite(LOG1_ID_APP_PROJECT_CONFIG_LEVEL0_SOE_CFG_FAIL);
                            }
                        }
                        break;
                        ...
                        case CFG_P2P:
                        {
                            cRet = ConfigP2PFileParse(pucFileDataAddr);
                            
                            if(RET_OK != cRet)
                            {
                                LogWrite(LOG1_ID_APP_PROJECT_CONFIG_LEVEL0_P2P_CFG_FAIL);
                            }
                        }
                        break;
                        ...
                        case CFG_VOTE:
                        {
                            cRet = ConfigVoteFileParse(pucFileDataAddr);
                            
                            if(RET_OK != cRet)
                            {
                                LogWrite(LOG1_ID_APP_PROJECT_CONFIG_LEVEL0_VOT_CFG_FAIL);
                            }
                        }
                        break;
                        ...
                        case CFG_IO_MODULAR:
                        {
                            cRet = ConfigIOConfigFileParse(pucFileDataAddr);
                            
                            if(RET_OK != cRet)
                            {
                                LogWrite(LOG1_ID_APP_PROJECT_CONFIG_LEVEL0_IOC_CFG_FAIL);
                            }
                        }
                        break;
                        ...
                        case CFG_RETAIN:
                        {
                            cRet = ConfigRetainFileParse(pucFileDataAddr);
                            
                            if(RET_OK != cRet)
                            {
                                LogWrite(LOG1_ID_APP_PROJECT_CONFIG_LEVEL0_RTN_CFG_FAIL);
                            }
                        }
                        break;
                        ...
                        case CFG_PRG:
                        {
                            cRet = ConfigPRGFileParse(pucFileDataAddr);
                            
                            if(RET_OK != cRet)
                            {
                                LogWrite(LOG1_ID_APP_PROJECT_CONFIG_LEVEL0_PRG_CFG_FAIL);
                            }
                        }
                        break;
    ...
    for(emCfgFileType = CFG_CTRL_STATION; emCfgFileType < NUM_OF_CFG_FILE_TYPE; emCfgFileType++)
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    usWritDataLen = SysSetCfgFileInfo(emCfgFileType, &ucFileDataBuff[0], iRet);
                    
                    if(usWritDataLen != iRet)
                    {/*写入内存失败*/
                        /*log*/
                    ...
                    {
                        iFileDataLen = iFileDataLen - iRet;
                ...
                else
                {
                    ...
                    {/*每一段的解析都正确，文件解析正确*/
                        ...
                        if(true == ConfigCheckFileCrc(pstCtrlStationInfo))
                        {
                            ...
                            {
                                ConfigInitDownLoadInit();
                            }
                            else if(ONLINE_DOWNLOAD == SharedGetDownloadFlg())
                            {
                                ConfigIncreDownLoadInit();
                            }
                            else
                            {
                                ConfigPowerOnInit();
                            ...
                            if((pstDLPrjInfo != NULL) && (pstPrjInfo != NULL))
                            {
                                   ...
                                   (ONLINE_DOWNLOAD == SharedGetDownloadFlg()))
                                {
                                    ...
                                    {
                                        pstDLPrjInfo->ucIsConfirmed = HAS_BE_CONFIRMED;
                                    }
                                }
                                else
                                {
                                    ...
                                    pstPrjInfo->ucIsConfirmed = HAS_BE_CONFIRMED;
                                    
                                    /*当上电解析文件完成时，设置上电切换配置信息标志*/
                        ...
                        else
                        {
                            ...
                            LogWrite(LOG1_ID_APP_PROJECT_CONFIG_LEVEL0_CHECK_CRC_ERR);
                        }
...
static int8_t ConfigStationInfoSectionParse(uint8_t ucSectionAddr[],CtrlStationConfigInfo_t* pstCtrlStationInfo)
{
    ...
    else
    {
        ...
        memcpy((puint8_t)&pstCtrlStationInfo->stStationConfigInfo.ucCommTolerate, &ucSectionAddr[uiBuffIndex], \
                sizeof(uint8_t));
        uiBuffIndex += sizeof(uint8_t);
        memcpy((puint8_t)&pstCtrlStationInfo->stStationConfigInfo.ucVoteTolerate, &ucSectionAddr[uiBuffIndex], \
                sizeof(uint8_t));
        uiBuffIndex += sizeof(uint8_t);
        memcpy((puint8_t)&pstCtrlStationInfo->stStationConfigInfo.ucSlaveConfirm, &ucSectionAddr[uiBuffIndex], \
                sizeof(uint8_t));
        uiBuffIndex += sizeof(uint8_t);
//        printf("SlaveConfirm %x\n",pstCtrlStationInfo->stStationConfigInfo.ucSlaveConfirm);
...
//        printf("CommTol: %x\n",pstCtrlStationInfo->stStationConfigInfo.ucCommTolerate);
//        printf("VoteTol: %x\n",pstCtrlStationInfo->stStationConfigInfo.ucVoteTolerate);
//        printf("SlaveConfirm: %x\n",pstCtrlStationInfo->stStationConfigInfo.ucSlaveConfirm);
...
//                            DEBUG_CONFIG("  MODTYPE_OUTPUT: uiParseDataSize is : %d \n",uiParseDataSize);
            ...
            {/*每一段的解析都正确，文件解析正确*/
                ...
                pstIoModInfo->uiModNum = uiModNum;
                /*记录文件CRC及版本*/
...
static int8_t ConfigPMConfigSectionParse(uint8_t ucSectionAddr[])
{
    ...
    else
    {
        ...
        else
        {
            ...
            uiBuffIndex += 8*sizeof(uint8_t);
            memcpy(&pstIoModInfo->stSysPMInfo, &ucSectionAddr[uiBuffIndex], sizeof(SysPMInfo_t));
            
//            printf("PPMPolarity: %d\n",pstIoModInfo->stSysPMInfo.usPPMPolarity);
//            printf("PPMSource: %d\n",pstIoModInfo->stSysPMInfo.usPPMSource);
...
static uint32_t ConfigIOSlaveParse(uint8_t ucIOSlaveInfo[],uint8_t ucModType,SysIoModInfo_t* pstIoModInfo)
{
    ...
    else
    {
        ...
        if((pstIoModInfo->stSysIoParamArea.usIoParamIndex + usParamSize) < IO_HARD_PARAM_LEN)
        {
    ...
    //        DEBUG_CONFIG("  ucModNum is : %x \n",ucModNum);
            ...
            if(MODTYPE_INPUT == ucModType)
            {
                ...
                uiBuffIndex += sizeof(uint16_t);
                if(usChannelNum <= MAX_CHANNEL_NUM)
                {
...
static int8_t ConfigSOEInfoSectionParse(uint8_t ucSectionAddr[], FileHead_t* pstFileHead)
{
    ...
    uint32_t uiBaseAddr1 = 0U;                              /*非掉电保持的软SOE块在DDR中的起始地址偏移*/
    uint32_t uiBaseAddr2 = 0U;                              /*掉电保持的软SOE块在SRAM中的起始地址偏移*/
    ...
    else
    {
        ...
        if((NULL == pstSOEConfigInfo) || (NULL == pstAppend))
        {
            /*log*/
        }
        else
        {/*解析记录SOE配置信息*/
            ...
            else
            {
                ...
                {/*逐个块解析文件*/
                    memcpy((puint8_t)&stSOEBlockInfo, &ucSectionAddr[uiBuffIndex], sizeof(SOEBlockInfo_t));
                    uiBuffIndex += sizeof(SOEBlockInfo_t);
                    
                    ucBlockID = (uint8_t)stSOEBlockInfo.uiBlockID;
                    pstSOEConfigInfo->stSoftSOEBlock[ucBlockID].usUpdateFlag = stSOEBlockInfo.usUpdateFlag;
                    pstSOEConfigInfo->stSoftSOEBlock[ucBlockID].usBlockType = stSOEBlockInfo.usBlockType;
                    pstSOEConfigInfo->stSoftSOEBlock[ucBlockID].ucProperty = stSOEBlockInfo.ucProperty;
                    pstSOEConfigInfo->stSoftSOEBlock[ucBlockID].uiMaxRecordCount = stSOEBlockInfo.uiMaxRecordCount;
                    pstSOEConfigInfo->stSoftSOEBlock[ucBlockID].uiSoftSOEVarNum = stSOEBlockInfo.uiSOEVarNum;
                    pstSOEConfigInfo->stSoftSOEBlock[ucBlockID].uiSoftSOEVarIndex = uiSOEVarIndex;
                    
                    if(0 == pstSOEConfigInfo->stSoftSOEBlock[ucBlockID].ucProperty)
                    {
                        pstSOEConfigInfo->stSoftSOEBlock[ucBlockID].uiSW_BaseAddr = uiBaseAddr1;
                        uiBaseAddr1 += pstSOEConfigInfo->stSoftSOEBlock[ucBlockID].uiMaxRecordCount;
                        pstSOEConfigInfo->stSoftSOEBlock[ucBlockID].ucProperty = SOE_STORAGE_DDR;
                        if(uiBaseAddr1 > MAX_SOE_RECORD_NUM)
                        {
                            /*log*/
                            break;
                        }
                    }
                    else if(1 == pstSOEConfigInfo->stSoftSOEBlock[ucBlockID].ucProperty)
                    {
                        pstSOEConfigInfo->stSoftSOEBlock[ucBlockID].uiSW_BaseAddr = uiBaseAddr2;
                        uiBaseAddr2 += pstSOEConfigInfo->stSoftSOEBlock[ucBlockID].uiMaxRecordCount;
                        pstSOEConfigInfo->stSoftSOEBlock[ucBlockID].ucProperty = SOE_STORAGE_SRAM;
                        if(uiBaseAddr2 > MAX_RTN_SOE_RECORD_NUM)
                        {
                            /*log*/
                            break;
                        }
                    ...
                    if(2 == pstSOEConfigInfo->stSoftSOEBlock[ucBlockID].usUpdateFlag)
                    {
                        pstAppend->ucSOECtrl[ucBlockID] = SOE_STATE_START;
                    }
                    
                    if((stSOEBlockInfo.uiSOEVarNum > MAX_SOE_VAR_SINGLE_BLOCK) ||
                       ((stSOEBlockInfo.uiSOEVarNum + uiSOEVarIndex) > MAX_SOE_VAR_NUM))
                    {
                        DEBUG_CONFIG("SOE VAR too many\n");
                    }
                    else
                    {
                        /*软SOE块配置*/
                        memset(emTaskActive, 0x00,MAX_TASK_NUM*sizeof(ActiveFlag_t));
                        for(uiSOEVarCount = 0U; uiSOEVarCount < stSOEBlockInfo.uiSOEVarNum; uiSOEVarCount++)
                        {
...
static int8_t ConfigP2PInfoSectionParse(uint8_t ucSectionAddr[], FileHead_t* pstFileHead)
{
    ...
    if((NULL == ucSectionAddr) || (pstFileHead == NULL))
    {
        /*log*/
        cRet = RET_FAIL;
        DEBUG_CONFIG("1 NULL !\n");
    ...
    else
    {
        ...
        pstAppend = (SharedAppend_t *) SysGetSharedMemAppendInfoStartAddr();
        if((NULL == pstP2PConfigInfo) || (NULL == pstAppend))
        {
            /*log*/
            cRet = RET_FAIL;
            DEBUG_CONFIG("2 NULL !\n");
        ...
        {
            memset((puint8_t)pstP2PConfigInfo, 0x00, sizeof(P2PConfigInfo_t));
            ...
            for(uiCount = 0U; uiCount < uiP2PBlockNum; uiCount++)
            {
                ...
                uiBlockID = stP2PBlockHead.uiBlockID;
                
                //DEBUG_CONFIG("    Step  2 \n");
//                printf("P2P Info: %x %x %x %x\n",uiBlockID,uiStaID,stP2PBlockHead.usCommProperty,stP2PBlockHead.usUpdateFlag);
                

                
                if((stP2PBlockHead.usCommProperty == P2P_SEND_BLOCK) && (uiStaID <= MAX_P2P_BLOCK_NUM) && (uiBlockID < MAX_P2P_CTRL_INFO))
                {
                    if(1 == stP2PBlockHead.usUpdateFlag)
                    {
                        pstAppend->ucP2PCtrl[stP2PBlockHead.uiBlockID] = 0x01;
                    }
                    else if(2 == stP2PBlockHead.usUpdateFlag)
                    {
                        CommP2PSendCtrlClear((uint8_t)uiStaID);
                        pstAppend->ucP2PCtrl[stP2PBlockHead.uiBlockID] = 0x01;
                    }
                    else
                    {
//                        printf("P2P %d %d\n",stP2PBlockHead.uiBlockID,stP2PBlockHead.usUpdateFlag);
                    }
                    
                    pstP2PConfigInfo->uiP2PSendBlockTabl[uiSendBlockNum] = uiStaID;
                    
                    //DEBUG_CONFIG("    Step  3 \n");
                    
//                    uiSendBlockNum = pstP2PConfigInfo->stP2PConfigHead.uiSendBlockNum;
                    memcpy((puint8_t)&pstP2PConfigInfo->stP2PSendBlockInfo[uiStaID].stP2PBlockHead, \
                    ...
                    {
                        memcpy((puint8_t)&pstP2PConfigInfo->stP2PSendBlockInfo[uiStaID].stP2PVar[uiP2PVarCount], \
                    ...
                    pstP2PConfigInfo->stP2PSendBlockInfo[uiStaID].emActive = ACTIVE;
//                    pstP2PConfigInfo->stP2PConfigHead.uiSendBlockNum++;
                    uiSendBlockNum++;
                }
                else if((stP2PBlockHead.usCommProperty == P2P_RECV_BLOCK) && (uiStaID <= MAX_P2P_BLOCK_NUM) && (uiBlockID < MAX_P2P_CTRL_INFO))
                {
                    
                    //DEBUG_CONFIG("    Step  6 \n");
                    
                    pstP2PConfigInfo->uiP2PRecvBlockTabl[uiRecvBlockNum] = uiStaID;
...
static int8_t ConfigModbusMasterSectionParse(uint8_t ucSectionAddr[],FileHead_t* pstFileHead)
{
    ...
    else
    {
        ...
        pstAppend = (SharedAppend_t *) SysGetSharedMemAppendInfoStartAddr();
        if((NULL == pstModbusMasterInfo) || (NULL == pstCMSlotIdTabl) || (NULL == pstAppend))
        {
            /*log*/
        }
        else
                    ...
                    else
                    {
                        for(uiCount = 0U; uiCount < uiPortNum; uiCount++)
                        {
                            ...
                            {/*变化从站个数超出性能允许范围*/
                                ...
                                DEBUG_CONFIG("AddTCPNum || DelTCPNum > %d\n",MAX_MODBUS_SLAVE_NUM);
                            ...
                            if(uiMasterBlockNum > MAX_MODBUS_BLOCK_NUM)
                            {
                                cSecHandleResult = HANDLE_FAIL;
                                
                                DEBUG_CONFIG("MasterBlockNum ERR %d\n",ucBlockID);
...
//                                DEBUG_CONFIG("  uiModbusVarNum is : %d\n",uiModbusVarNum);
                                ...
                                if(1 == usPortUpdate)
                                {
                                    pstAppend->ucModBusCtrl[ucBlockID] = 0x01;
                                }
                            }
...
//                    DEBUG_CONFIG("  BLOCK %d :uiModbusVarNum is :%d \n",uiBlockID, uiModbusVarNum);
                    if((uiBlockID > MAX_MODBUS_AREA_NUM) || (uiModbusVarNum > MAX_MODBUS_RGN_VAR_NUM))
                    {
                        cSecHandleResult = HANDLE_FAIL;
                        break;
                    }
...
static void ConfigSetMasterPrm()
{
    ...
    if(pstSysIoModInfo != NULL)
    {
        IPBUSSetSlaveDelayMin(pstSysIoModInfo->stSysPMInfo.usMinTsdr);
        IPBUSSetSlaveDelayMax(pstSysIoModInfo->stSysPMInfo.usMaxTsdr);
        IPBUSSetSlaveAckTimeout(pstSysIoModInfo->stSysPMInfo.usCycResTime);
        IPBUSSetMasterMailTimeout(pstSysIoModInfo->stSysPMInfo.usAcyResTime);
        IPBUSSetRetransmissionNumber(pstSysIoModInfo->stSysPMInfo.usMaxRetryLimit);
    }
...
static void ConfigDownloadIOConfig(void)
{
    ...
    if((IPBUS_SLAVE_ENABLE_BUF0 == ucWRParaBufID) || (IPBUS_SLAVE_ENABLE_BUF1 == ucWRParaBufID))
    {
        ...
        do
        {
            ...
            if(((uiStartTime%TASK_PERIOD) >= 20)&&((uiStartTime%TASK_PERIOD) <= 21))
            {
                break;
            }
...
{
    uint8_t i = 0;
    ConfigDataAreaAllInit();
    
    ConfigDisableAreaInit();
    ...
    SharedSOEInit();
    SharedSOEQueueCfg();
    DataProcessSoftSOEInit(TASK_UP1,false);
    DataProcessSoftSOEInit(TASK_UP2,false);
...
static void ConfigDataAreaAllInit(void)
{
    ...
    for(emTaskIndex = TASK_UP1; emTaskIndex < MAX_TASK_NUM; emTaskIndex++)
    {
        ...
        pstCtrlStationConfigInfo = SharedGetCtrlStationInfoWrit();
        
        if((pstCtrlStationConfigInfo != NULL) && (ACTIVE == pstCtrlStationConfigInfo->stTaskConfigInfo[emTaskIndex].emActiveFlag))
        {
            pucCodeAreaAddr = SharedGetCodeAreaWrit();
            if(pucCodeAreaAddr != NULL)
            {
                uiGlobalInitEntry = pstCtrlStationConfigInfo->stTaskConfigInfo[emTaskIndex].uiGlobalInitEntry;
        ...
        GlobalInitPtr = uiGlobalInitEntry;
        
        if((pucDataAreaAddr != NULL) && (GlobalInitPtr != NULL))
        {
            printf("TOC UP%d Init %x %x\n",emTaskIndex,GlobalInitPtr,pucDataAreaAddr);
            disable_inst();
...
static void ConfigDataAreaIncreInit(void)
{
    ...
    for(emTaskIndex = TASK_UP1; emTaskIndex < MAX_TASK_NUM; emTaskIndex++)
    {
        ...
        if((pstCtrlStationConfigInfo != NULL) && (ACTIVE == pstCtrlStationConfigInfo->stTaskConfigInfo[emTaskIndex].emActiveFlag))
        {
            ...
            if(pucCodeAreaAddr != NULL)
            {
                uiGlobalInitEntry = pstCtrlStationConfigInfo->stTaskConfigInfo[emTaskIndex].uiChangeGlobalInitEntry;
                uiGlobalInitEntry += (uint32_t)pucCodeAreaAddr;
            }
        }
        else
...
/*
***********************************************************************************************************************
* 函数名称: ConfigDisableAreaInit
*
* 功能描述: 进行Disable数据区清空初始化
*
* 输入参数: 无
*
* 输出参数: 无
...
static void ConfigDisableAreaInit(void)
{
    ...
    /*初始化下装，进行全Disable数据区初始化*/
    pucDisableAreaAddr = SysGetRtDataDisableFlagStartAddr(RT_DATA_TYPE_Q);
    if((pucDisableAreaAddr != NULL))
    {
        memset(pucDisableAreaAddr,0,MAX_RTDATA_Q_AREA_LEN);
    }
    pucDisableAreaAddr = SysGetRtDataDisableFlagStartAddr(RT_DATA_TYPE_I);
    if((pucDisableAreaAddr != NULL))
    {
        memset(pucDisableAreaAddr,0,MAX_RTDATA_I_AREA_LEN);
    }
    pucDisableAreaAddr = SysGetRtDataDisableFlagStartAddr(RT_DATA_TYPE_G);
    if((pucDisableAreaAddr != NULL))
    {
        memset(pucDisableAreaAddr,0,MAX_RTDATA_G_AREA_LEN);
    }
