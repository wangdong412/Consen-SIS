...
void SyncInitSyncData(SyncData_t* pstSyncData);
...
uint32_t SyncHandleStateData(const SysPMStateInfo_t* pstOneStatData, PMController_t emSrcPMId,int16_t sDataLen, puint8_t pucAjustFlg);
...
void SyncStart(void)
{
    ...
    if(pstLocalPMState != NULL)
    {
        ...
        pstLocalPMState->emSwitchKeys = GetSwitchKeyPos();
        SharedSetSwitchPosTmp(pstLocalPMState->emSwitchKeys);
        SharedSetSwitchPos(pstLocalPMState->emSwitchKeys);
    ...
    do
    {
        ...
        if((!(usState & BITMASK(0))) && (!(usState & BITMASK(1))))
        {
            bAjustFlg = true;
        }
...
//                    s_stSyncData.stSyncRecvMSGA.stSyncSrvData.stPMState.emDownloadFlg);
               ...
               (NOT_DOWNLOAD == s_stSyncData.stSyncRecvMSGA.stSyncSrvData.stPMState.emDownloadFlg))
            {
                ...
                SyncHandleStateData(&s_stSyncData.stSyncRecvMSGA.stSyncSrvData.stPMState, emNextPMId, \
                        s_stSyncData.sLinkASrvLen, &bAjustFlg);
            }
            else
            {
...
//                    s_stSyncData.stSyncRecvMSGB.stSyncSrvData.stPMState.emDownloadFlg);
               ...
               (NOT_DOWNLOAD == s_stSyncData.stSyncRecvMSGB.stSyncSrvData.stPMState.emDownloadFlg))
            {
                ...
                SyncHandleStateData(&s_stSyncData.stSyncRecvMSGB.stSyncSrvData.stPMState, emPrePMId, \
                        s_stSyncData.sLinkBSrvLen, &bAjustFlg);
            }
            else
            {
        ...
        emTimerStatus = SysGetTimerStatus(emLocalPMId);
        if(TIMER_MASTER == emTimerStatus)
        {
            bAjustFlg = true;
        }
        ...
        udelay(1000000u);
    }while((SysGetBin1Num(uiRecordCon) < NUM_OF_PM) || (bAjustFlg == false));
...
void SyncInitSyncData(SyncData_t* pstSyncData)
{
    if(pstSyncData != NULL)
    {
        memset(pstSyncData, 0x00, 8*sizeof(int16_t));
        memset(&pstSyncData->stSyncRecvMSGA.stSyncSrvData.stPMState, 0x00, sizeof(SysPMStateInfo_t));
        pstSyncData->stSyncRecvMSGA.stSyncSrvData.uiSyncSrvDataLen = 0;
        pstSyncData->stSyncRecvMSGA.stSyncPrjData.uiSyncPrjDataLen = 0;
        memset(&pstSyncData->stSyncRecvMSGB.stSyncSrvData.stPMState, 0x00, sizeof(SysPMStateInfo_t));
        pstSyncData->stSyncRecvMSGB.stSyncSrvData.uiSyncSrvDataLen = 0;
        pstSyncData->stSyncRecvMSGB.stSyncPrjData.uiSyncPrjDataLen = 0;
        memset(&pstSyncData->stSyncSendMSGA.stSyncSrvData.stPMState, 0x00, sizeof(SysPMStateInfo_t));
        pstSyncData->stSyncSendMSGA.stSyncSrvData.uiSyncSrvDataLen = 0;
        pstSyncData->stSyncSendMSGA.stSyncPrjData.uiSyncPrjDataLen = 0;
        memset(&pstSyncData->stSyncSendMSGB.stSyncSrvData.stPMState, 0x00, sizeof(SysPMStateInfo_t));
        pstSyncData->stSyncSendMSGB.stSyncSrvData.uiSyncSrvDataLen = 0;
        pstSyncData->stSyncSendMSGB.stSyncPrjData.uiSyncPrjDataLen = 0;
    }
...
void SyncRecvSyncData(SyncData_t* pstSyncData)
{
    ...
    {/* 校验接收缓存指针不为空 */
        ...
        if(sRetLen == -3)
        {
            LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_NEXT_LINK_ERR);
        }
...
//                    printe("N%d %d\n",uiRecvNextPackNum,pstPMStateInfo->ucPackNum);
                    ...
                    LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_NEXT_NUM_ERR);
                }
                ...
                else
                {
                    ...
                    LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_NEXT_PMSTAT_ERR);
                }
        ...
        if(sRetLen == -3)
        {
            LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_PRE_LINK_ERR);
        }
...
//                    printe("P%d %d\n",uiRecvPrePackNum,pstPMStateInfo->ucPackNum);
                    ...
                    LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_PRE_NUM_ERR);
                }
                ...
                else
                {
                    ...
                    LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_PRE_PMSTAT_ERR);
                }
                uiBuffIndex += sizeof(SysPMStateInfo_t);
...
//    uint8_t ucPreSendFlag = 0U;
    ...
    if(pstSyncData != NULL)
    {
        if((SysGetWorkStatus(emNextPMId) != WS_OFFLINE) || (true == bForceSendFlg))
        {/*后一系在线*/
//            memset((puint8_t)ucSyncSendBuff, 0x00, MAX_SYNC_SIZE);
            ...
            else
            {
                ...
                pstSyncData->stSyncSendMSGA.stSyncSrvData.stPMState.stAjustSyncTime.ullSystemTime = SysGetSystemtime();
            }
...
//            s_uiStateSendFlag1 = 0U;
            ...
            {/* 发送错误重新启动文件传输 */
                ...
                LogWrite(LOG1_ID_APP_SYNC_LV0_SEND_NEXT_ERR);
            }
            else
            {/* 发送成功，等待应答 */
        ...
        if((SysGetWorkStatus(emPrePMId) != WS_OFFLINE) || (true == bForceSendFlg))
        {/*前一系在线*/
//            memset((puint8_t)ucSyncSendBuff, 0x00, MAX_SYNC_SIZE);
            uiBuffIndex = 0U;
            ...
            else
            {
                ...
                pstSyncData->stSyncSendMSGB.stSyncSrvData.stPMState.stAjustSyncTime.ullSystemTime = SysGetSystemtime();
            }
...
//            s_uiStateSendFlag2 = 0U;
            ...
            {/* 发送错误重新启动文件传输 */
                ...
                LogWrite(LOG1_ID_APP_SYNC_LV0_SEND_PRE_ERR);
            }
            else
            {/* 发送成功，等待应答 */
        ...
        {/* 若数据长度等于PM状态数据结构，说明只有有状态数据 */
         /*具体的PM同步状态信息包含哪些内容，需要经讨论决定，而后确定具体处理函数*/
            uiRet = SyncHandleStateData(&pstSyncData->stSyncRecvMSGA.stSyncSrvData.stPMState, emNextPMId, \
                    pstSyncData->sLinkASrvLen, &bAjustFlg);
        }
    ...
    {/* 处理链接B接收到数据，即前一个PM 发送的数据 */
        ...
        {/* 若数据长度等于PM状态数据结构，说明只有状态数据 */
            uiRet = SyncHandleStateData(&pstSyncData->stSyncRecvMSGB.stSyncSrvData.stPMState, emPrePMId, \
                                        pstSyncData->sLinkBSrvLen, &bAjustFlg);
        }
           ...
           ((pstLocalPMState->emSyncStatus == SS_NOSYNC) && (pstLocalPMState->emDownloadFlg == NOT_DOWNLOAD)))
        {
            ...
            {/* 发送端PM同步时钟状态为主 */
                ...
                {/* 若本机未接收过校时，直接校时处理 */
                    ...
                    SysAdjustSystemTime(pstPMState->stAjustSyncTime.ullSystemTime);
...
//                    DEBUG_SYNC("AJUST_HAS\n");
    ...
    //                    DEBUG_SYNC("LOG_SYNC_AJUSTTIME_BACK \n");
            ...
            {
                printf("Ajust Stat:%d %d\n",pstPMState->stAjustSyncTime.uiMaster,pstPMState->stAjustSyncTime.emAjustEn);
            }
    ...
    switch(pstSyncCtrlSrv->uiSrvId)
    {
        case SYS_BROADCAST_CTRLSRV:
        {
            ...
            if((pstSysSrvInfo != NULL) && (uiDataLen <= CFG_SINGLE_SRV_SIZE))
            {
                ...
                LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_CTRL_SRV);
                /* 若该状态数据需要应答 */
                if(NEED_ACK == pstSrvData->emAckFlag)
                {/**/
                    /* 清空应答数据结构 */
        ...
        switch(stSyncTask.emTaskStatus)
        {
            case TASK_SEND:
                ...
                if(stSyncSrvData.uiSrvDataLen > CFG_SINGLE_SRV_SIZE)
                {
                    stSyncSrvData.uiSrvDataLen = 0;
                    DEBUG_SYNC("SrvLen > %d \n",CFG_SINGLE_SRV_SIZE);
                    LogWrite(LOG1_ID_APP_SYNC_LV0_BROAD_SRV_LEN_ERR);
                    break;
                }
                ...
                LogWrite(LOG1_ID_APP_SYNC_LV0_BROAD_CTRL_SRV);
...
//                        DEBUG_SYNC("SEND P:Len: %d\n",uiSendLen);
            ...
            case TASK_WAITACK:
                ...
                {/* 超时时间内接收到应答，CTRL服务同步成功 */
                    ...
                    LogWrite(LOG1_ID_APP_SYNC_LV0_BROAD_SRV_ACK_OK);
                ...
                {/* 判断是否超时 */
                    ...
                    {/* 等待应答超时 */
                        ...
                        DEBUG_SYNC("CTRL WAIT ACK TO  Cur: %d Time: %d \n",unCurTime.stTimeStruct.uiLow32b, \
                                pstSysSrvInfo->stSysSrvInfoHead.unTimeOutTime.stTimeStruct.uiLow32b);
                        if(NO_ACK != s_uiWaitAck[emPrePMId])
                        {
                            LogWrite(LOG1_ID_APP_SYNC_LV0_BROAD_SRV_PRE_ACK_TO);
                        }
                        
                        if(NO_ACK != s_uiWaitAck[emNextPMId])
                        {
                            LogWrite(LOG1_ID_APP_SYNC_LV0_BROAD_SRV_NEXT_ACK_TO);
                        }
...
void SyncRecvData(SyncRealData_t* pstSyncPrjData, PMController_t emSrcPMId)
{
    ...
    puint8_t pucPRGBaseAddr = 0U;                                   /* PRG数据区域基址         */
    int32_t iSendLen = 0;                                           /* 发送数据长度         */
    ...
    static SysRtDataType_t s_emAreaId = INVALID_RT_DATA_TYPE;
    ...
    do
    {
           ...
           (pstSyncPrjData->stDataHead.emAreaId != RT_DATA_TYPE_Q))
        {
            ...
            LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_DATA_AREA_ERR);
        ...
        else
        {
            if(RT_DATA_TYPE_G == pstSyncPrjData->stDataHead.emAreaId)
            {
                if((pstSyncPrjData->stDataHead.uiOffset + pstSyncPrjData->stDataHead.uiDataLen) > MAX_RTDATA_G_AREA_LEN)
                {
                    ...
                    LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_DATA_G_LEN_ERR);
            ...
            if(RT_DATA_TYPE_I == pstSyncPrjData->stDataHead.emAreaId)
            {
                if((pstSyncPrjData->stDataHead.uiOffset + pstSyncPrjData->stDataHead.uiDataLen) > MAX_RTDATA_I_AREA_LEN)
                {
                    ...
                    LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_DATA_I_LEN_ERR);
            ...
            if(RT_DATA_TYPE_Q == pstSyncPrjData->stDataHead.emAreaId)
            {
                if((pstSyncPrjData->stDataHead.uiOffset + pstSyncPrjData->stDataHead.uiDataLen) > MAX_RTDATA_Q_AREA_LEN)
                {
                    ...
                    LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_DATA_Q_LEN_ERR);
        ...
        if(pstSyncPrjData->stDataHead.emTaskID >= MAX_TASK_NUM)
        {
            ...
            LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_DATA_TASKID_ERR);
        ...
        pucPRGBaseAddr = SysGetPRGRtDataStartAddr(pstSyncPrjData->stDataHead.emAreaId);
        
        if((NULL == pucAreaBaseAddr) || (NULL == pucPRGBaseAddr))
        {
...
//                pstSyncPrjData->stDataHead.uiOffset, pstSyncPrjData->stDataHead.uiDataLen,pucAreaBaseAddr);
        if(s_emAreaId != pstSyncPrjData->stDataHead.emAreaId)
        {
            switch(pstSyncPrjData->stDataHead.emAreaId)
            {
                case RT_DATA_TYPE_G:
                {
                    LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_DATA_G_AREA);
                }
                break;
                case RT_DATA_TYPE_I:
                {
                    LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_DATA_I_AREA);
                }
                break;
                case RT_DATA_TYPE_Q:
                {
                    LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_DATA_Q_AREA);
                }
                break;
                default:
                {
...
void SyncRecvRealDataOk(SyncRealData_t* pstSyncPrjData,PMController_t emSrcPMId)
{
    ...
    if(uiDataLen < uiAppendDataLen)
    {
        ...
        LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_DATA_A_LEN_ERR);
    ...
    {
        LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_DATA_A_AREA);
        
        memcpy((puint8_t)(&pstAppend->ucModBusCtrl[0]), &pstSyncPrjData->ucData[0], \
                uiAppendDataLen);
        uiDataLen -= uiAppendDataLen;
        if((uiDataLen > 0) && (uiDataLen <= RETAIN_SRAM_DISABLE_SIZE))
        {
            ...
            if(usDisableTablLen == pstDisableTablHead->uiSize)
            {
                ...
                if(uiCheckSumCal != pstDisableTablHead->uiCheckSum)
                {
                    ...
                    LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_DATA_A_CRC_ERR);
...
//                    printf("\n");
    ...
    LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_DATA_END);
...
void SyncSendData(PMController_t emDstPMId)
{
    ...
    if(pstTaskConfigInfo != NULL)
    {
        ...
        do
        {
            ...
            if(s_stSyncDataStatus.uiCurDataLen >= uiAreaUsedLen)
            {
                ...
                else
                {
                    ...
                    switch(s_stSyncDataStatus.emAreaId)
                    {
                        case RT_DATA_TYPE_G:
                        {
                            LogWrite(LOG1_ID_APP_SYNC_LV0_SEND_DATA_G_AREA);
                        }
                        break;
                        case RT_DATA_TYPE_I:
                        {
                            LogWrite(LOG1_ID_APP_SYNC_LV0_SEND_DATA_I_AREA);
                        }
                        break;
                        case RT_DATA_TYPE_Q:
                        {
                            LogWrite(LOG1_ID_APP_SYNC_LV0_SEND_DATA_Q_AREA);
                        }
                        break;
                        default:
                        {
        ...
        }while(true != bSendDataFlg);

        /* 初始化数据区基地址 */
        pucAreaBaseAddr = SysGetPRGRtDataStartAddr(s_stSyncDataStatus.emAreaId);
        
        if((true == bSendDataFlg) && (pucAreaBaseAddr != NULL))
        {
            /* 初始化数据区使用长度 */
            if(RT_DATA_TYPE_G == s_stSyncDataStatus.emAreaId )
            {
...
//                printe("S %d",s_stSyncDataStatus.emAreaId);
    //            SyncTime64_t unCurTime;
...
void SyncSendDataOk(PMController_t emDstPMId)
{
    ...
    if(TASK_UP1 == s_stSyncDataStatus.emTaskID)
    {
        ...
        LogWrite(LOG1_ID_APP_SYNC_LV0_SEND_DATA_A_AREA);
    }
...
//            emRecvSrvType = SYNC_COMPLETE;
    ...
    {/* 接收到数据长度为0或接收到错误数据 */
        ...
        if(SysDiffSyncTime(&s_stSyncTimer.unTaskStartTime, &unCurTime) >= uiTimeOut)
        {/* 等待应答超时或接收应答错误 */

            LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_ACK_TO);
    ...
    {/* 接收到应答数据 */
        if(emLastStatus != TASK_COMPLETE)
        {
            ...
            {/* 接收到错误应答数据 */
                ...
                if(emRecvSrvType != pstSyncAck->stSyncHead.emSrvType)
                {
                    LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_ACK_TYPE_ERR);
                }
                else
                {
                    LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_ACK_ERR);
                }
            ...
            {/* 接收到正确应答数据 */
                ...
                {/* 发送新文件 */
                    LogWrite(LOG1_ID_APP_SYNC_LV0_SEND_SINGLE_FILE_END);
                    ...
                    {/* 文件发送完成，更新同步任务 */
                        ...
                        LogWrite(LOG1_ID_APP_SYNC_LV0_SEND_FILE_END);
                    }
                    else
                    {
                        SysSyncPrjQueueUpdate(emLastStatus);
                ...
                else if(TASK_SYNC_OK == emLastStatus)
                {
                    ...
                    LogWrite(LOG1_ID_APP_SYNC_LV0_SEND_DATA_END);
                }
                else
                {
                    SysSyncPrjQueueUpdate(emLastStatus);
            ...
            else
            {
                ...
                {/* 不是接收到新文件 */
                    ...
                    if(usWritDataLen != uiDataLen)
                    {
                        ...
                        LogWrite(LOG1_ID_APP_SYNC_LV0_DDR_WR_FILE_ERR);
                    }
                    else
                    {
...
//                      printe("%d",pstSyncRecvFile->stFileHead.emCfgFileType);
                ...
                {/* 接收到新文件 */
                    ...
                    LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_NEW_FILE);
            ...
            case FILE_WRITE_START:
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        LogWrite(LOG1_ID_APP_SYNC_LV0_FLASH_WR_FILE_START_ERR);
                    }
                }
            }
            break;
            case FILE_WRITE_CONTINUE:
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        LogWrite(LOG1_ID_APP_SYNC_LV0_FLASH_WR_FILE_CONT_ERR);
                    }
                }
            }
            ...
            case FILE_WRITE_DONE:
            {
                ...
                else
                {
                    ...
                    {/* 接收到文件最后一包数据 */
                        ...
                        LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_SINGLE_FILE_END);
                        ...
                        if(CFG_CTRL_STATION == s_stSyncFileStatus.emCfgFileType)
                        {
                            ...
                            LogWrite(LOG1_ID_APP_SYNC_LV0_RECV_FILE_END);
                        }
                    }
                    else
                    {
                        LogWrite(LOG1_ID_APP_SYNC_LV0_FLASH_WR_FILE_END_ERR);
                    }
                }
            }
    ...
    do
    {
        ...
        {/* 发送新文件 */
            if(s_stSyncFileStatus.emCfgFileType >= INVALID_CFG_FILE_TYPE)
            {
                ...
                LogWrite(LOG1_ID_APP_SYNC_LV0_SEND_NEW_FILE);
            }
        }
        ...
        {/* 文件不可读 */
            ...
            LogWrite(LOG1_ID_APP_SYNC_LV0_DDR_RD_FILE_STAT_ERR);
        ...
        if(usRealFileLen != usReadFileLen)
        {
            ...
            LogWrite(LOG1_ID_APP_SYNC_LV0_DDR_RD_FILE_ERR);
        }
        else
        {
            printf("SD F Len %d!\n",usRealFileLen);
