...
#define DEBUG_SYNC(format, args...)  printf ("D_SYNC: ") ; printf(format, ##args)
#else
...
/* 最大数据交换长度 */
#define MAX_SRV_SYNC_SIZE               (1024*3 + 512)          /* 服务数据同步数据大小*/
#define MAX_PRJ_SYNC_SIZE               (1024*4 + 512 - sizeof(uint32_t))          /* 工程数据同步数据大小*/

#define MAX_SRV_TYPE_NUM                (4)

#define CFG_MAX_SYNC_SRV_SIZE           (MAX_SRV_SYNC_SIZE - sizeof(SysPMStateInfo_t) - sizeof(uint32_t))

//#define CFG_SINGLE_SRV_SIZE             ((CFG_MAX_SYNC_SRV_SIZE/MAX_SRV_TYPE_NUM) - 5*sizeof(uint32_t))
#define CFG_SINGLE_SRV_SIZE             ((CFG_MAX_SYNC_SRV_SIZE) - 5*sizeof(uint32_t))
...
/* 同步文件最大文件长度 */
//#define CFG_MAX_SYNC_FILE_SIZE          (1024*4)
#define CFG_MAX_SYNC_FILE_SIZE          (2*1024)
/* 同步数据头长度 */
...
/* 同步定时器应答超时时间 (2000ms) 单位是100us */
#define SYNC_TIMER_TIMEOUT              (30000U)


#define SYS_ONE_SEC_VALUE               (0x00002710U)
...
typedef struct SyncCtrlSrvTag
{
    ...
    srv_srvdata_type_t emSrvType;                       /* 用于取消服务，指示取消哪一类服务*/
...
typedef struct SrvDataTag
{
    ...
    uint8_t ucData[CFG_SINGLE_SRV_SIZE];                /* 具体数据        */
}SrvData_t;

typedef struct SyncSrvDataTag
{
    SysPMStateInfo_t stPMState;                          /* PM用于同步的信息，具体数据结构待定*/
    uint32_t uiSyncSrvDataLen;                           /* 服务数据长度，不包含状态数据*/
    uint8_t ucSrvData[CFG_MAX_SYNC_SRV_SIZE];            /* 服务数据 */
}SyncSrvData_t;
...
{/*内容分为：
        服务数据：包含数据长度，PM信息，服务数据。总长度：1024*6
...
typedef struct SyncDataTag
{
    ...
    SyncMSG_t stSyncRecvMSGA;                           /* 链路A接收同步数据缓存 */
    SyncMSG_t stSyncRecvMSGB;                           /* 链路B接收同步数据缓存 */
    SyncMSG_t stSyncSendMSGA;                           /* 链路A发送同步数据缓存 */
    SyncMSG_t stSyncSendMSGB;                           /* 链路B发送同步数据缓存 */
}SyncData_t;
