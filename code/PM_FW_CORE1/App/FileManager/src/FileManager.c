...
/* Erase RTS flag */
static void FMEraseRTSFlag(void);
/* Erase FPGA flag */
static void FMEraseFPGAFlag(void);
/* Erase RTS area0 */
static void FMEraseRTSArea0(void);
/* Erase RTS area1 */
static void FMEraseRTSArea1(void);
/* Erase FPGA area0 */
static void FMEraseFPGAArea0(void);
/* Erase FPGA area1 */
static void FMEraseFPGAArea1(void);
/*
***********************************************************************************************************************
...
void FileManagerCycle(void)
{
    ...
    if(true == bReadReq)
    {
        ...
        switch(uiCTW)
        {
            ...
            case FM_ERASE_RTS_FLG:
            {
                FMEraseRTSFlag();
            }
            break;

            case FM_ERASE_FPGA_FLG:
            {
                FMEraseFPGAFlag();
            }
            break;

            case FM_ERASE_RTS_AREA0:
            {
                FMEraseRTSArea0();
            }
            break;

            case FM_ERASE_RTS_AREA1:
            {
                FMEraseRTSArea1();
            }
            break;

            case FM_ERASE_FPGA_AREA0:
            {
                FMEraseFPGAArea0();
            }
            break;

            case FM_ERASE_FPGA_AREA1:
            {
                FMEraseFPGAArea1();
            }
            break;
...
{
    bool_t bSendAck = false;                    /* 发送应答是否成功 */
    int32_t iRet = FS_FAIL;                     /* 擦除flash返回值 */

    /* 擦除Nor-flash中的RTS flag */
    iRet = SysEraseNorflash(FW_FM_RTS_FLG_START_BLK, FW_FM_RTS_FLG_BLK_NUM);
    ...
    {
        /* 擦除成功 */
        s_stSysFMAck.uiStatusWord |= FM_ERASE_RTS_FLG_ACK;
        s_stSysFMAck.uiErrCode = FM_OK;
        
        printf("Erase %d %d\n",FW_FM_RTS_AREA1_START_BLK,FW_FM_RTS_BLK_NUM);
    }
    else
    {
        /* 擦除失败 */
        s_stSysFMAck.uiStatusWord &= FM_ERASE_RTS_FLG_NACK;
        s_stSysFMAck.uiErrCode = FM_ERASE_FAILED;
    }
...
static void FMEraseFPGAFlag(void)
{
    ...
    /* 擦除Nor-flash中的FPGA flag */
    iRet = SysEraseNorflash(FW_FM_FPGA_FLG_START_BLK, FW_FM_FPGA_FLG_BLK_NUM);
    ...
    {
        /* 擦除成功 */
        s_stSysFMAck.uiStatusWord |= FM_ERASE_FPGA_FLG_ACK;
        s_stSysFMAck.uiErrCode = FM_OK;
        
        printf("Erase %d %d\n",FW_FM_FPGA_FLG_START_BLK,FW_FM_FPGA_FLG_BLK_NUM);
    }
    else
    {
        /* 擦除失败 */
        s_stSysFMAck.uiStatusWord &= FM_ERASE_FPGA_FLG_NACK;
        s_stSysFMAck.uiErrCode = FM_ERASE_FAILED;
    }
...
static void FMEraseRTSArea0(void)
{
    ...
    /* 擦除Nor-flash中的RTS AREA0 */
    iRet = SysEraseNorflash(FW_FM_RTS_AREA0_START_BLK, FW_FM_RTS_BLK_NUM);
    ...
    {
        /* 擦除成功 */
        s_stSysFMAck.uiStatusWord |= FM_ERASE_RTS_AREA0_ACK;
        s_stSysFMAck.uiErrCode = FM_OK;
        
        printf("Erase %d %d\n",FW_FM_RTS_AREA0_START_BLK,FW_FM_RTS_BLK_NUM);
    }
    else
    {
        /* 擦除失败 */
        s_stSysFMAck.uiStatusWord &= FM_ERASE_RTS_AREA0_NACK;
        s_stSysFMAck.uiErrCode = FM_ERASE_FAILED;
    }
...
static void FMEraseRTSArea1(void)
{
    ...
    /* 擦除Nor-flash中的RTS AREA1 */
    iRet = SysEraseNorflash(FW_FM_RTS_AREA1_START_BLK, FW_FM_RTS_BLK_NUM);
    ...
    {
        /* 擦除成功 */
        s_stSysFMAck.uiStatusWord |= FM_ERASE_RTS_AREA1_ACK;
        s_stSysFMAck.uiErrCode = FM_OK;
    ...
    {
        /* 擦除失败 */
        s_stSysFMAck.uiStatusWord &= FM_ERASE_RTS_AREA1_NACK;
        s_stSysFMAck.uiErrCode = FM_ERASE_FAILED;
    }
...
static void FMEraseFPGAArea0(void)
{
    ...
    /* 擦除Nor-flash中的FPGA AREA0 */
    iRet = SysEraseNorflash(FW_FM_FPGA_AREA0_START_BLK, FW_FM_FPGA_BLK_NUM);
    ...
    {
        /* 擦除成功 */
        s_stSysFMAck.uiStatusWord |= FM_ERASE_FPGA_AREA0_ACK;
        s_stSysFMAck.uiErrCode = FM_OK;
        
        printf("Erase %d %d\n",FW_FM_FPGA_AREA0_START_BLK,FW_FM_FPGA_BLK_NUM);
    }
    else
    {
        /* 擦除失败 */
        s_stSysFMAck.uiStatusWord &= FM_ERASE_FPGA_AREA0_NACK;
        s_stSysFMAck.uiErrCode = FM_ERASE_FAILED;
    }
...
static void FMEraseFPGAArea1(void)
{
    ...
    /* 擦除Nor-flash中的FPGA AREA1 */
    iRet = SysEraseNorflash(FW_FM_FPGA_AREA1_START_BLK, FW_FM_FPGA_BLK_NUM);
    ...
    {
        /* 擦除成功 */
        s_stSysFMAck.uiStatusWord |= FM_ERASE_FPGA_AREA1_ACK;
        s_stSysFMAck.uiErrCode = FM_OK;
        
        printf("Erase %d %d\n",FW_FM_FPGA_AREA1_START_BLK,FW_FM_FPGA_BLK_NUM);
    }
    else
    {
        /* 擦除失败 */
        s_stSysFMAck.uiStatusWord &= FM_ERASE_FPGA_AREA1_NACK;
        s_stSysFMAck.uiErrCode = FM_ERASE_FAILED;
    }
