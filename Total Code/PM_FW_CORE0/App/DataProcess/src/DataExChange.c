...
void DataExChange(uint8_t ucSrcData[], uint16_t usSrcDataLen, \
                  uint8_t ucPreData[], uint8_t ucNextData[], puint8_t pucPreDataFlg, puint8_t pucNextDataFlg, uint8_t ucDataType)
{
    uint16_t usState = 0U; 
    ...
    uint8_t ucPreRecvTOFlg = 0xff;
    uint8_t ucNextRecvTOFlg = 0xff;
    uint32_t uiTimeOut = 0;
    uint8_t ucPreSendFlag = 0;
    uint8_t ucNextSendFlag = 0;
    int16_t res = -1;
    ...
    if(pstAppend != NULL)
    {
        pstAppend->stPMState.stPMFirmwareState.ucNextPMBUSState = 
                pstAppend->stPMState.stPMFirmwareState.ucNextPMBUSState & 0xfd;
        
        pstAppend->stPMState.stPMFirmwareState.ucPrePMBUSState = 
                pstAppend->stPMState.stPMFirmwareState.ucPrePMBUSState & 0xfd;
    }
    ...
    *pucNextDataFlg = 0x00;
    ...
    do
    {
        ...
        uiCurrentTime = getTblus();
    }while(((uiCurrentTime - uiStartTime) < uiTimeOut) && (SysGetBin1Num(uiRecordCon) < NUM_OF_PM));

    if((uiCurrentTime - uiStartTime) >= uiTimeOut)
    {
        if(0xff == ucPreRecvTOFlg)
        {
            PMHandleRecvTO(emPrePMId);
            if(pstAppend != NULL)
            {
                pstAppend->stPMState.stPMFirmwareState.ucPrePMBUSState = 
                        pstAppend->stPMState.stPMFirmwareState.ucPrePMBUSState | 0x02;
            }
        }
        
        if(0xff == ucNextRecvTOFlg)
        {
            PMHandleRecvTO(emNextPMId);
            
            if(pstAppend != NULL)
            {
                pstAppend->stPMState.stPMFirmwareState.ucNextPMBUSState = 
                        pstAppend->stPMState.stPMFirmwareState.ucNextPMBUSState | 0x02;
            }
        }
    }
...
bool_t DataExChangeSend(PMController_t emPMID , uint8_t ucSendDest[], uint16_t usDataLen)
{
    ...
    if((usDataLen <= MAX_IO_EXCHANGE_SIZE) && (ucSendDest != NULL) && (emPMID < NUM_OF_PM))
    {
        /*安全层打包接口编译有问题，请王东解决*/
        /*将IO数据进行安全层打包*/
        usSendLen = MAX_IO_EXCHANGE_SIZE;
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    sRetSend = PMBUS_Send_DataEx(PM_BUS_A, PM_BUFF_EXDATA, (puint8_t)&ucSendBuff[0], \
                            (int32_t)usSendLen, PMBUS_SEND_TIMEOUT);
                }
                
                if(emPMID == emPrePMId)
                {
                    sRetSend = PMBUS_Send_DataEx(PM_BUS_B, PM_BUFF_EXDATA, (puint8_t)&ucSendBuff[0], \
                            (int32_t)usSendLen, PMBUS_SEND_TIMEOUT);
                }
    ...
    *pusDataLen = 0U;
    if((ucRecvData != NULL) && (emPMID < NUM_OF_PM))
    {
        ...
        if((sRetRecv > 0) && (sRetRecv <= MAX_IO_EXCHANGE_SIZE))
        {
            /*安全层解包接口编译有问题，请王东解决*/
            /*将数据经安全层解包*/
            usRecvLen = (DATAPROCESS_AI_QDATA_LEN +DATAPROCESS_AI_SAFETYDATA_LEN) * DATAPROCESS_SLAVE_POLL_NUM_TOTAL;
                                  ...
                                  uint8_t ucOldData[],uint8_t ucDestBuff[], uint8_t ucDiagBuff[])
{
    ...
    pstVoteModInfo = SharedGetVoteInfo(emTaskID,ucModID);
    ...
    if((pstVoteModInfo != NULL) && (emVoteVarType < VVT_INVALID_ID))
    {
        ...
        for(usCount = 0U;usCount < usChannelNum; usCount++)
        {
            ...
            if((VVT_ANALOG_AO == emVoteVarType) || (VVT_ANALOG_AI == emVoteVarType))
            {
                ...
                {
                    if(SysGetDeviation(usValue2, usValue3) > MAX_ANALOG_DEVIATION)
                    {
                        ucQualityBit2 = false;
                        ucQualityBit3 = false;
                    }
                ...
                {
                    if(SysGetDeviation(usValue1, usValue3) > MAX_ANALOG_DEVIATION)
                    {
                        ucQualityBit1 = false;
                        ucQualityBit3 = false;
                    }
            ...
            if(VVT_FLOAT == emVoteVarType)
            {
                ...
                else
                {
                    ...
                    {
                        memcpy(&fVoteValue, &pstVoteModInfo->stVoteChannelInfo[usCount].uiSafetyValue, sizeof(float_t));
                    }
                }
...
static uint8_t DataExChangeCheckTolerate(uint8_t ucModAddr, uint8_t ucChannelID)
{
    uint8_t ucChannelTolerate = SharedGetVoteTolerate();
