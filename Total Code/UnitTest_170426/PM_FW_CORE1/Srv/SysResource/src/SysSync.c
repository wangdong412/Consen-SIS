...
static SysSrvInfo_t s_stCtrlSrvInfo[MAX_TASK_NUM];

SysJoinReqStatus_t  stSysActiveStatus;
...
{
    memset(&s_stSysSyncSrvQueue, 0x00, sizeof(SysSyncSrvQueue_t));
    memset(&s_stSysSyncPrjQueue, 0x00, sizeof(SysSyncPrjQueue_t));
    memset(&s_emSyncDataInit, 0x00, sizeof(sync_data_init_t));
    memset(&s_stSysSrvInfo[0][0], 0x00, NUM_OF_CM*NUM_OF_SRVTYPE*sizeof(SysSrvInfo_t));
    memset(&s_stCtrlSrvInfo[0], 0x00, MAX_TASK_NUM*sizeof(SysSrvInfo_t));
    memset(&stSysActiveStatus, 0x00, sizeof(SysJoinReqStatus_t));
...
{
    if(pstSyncSrvQueue->usHead < SYNC_SRV_TASKS_NUM)
    {
...
uint32_t SysSyncPrjQueueExist(sync_task_id_t emTaskId)
{
    ...
    {
        uiIndex = (pstSyncPrjQueue->usHead + uiCount) % SYNC_PRJ_TASKS_NUM;
...
{
    if((pstSyncPrjQueue->usHead < SYNC_PRJ_TASKS_NUM) && (pstSyncPrjQueue->uiTaskNum <= SYNC_PRJ_TASKS_NUM) &&
       (pstSyncPrjQueue->uiTaskNum != 0))
    {
    ...
    {
        pstSyncTask->emTaskId = SYS_TASK_ID_NULL;
    }
...
void SysEnSyncPrjQueue(SysSyncPrjQueue_t* pstSyncPrjQueue, SysSyncTask_t* pstSyncTask)
{
    ...
    pstPMState = SysGetPMStateInfo(pstSyncTask->emPMId);
    
    if(SYS_QUEUE_NOFULL == SysSyncPrjQueueFull(pstSyncPrjQueue))
    {
        if(pstPMState != NULL)
        {
            if(pstPMState-> emPoweronFlg != PM_POWERON)
            {
                pstSyncPrjQueue->uiTaskNum++;
...
void SysSyncPrjQueueUpdate(sync_task_stat_t emTaskStatus)
{
    ...
    if(SYS_QUEUE_NULL != pstSyncPrjQueue->uiTaskNum)
    {
        if(pstSyncPrjQueue->usHead < SYNC_PRJ_TASKS_NUM)
        {
...
long64_t SysDiffSyncTime(const SyncTime64_t* punSyncOldTime, const SyncTime64_t* punSyncCurTime)
{
    ...
    llRet = punSyncCurTime->ullTime - punSyncOldTime->ullTime;
...
timer_status_t SysGetTimerStatus(PMController_t emPMId)
{
    ...
    uiTimerStatus = SysGetMainBackupState();
...
{
    SysSrvInfo_t* pstRetSysSrvInfo = NULL;
    
    if((emCMID < NUM_OF_CM) && (emSrvType < NUM_OF_SRVTYPE))
    {
        pstRetSysSrvInfo = (SysSrvInfo_t* )&s_stSysSrvInfo[emCMID][emSrvType];
    }
...
SysSrvInfo_t* SysGetCtrlSrvInfo(Task_ID_t emTaskID)
{
    ...
    {
        pstRetSysSrvInfo = (SysSrvInfo_t* )&s_stCtrlSrvInfo[emTaskID];
    }
...
{
    return (SysJoinReqStatus_t*)&stSysActiveStatus;
}
