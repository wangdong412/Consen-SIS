...
#define CLIENT_PM_TIMEOUT_CYCLE_CNT  (10*1000*1000/(MAIN_TASK_PERIOD))   /* PM应答Client命令的超时拍数 */
#define CLIENT_SW_TIMEOUT_CYCLE_CNT  (120*1000*1000/(MAIN_TASK_PERIOD))  /* Client软件超时拍数 */
...
#define CLI_READ_FWUP_STS   (2513)  /* 读固件升级状态服务 */
...
/* Diag Info. Header  诊断信息头 */
typedef struct DiagInfoHeadTag
{
    uint8_t ucPMID;       /* PMID  */
    uint8_t ucSecCnt;     /* Section Count */
    uint16_t usICStatus;  /* Inter comm status */
}DiagInfoHead_t;
