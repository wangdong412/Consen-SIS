...
/* Modbus master port polling interval */
static uint32_t s_uiMstrPortCnt;
...
/* Make PM Frame */
static void MstrMakePMMBMsg( MstrMakeMsgInfo_t const *pstMakeInfo, uint8_t const pucMBData[], uint16_t usMBDataLen,
...
static void HandleModbusSerlResp( PortParaInfo_t const *pstPortPara, ModbusMasterBlock_t const *pstMBMstrBlk,
...
static void DecodeModbusSerlReq( uint8_t ucSlaveID, PortParaInfo_t const *pstPortPara,
...
static void HandleConfiguration( void )
{
    ...
    if( HandleModbusSlaveCfgInfo( &s_stMBSlaveCfgInfo, &s_uiMBSlaveCfgOffset ))
    {
        ...
        LogWrite( (uint32_t)(LOGID_MB_CFG_SWITCH_MBSLAVE));
    }
    ...
    if( HandleModbusMstrInfo( &s_stMBMstrCfgInfo, &s_uiMBMstrCfgOffset, s_ucLocalCMSlotID ))
    {
        ...
        LogWrite( (uint32_t)(LOGID_MB_CFG_SWITCH_MBMSTR));
        ...
        for( ucIndex = 0U; ucIndex < (uint8_t)(MAX_PORT_NUM); ucIndex++ )
        {
            ...
            /* Port Modbus Master Block Count */
            if( GetPortMBMstrBlkCnt( &s_stMBMstrCfgInfo, (uint32_t)ucIndex ) > 0U )
            {
                s_uiMstrPortCnt++;
            }
        }
       ...
       (puiOffset != NULL))
    {
        if( *puiOffset > 0U )
        {
            ...
            if( !GetModbusSlaveInfo( pstMBSlaveInfo, puiOffset, &bComplete ))
            {
                ...
                LogWrite( (uint32_t)(LOGID_MB_E_CFG_GET_MBSLAVE_INFO));
        ...
        if( bComplete )
        {
            if( !CheckCurModbusSlaveInfo( pstMBSlaveInfo ))
            {
                ...
                LogWrite( (uint32_t)(LOGID_MB_E_CFG_MBSLAVE));
            }
            else
            {
                /* Valid */
    ...
    if((emPort >= PORT_COM1) && (emPort <= PORT_NET2))
    {
        ...
        if((emPort >= PORT_COM1) && (emPort <= PORT_COM4))
        {
            ...
            if( bSwitch )
            {
                ...
                LogWrite( (uint32_t)(LOGID_MB_CFG_SWITCH_COM));
        ...
        else
        {
            ...
            if( bSwitch )
            {
                ...
                LogWrite( (uint32_t)(LOGID_MB_CFG_SWITCH_MBTCP));
                           ...
                           ( ACTIVE == stMBTCPCfgInfo.emActive ))
                        {
                                ...
                                ((0U == pstMBTCPInfo->uiCMCfgCRC ) && (0U == pstMBTCPInfo->uiCMCfgVer)))
                            {
                                ...
                                else
                                {
                                    ...
                                    LogWrite( (uint32_t)(LOGID_MB_E_CFG_MBTCP));
                                }
                            }
                        }
                    }
                           ...
                           ( ACTIVE == stCOMCfgInfo.emActive ))
                        {
                                ...
                                ((0U == pstCOMInfo->uiCMCfgCRC ) && (0U == pstCOMInfo->uiCMCfgVer)))
                            {
                                ...
                                else
                                {
                                    ...
                                    LogWrite( (uint32_t)(LOGID_MB_E_CFG_COM));
                                }
                            }
                        }
                    }
       ...
       (usPMDataLen > 0))
    {
        ...
        if( emSelPMID != SELPM_NONE )
        {
            ...
            else
            {
                ...
                LogWrite( (uint32_t)(LOGID_MB_E_SYS_SEND_PM));
            }
        }
        else
        {
            DEBUG_MB( "SendMBDataToPM: CM_BUS Err\n" );
            LogWrite( (uint32_t)(LOGID_MB_E_CMBUS));
        }
    }
...
static uint8_t const* GetModbusVarAddr( PMController_t emSelPM, ModbusVar_t const *pstMBVar )
{
       ...
       (pstMBVar != NULL))
    {
        ...
        else
        {
            ...
            LogWrite( (uint32_t)(LOGID_MB_E_SYS_GET_RTDATA_ADDR));
        }
    }
...
static bool_t GetMBBOOLValue( PMController_t emSelPM, ModbusVar_t const *pstMBVar, bool_t *pbValue )
{
       ...
       (pbValue != NULL))
    {
        ...
        else
        {
            ...
            LogWrite( (uint32_t)(LOGID_MB_E_GET_MBVAR_ADDR));
        }
    }
                ...
                if( pstMBVar->uiModbusIndex >= (uint32_t)usStartAddr )
                {
                       ...
                       (1U == pstMBVar->usVarSize))
                    {
                        ...
                        else
                        {
                            ...
                            LogWrite( (uint32_t)(LOGID_MB_E_GET_MBBOOL_VALUE));
                            usDataLen = 0U;
                            break;
                        }
                    }
    ...
    if( pstBlkHead != NULL )
    {
        switch( ucFuncCode )
        {
            ...
            default:
            {
                ...
                LogWrite( (uint32_t)(LOGID_MB_E_FUNC_CODE));
                usDataLen = 0U;
            }
            break;
        }
                                 ...
                                 uint8_t pucReqData[], uint8_t ucFPOrder )
{
       ...
       (((uint8_t)(FP_ORDER_H) == ucFPOrder) || ((uint8_t)(FP_ORDER_L) == ucFPOrder)))
    {
        ...
        if( SelectRTData( &s_emLastPMID, &bCMBus ))
        {
            ...
            /* Starting Address */
            memcpy( &pucReqData[usIndex], &usStartAddr, sizeof(uint16_t));
            ...
            switch( ucFuncCode )
            {
                case (uint8_t)(FORCE_SINGLE_COIL):
                {
                    if( 1U == pstBlkHead->uiModbusVarNum )
                    {
                        if( GetMBBOOLValue( s_emLastPMID, &(pstMBMstrBlk->stModbusVar[0]), &bCoilValue ))
                        {
                            ...
                            /* Value */
                            memcpy( &pucReqData[usIndex], &usValue, sizeof(uint16_t));
                            usIndex += sizeof(uint16_t);
                        }
                        else
                        {
                ...
                case (uint8_t)(PRESET_SINGLE_REG):
                {
                    if( 1U == pstBlkHead->uiModbusVarNum )
                    {
                        ...
                        if( pucVarAddr != NULL )
                        {
                            ...
                            memcpy( &pucReqData[usIndex], &pucVarAddr[0], sizeof(uint16_t));
                            usIndex += sizeof(uint16_t);
                        }
                        else
                        {
                ...
                case (uint8_t)(FORCE_MULT_COILS):
                {
                    ...
                    else
                    {
                        ...
                        LogWrite( (uint32_t)(LOGID_MB_E_ADD_COILS_REQDATA));
                        usIndex = 0U;
                    }
                }
                break;
                ...
                case (uint8_t)(PRESET_MULT_REGS):
                {
                    ...
                    else
                    {
                        ...
                        LogWrite( (uint32_t)(LOGID_MB_E_ADD_REGS_REQDATA));
                        usIndex = 0U;
                    }
                }
                break;
                             ...
                             uint8_t pucPMMBMsg[], puint16_t pusPMMBMsgLen )
{
    ...
    if((pstMakeInfo != NULL ) &&
       ((pstMakeInfo->emPort >= PORT_COM1) && (pstMakeInfo->emPort <= PORT_NET2)) &&
       (((uint8_t)(FP_ORDER_H) == pstMakeInfo->ucFPOrder) || ((uint8_t)(FP_ORDER_L) == pstMakeInfo->ucFPOrder)) &&
       (pstMakeInfo->pstRTDataVer != NULL) &&
       (pstMakeInfo->pstMBMstrBlk != NULL) &&
       (pstMakeInfo->ucBlkID < (uint8_t)(MAX_MODBUS_BLOCK_NUM)) &&
       (pucMBData != NULL) &&
       (pucPMMBMsg != NULL) &&
       (pusPMMBMsgLen != NULL))
    {
        /* Maximum Data Length of the Modbus TCP/RTU Message */
        if((pstMakeInfo->emPort >= PORT_COM1) && (pstMakeInfo->emPort <= PORT_COM4))
        {
        ...
        {
            pstHead = &( pstMakeInfo->pstMBMstrBlk->stModbusMasterBlockHead );
            ...
            *pusPMMBMsgLen = 0U;             /* Reset PM Data Length */
                ...
                case (uint8_t)(MR_HOLD_REGS):
                {
                    ...
                    if( usDstBuffLen >= usEstimateLen )
                    {
                        if( pucMBData[ucMBDataIndex] == ucByteCounts )
                        {
                            ...
                            {
                                pstMBVar = &( pstMakeInfo->pstMBMstrBlk->stModbusVar[uiMBVarIndex]);
                                ...
                                {
                                    /* H: 0-3 / L: 2,3,0,1 */
                                    if((uint8_t)(FP_ORDER_H) == pstMakeInfo->ucFPOrder )
                                    {
            ...
            if( bValid )
            {
                *pusPMMBMsgLen = usEstimateLen;
                ...
                pstPortMBHead->ucPortID = (uint8_t)pstMakeInfo->emPort;  /* Port ID */
                pstPortMBHead->uiBlockID = pstMakeInfo->ucBlkID;                      /* Block ID */
                memcpy( &(pstPortMBHead->stRTDataVer), pstMakeInfo->pstRTDataVer, sizeof(RTDataVerInfo_t));
            }
            else
            {
                *pusPMMBMsgLen = 0U;
        ...
        if((usReqDataLen > 2U) && (usReqDataLen <= usMaxReqDataLen))
        {
            ...
            *pusPMMBMsgLen = 0U;             /* Reset PM Data Length */
            ...
            switch( ucFuncCode )
            {
                case (uint8_t)(FORCE_SINGLE_COIL):
                {
                    if( 4U == usReqDataLen )  /* 4 Bytes: Start Address(2 bytes) + Value(2 bytes) */
                    {
                        usDstMBDataLen = 1U;  /* One BOOL Need One Byte */
                        usEstimateLen = (uint16_t)(PMMB_HEADER_LEN) + usDstMBDataLen;

                        if( usDstBuffLen >= usEstimateLen )  /* Check if buffer is long enough */
                        {
                            bValid = true;
                            usAliasNum = 1U;
                ...
                case (uint8_t)(PRESET_SINGLE_REG):
                {
                    if( 4U == usReqDataLen )  /* 4 Bytes: Start Address(2 bytes) + Value(2 bytes) */
                    {
                        ...
                        if( usDstBuffLen >= usEstimateLen )  /* Check if buffer is long enough */
                        {
                            ...
                            /* Add Modbus Data: Big Endian */
                            usPMMsgIndex = (uint16_t)sizeof( PortMBSlaveHead_t );
                            memcpy( &pucPMMBMsg[usPMMsgIndex], &pucReqFunc[usIndex], sizeof(uint16_t));
                            usPMMsgIndex += sizeof(uint16_t);
                            usIndex += sizeof(uint16_t);
                        }
                    }
                }
                ...
                case (uint8_t)(FORCE_MULT_COILS):
                {
                    if( usReqDataLen > 5U )  /* 5 Bytes: Start Address(2) + Regs(2) + Byte Counts(1) + Value */
                    {
                ...
                case (uint8_t)(PRESET_MULT_REGS):
                {
                    if( usReqDataLen > 5U )  /* 5 Bytes: Start Address(2) + Regs(2) + Byte Counts(1) + Value */
                    {
                                    ...
                                    while( ucMBDataIndex < ucByteCounts )
                                    {
                                        ...
                                        if( usVarRgnIndex < (uint16_t)(MAX_MODBUS_RGN_VAR_NUM))
                                        {
                                            ...
                                            if((uint8_t)(MODBUS_VAR_ACTIVE) == pstMBSlaveVar->ucActive )
                                            {
                                                ...
                                                if( 2U == pstMBVar->usVarSize )
                                                {
                                                    memcpy( &pucPMMBMsg[usPMMsgIndex], &pucReqFunc[usIndex], \
                                                            sizeof(uint16_t));
                                                    usPMMsgIndex += sizeof(uint16_t);
                                                    usIndex += sizeof(uint16_t);
                                                    ucMBDataIndex += 2U;
                        ...
                        {
                            DEBUG_MB( "Regs: RegNums %d, ByteCnts %d != %d\n", usNum, ucByteCounts, usReqDataLen-5U );
                        }
                    }
                }
                break;
                    ...
                    if( bBlkStart )
                    {
                                              ...
                                              ucFPOrder ))
                        {
                            if( pstPortMstrInfo->stMBReqMsg.usLen > 0U )
                            {
                                ...
                                else
                                {
                                    ...
                                    //LogWrite( (uint32_t)(LOGID_MB_E_SEND_MBTCP_MSG));
                        ...
                        else
                        {
                            DEBUG_MB( "MakeMBTCPReq Failed: Net-%d, Block-%d\n", \
                                       ucNetIndex+1, pstPortMstrInfo->uiBlockIndex );
                            LogWrite( (uint32_t)(LOGID_MB_E_MAKE_MBTCPREQ));
                            pstPortMstrInfo->uiBlockIndex = 0U;
                        }
                ...
                else
                {
                    if( s_stMBMstrCfgInfo.bValid )
                    {
                           ...
                           (ucBlkID < (uint8_t)(MAX_MODBUS_BLOCK_NUM)))
                        {
                            ...
                            pstPortMstrInfo->uiWaitLoopCycleCnt = (uint32_t)(LOOPBLK_INVL_CYCLE_CNTS)*s_uiMstrPortCnt;
                        }
                        else
                        {
               ...
               ( usDataLen <= (uint16_t)(MAX_MBTCP_DATA_LEN)))
            {
                ...
                if((uint8_t)(VM_READ) == pstBlkHead->ucVisitMode )
                {
                    ...
                    else
                    {
                        ...
                        LogWrite( (uint32_t)(LOGID_MB_E_ADD_READREQDATA));
                        bResult = false;
                    }
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        LogWrite( (uint32_t)(LOGID_MB_E_ADD_WRITEREQDATA));
                        bResult = false;
                    }
                }
        ...
        if( usRespLen == pstPortMstrInfo->usEstimateRespLen )
        {
            ...
            if( bValid )
            {
                ...
                if((uint8_t)(VM_READ) == pstHead->ucVisitMode )
                {
                    ...
                    stMakeInfo.emPort = emPort;
                    stMakeInfo.ucFPOrder = ucFPOrder;
                    stMakeInfo.ucBlkID = ucBlkID;
                    stMakeInfo.pstMBMstrBlk = pstMBMstrBlk;
                    stMakeInfo.pstRTDataVer = &(s_stMBMstrCfgInfo.stRTDataVer);
                    MstrMakePMMBMsg( &stMakeInfo, &pucResp[usIndex], usRespLen - usNonDataLen, \
       ...
       (s_stMBSlaveCfgInfo.bValid))
    {
        while( uiCheckCnt < (uint32_t)(MAX_MBTCP_MSTR_CNT))
        {
            ...
            if( !SysGetModbusTCPMasterMsg( s_ucMBTCPMstrIndex, (puint8_t)&s_stMBTCPMsg, &usLen ))
            {
                ...
                LogWrite( (uint32_t)(LOGID_MB_E_SYS_GETMBTCP_MSTR_MSG));
            }
        ...
        if( !CheckReqFuncCode( ucFuncCode ))
        {
            ...
            *pusRespLen = MakeMBTCPExRespMsg( pucResp, ucFuncCode, emExcCode );
            if( 0U == *pusRespLen )
            {
                ...
                LogWrite( (uint32_t)(LOGID_MB_E_MAKE_EXCEPT_RESP));
            }
        }
        else
        {
            ...
            if( usLenArea == (usReqLen - 6U))
            {
                ...
                *pusRespLen = MakeMBTCPRespMsg( ucFPOrder, pucReq, pucResp );
                ...
                else
                {
                    ...
                    LogWrite( (uint32_t)(LOGID_MB_E_MAKE_MBTCP_RESP));
                }
            }
            else
            {
                DEBUG_MB( "Net[%d] MBTCPReq: LenArea-%d != %d\n", ucNetIndex+1, usLenArea, usReqLen - 6U );
                LogWrite( (uint32_t)(LOGID_MB_E_MBTCP_REQ_LEN));
            }
        }
    }
    else
       ...
       (usStartAddr <= (uint16_t)(MAX_MBRGN_INDEX)))
    {
        if( ACTIVE == pstSlaveBlk->emActive )
        {
            ...
            else
            {
                ...
                LogWrite( (uint32_t)(LOGID_MB_E_SEL_RTDATA));
            }
        }
    }
    ...
    for( ucCOMIndex = 0U; ucCOMIndex < NUM_OF_SERIAL_PORT; ucCOMIndex++ )
    {
        ...
        if( pstCOMCfg != NULL )
        {
               ...
               (pstCOMCfg->ucProtocol != (uint8_t)(MODBUS_MASTER_ASCII)))
            {
                ...
                /* Clear ucModbusOnlineCnt */
                ucModbusOnlineCnt[(CommPort_t)uiPortIndex] = 0;
                SysSetCM2PMModbusState((CommPort_t)uiPortIndex, 0);
            }
       ...
       ((usMsgLen > (uint16_t)(MBRTU_MSG_NONDATA_LEN)) && (usMsgLen <= (uint16_t)(MAX_MBRTU_MSG_LEN))))
    {
        ...
        if( bValid )
        {
            ...
            if( usCRC != ((pucMsg[usMsgLen-1] << 8U) | pucMsg[usMsgLen-2U]))
            {
                ...
                LogWrite( (uint32_t)(LOGID_MB_E_MBRTU_CRC));
            }
        }
    }
       ...
       ((usASCIIMsgLen > usNonDataLen) && (usASCIIMsgLen <= usMaxMsgLen)))
    {
        ...
        if( bValid )
        {
            ...
            if( ucLRC != ( pucASCIIMsg[usASCIIMsgLen - 1U]))
            {
                ...
                LogWrite( (uint32_t)(LOGID_MB_E_MBASCII_LRC));
            }
        }
    }
       ...
       (pusLen != NULL))
    {
        ...
        else
        {
            *pusLen = 0U;
            ...
            LogWrite( (uint32_t)(LOGID_MB_E_SERIAL_RECV));
        }
    }
...
static bool_t SendModbusSerlMsg( CommPort_t emPort, uint8_t const pucData[], uint16_t usDataLen )
{
        ...
        (usDataLen > 0))
    {
        ...
        else
        {
            ...
            LogWrite( (uint32_t)(LOGID_MB_E_SERIAL_SEND));
        }
    }
                    ...
                    if( bBlkStart )
                    {
                                               ...
                                               pstMBMstrBlk, &s_stMBSerlMsg ))
                        {
                            if( s_stMBSerlMsg.usLen > 0U )
                            {
                                ...
                                else
                                {
                                    ...
                                    //LogWrite( (uint32_t)(LOGID_MB_E_SEND_MBSERL_MSG));
                        ...
                        else
                        {
                            DEBUG_MB( "MakeMBSerlReq Failed: COM-%d, Blk-%d\n", \
                                       ucCOMIndex+1, pstPortMstrInfo->uiBlockIndex );
                            LogWrite( (uint32_t)(LOGID_MB_E_MAKE_MBSERL_REQ));
                            pstPortMstrInfo->uiBlockIndex = 0U;
                        }
            ...
            else
            {
                if( s_stMBMstrCfgInfo.bValid )
                {
                    ...
                    if((pstMBMstrBlk != NULL) &&
                       (ucBlkID < (uint8_t)(MAX_MODBUS_BLOCK_NUM)))
                    {
                        stPortPara.bRTUProt = bRTUProt;
                        stPortPara.emPort = emPort;
                        HandleModbusSerlResp( &stPortPara, pstMBMstrBlk, ucBlkID, pstPortMstrInfo, \
                                              s_ucRespBuff, usLen );
        ...
        else
        {
            ...
            if( pstPortMstrInfo->uiWaitRespCycleCnt > (uint32_t)(MBSERL_RESP_TO_CYCLE_CNT))
            {
                ...
                DEBUG_MB( "MBSerl Slave Timeout: COM-%d, Blk-%d\n", \
                           ucCOMIndex+1, pstPortMstrInfo->uiBlockIndex );
                //LogWrite( (uint32_t)(LOGID_MB_E_MBSERL_SLAVE_TIMEOUT));
                HandleModbusSlaveTimeout( emPort, pstPortMstrInfo, &s_stMBMstrCfgInfo );
            }
        }
    }
       ...
       (pstMBReqMsg != NULL))
    {
        ...
        if( ucFuncCode != (uint8_t)(INVALID_MB_FUNCCODE))
        {
            ...
            else
            {
                ...
                LogWrite( (uint32_t)(LOGID_MB_E_MBSERL_REQ_LEN));
            }
        }
        else
        {
...
*
* 输入参数: bRTUProt：RTU协议标志；
*          pstPortPara：端口参数，包括bRTUProt(RTU协议标志)和emPort(端口号)；
                                  ...
                                  uint8_t const pucResp[], uint16_t usRespLen )
{
    ...
    if((pstPortPara != NULL) &&
       ((pstPortPara->emPort >= PORT_COM1) && (pstPortPara->emPort <= PORT_COM4)) &&
       (pstMBMstrBlk != NULL) &&
       (ucBlkID < (uint8_t)(MAX_MODBUS_BLOCK_NUM)) &&
       (pstPortMstrInfo != NULL) &&
       (pucResp != NULL))
    {
        ...
        {
            /* 当作为主站收到有效的modbus数据后，标志加1 */
            ucModbusOnlineCnt[pstPortPara->emPort] = ucModbusOnlineCnt[pstPortPara->emPort] + 1;
            ...
            if( usRespLen == pstPortMstrInfo->usEstimateRespLen )
            {
                ...
                bValid = CheckMBSerlResp( pstPortPara->bRTUProt, pstPortMstrInfo->stMBReqMsg.ucBuff, \
                                          pucResp, usRespLen );
                if( bValid )
                {
                    if((uint8_t)(VM_READ) == pstHead->ucVisitMode )
                    {
                        /* Port FP Order */
                        ucFPOrder = GetPortFPOrder( pstPortPara->emPort );
                        ...
                        /* Make PM Data Frame */
                        usPMDataLen = (uint16_t)sizeof( s_ucPMDataBuff );
                        stMakeInfo.emPort = pstPortPara->emPort;
                        stMakeInfo.ucFPOrder = ucFPOrder;
                        stMakeInfo.ucBlkID = ucBlkID;
                        stMakeInfo.pstMBMstrBlk = pstMBMstrBlk;
                        stMakeInfo.pstRTDataVer = &(s_stMBMstrCfgInfo.stRTDataVer);
                        ...
                        if( usPMDataLen > 0U )
                        {
                            ...
                            printf( "COM[%d] MBMaster Message To PM: Length: %d, Content: \n", \
                                    pstPortPara->emPort-PORT_COM1+1, usPMDataLen );
                ...
                {
                    DEBUG_MB( "COM[%d] Resp Data Invalid\n", pstPortPara->emPort-PORT_COM1+1 );
                    //LogWrite( (uint32_t)(LOGID_MB_E_MBSERL_RESP));
                }
            }
            else
            {
                DEBUG_MB( "COM[%d] RespLen-%d != %d\n", \
                          pstPortPara->emPort-PORT_COM1+1, usRespLen, pstPortMstrInfo->usEstimateRespLen );
                LogWrite( (uint32_t)(LOGID_MB_E_MBSERL_RESP_LEN));
            }
        }/* end of if - Valid */
        else
        {
           ...
           (ucSlaveID <= (uint8_t)(MAX_MODBUS_SLAVE_ID)))
        {
               ...
               (s_stMBSlaveCfgInfo.bValid))
            {
                ...
                if( usReqLen > 0U )
                {
                    ...
                    stPortPara.emPort = emPort;
                    usRespLen = (uint16_t)sizeof( s_ucRespBuff );
                    DecodeModbusSerlReq( ucSlaveID, &stPortPara, s_ucDataBuff, usReqLen, s_ucRespBuff, &usRespLen );
                    if( usRespLen > 0U )
                    {
#ifdef DEBUG_MODBUS
...
*          pstPortPara：端口信息，包括bRTUProt 和 emPort；
*          pucReq：请求帧首地址；
*          usReqLen：请求帧长度；
*          pucResp：应答帧首地址；
*
                                 ...
                                 uint8_t pucResp[], puint16_t pusRespLen )
{
    ...
    if(((ucSlaveID >= (uint8_t)(MIN_MODBUS_SLAVE_ID)) && (ucSlaveID <= (uint8_t)(MAX_MODBUS_SLAVE_ID))) &&
       ...
       (pusRespLen != NULL))
    {
        ucCOMIndex = pstPortPara->emPort - PORT_COM1;
        *pusRespLen = 0U;  /* Reset Response Length */
        ...
        if( bValid )
        {
            ...
            if( !CheckReqFuncCode( ucFuncCode ))
            {
                DEBUG_MB( "COM[%d]: Not Support ReqFuncCode-0x%02X\n", ucCOMIndex+1, ucFuncCode );
                LogWrite( (uint32_t)(LOGID_MB_E_FUNC_CODE));
                emExcCode = MBEX_ILLEGAL_FUNC;
            ...
            else
            {
                ...
                if((usRespDataLen > 0U) && (usRespDataLen <= (uint16_t)(MAX_MBRTU_DATA_LEN)))
                {
                    ...
                    {
                        /* Port Write Enable */
                        if( GetPortWriteEnable( pstPortPara->emPort ))
                        {
                            if( pstPortPara->bRTUProt )
                            {
                                /* RTU:          Slave ID(1) + Function Code(1) + Data + CRC(2) */
                            ...
                            /* Make PM Data Frame */
                            usPMDataLen = (uint16_t)sizeof( s_ucPMDataBuff );
                            SlaveMakePMMBMsg( pstPortPara->emPort, ucFPOrder, &pucReq[MBRTU_FUNCCODE_INDEX], \
                                              usReqDataLen, s_ucPMDataBuff, &usPMDataLen );
                            if( usPMDataLen > 0U )
                            {
#ifdef DEBUG_MODBUS
