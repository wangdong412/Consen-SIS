...
static void SetAdjustTime(void);
...
static void HandleCOMTimingRegs( void )
{
    ...
    /* Pulse Source & Pulse Polarity */
    pstTSyncCfg = GetCurTSyncCfg( &s_stTimeSyncCfgInfo );
    if( pstTSyncCfg != NULL )
    {
        stPPMSets.ucPulseSrc = pstTSyncCfg->ucPulseSrc;
...
static void SetAdjustTime(void)
{
    ...
    uiSysTime = GetTimingPulse();

    if(((uiSysTime % (60*1000000)) - (30*1000000)) < MAIN_TASK_PERIOD)
    {
//        printf("SysTime1:%d\n",uiSysTime);
        SetTimingPulse();
        uiSysTime = GetTimingPulse();
