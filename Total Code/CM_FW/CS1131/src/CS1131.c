...
static void Decode1131Timing( CS1131Info_t *pstSWInfo, uint8_t const pucReq[], RespMsg_t *pstResp );

/* Check timing data */
static bool_t Check1131TimingData( TimingInfo_t const *pstTimeInfo );
...
void CS1131Cycle( void )
{
    ...
    if( RecvOprRetMsg( s_stFUPOprInfo.iMsgQID, MSGT_SRCUP_OPR_RET, NULL ))
    {
...
static bool_t CheckPMHandle( uint8_t const pucReq[])
{
    ...
    if( pucReq != NULL )
    {
        ...
        if(((uint16_t)(CS1131_LOGIN) == usAType) ||
           ...
           ((uint16_t)(CS1131_TIMING) == usAType))
        {
            bPMHandle = false;
        }
...
static uint16_t AddStatusData( StsReqType_t emType, uint8_t pucRespData[])
{
    ...
    if( pucRespData != NULL )
    {
        ...
        /* CM Config flag: 0-no cfg; 1-has cfg */
        if( usIndex > 0U )
        {
            if( s_bCMHasCfg )
            {
                pucRespData[usIndex] = 1;
            }
            else
            {
                pucRespData[usIndex] = 0;
            }
...
static void DecodeCmd( CS1131Info_t *pstSWInfo, uint8_t const pucReq[], RespMsg_t *pstResp, bool_t bCMHasCfg )
{
       ...
       (pstResp != NULL))
    {
        ...
        switch( usAType )
        {
            ...
            case (uint16_t)(CS1131_TIMING):
            {
                Decode1131Timing( pstSWInfo, pucReq, pstResp );
            }
            break;
...
static void Decode1131Timing( CS1131Info_t *pstSWInfo, uint8_t const pucReq[], RespMsg_t *pstResp )
{
    ...
    TimingInfo_t const *pstTimeInfo = NULL;
    ...
    {
        usAType = GetReqActualType( pucReq );
        if((uint16_t)(CS1131_TIMING) == usAType )
        {
            /* LOGIN */
            if( LOGIN == pstSWInfo->emLog )
            {
                pstTimeInfo = (TimingInfo_t const *)&pucReq[APP_MSG_HEADER_LEN];

                /* Check Valid */
                if( Check1131TimingData( pstTimeInfo ))
                {
                    ...
                    stTime.tm_year = (long)(pstTimeInfo->usYear - 1900U);

                    /* int tm_mon: the number of months since January, in the range 0 to 11 */
                    /* pstTimeInfo->ucMonth: 1-12 */
                    stTime.tm_mon = (int32_t)(pstTimeInfo->ucMonth - 1U);

                    /* int tm_mday: the day of the month, in the range 1 to 31 */
                    /* pstTimeInfo->ucDay: 1-31 */
                    stTime.tm_mday = (int32_t)pstTimeInfo->ucDay;

                    /* int tm_hour: the number of hours past midnight, in the range 0 to 23 */
                    /* pstTimeInfo->ucHour: 0-23 */
                    stTime.tm_hour = (int32_t)pstTimeInfo->ucHour;

                    /* int tm_min: the number of minutes after the hour, in the range 0 to 59*/
                    /* pstTimeInfo->ucMinute: 0-59 */
                    stTime.tm_min = (int32_t)pstTimeInfo->ucMinute;

                    /* int tm_sec: the number of seconds after the minute, normally in the range
                     * 0 to 59, but can be up to 60 to allow for leap seconds */
                    /* pstTimeInfo->ucSecond: 0-59, 60(leap seconds) */
                    stTime.tm_sec = (int32_t)pstTimeInfo->ucSecond;

                    stTimeval.tv_sec = (long)mktime( &stTime );  /* s */
                    stTimeval.tv_usec = (long)(pstTimeInfo->uiMilliSec*1000 + pstTimeInfo->microsec);

                    DEBUG_CS1131( "Timing: %d.%d.%d %d:%d:%d %d %d\n", pstTimeInfo->usYear, pstTimeInfo->ucMonth, \
                                  pstTimeInfo->ucDay, pstTimeInfo->ucHour, pstTimeInfo->ucMinute, \
                                  pstTimeInfo->ucSecond, pstTimeInfo->uiMilliSec, pstTimeInfo->microsec );

                    /* Set Linux System Time */
                    if( -1 == settimeofday( &stTimeval, NULL ))
                    {
                        DEBUG_CS1131( "settimeofday E-%d\n", errno );
                    }
                    else
                    {
                        DEBUG_CS1131( "settimeofday: S-0x%08X, US-0x%08X\n", \
                                      (uint32_t)(stTimeval.tv_sec), (uint32_t)(stTimeval.tv_usec));
                    }
                }
                else
                {
                    DEBUG_CS1131( "1131 timing data invalid\n" );
                    ucSubReason = (uint8_t)(ERR_TIMING_DATA);
                }
            }
            else
            {
...
static bool_t Check1131TimingData( TimingInfo_t const *pstTimeInfo )
{
    ...
    if( pstTimeInfo != NULL )
    {
        if((pstTimeInfo->usYear >= 1970U) &&
           ((pstTimeInfo->ucMonth >= 1U ) && (pstTimeInfo->ucMonth <= 12U)) &&
           ((pstTimeInfo->ucDay >= 1U ) && (pstTimeInfo->ucDay <= 31U)) &&
           (pstTimeInfo->ucHour <= 23U) &&
           (pstTimeInfo->ucMinute <= 59U) &&
           (pstTimeInfo->ucSecond <= 60U))  /* Leap second: 60 */
        {
            bValid = true;
        }
    }
        ...
        if( PM_RESP == s_emResp )
        {
            ...
            if( 0U == s_stRespMsg.usLen )
            {
                ...
                if( s_uiWaitPMCycleCnt >= (uint32_t)(CS1131_PM_TIMEOUT_CYCLE_CNT))
                {
                    ...
                    /* Clear Req */
                    if( !SysClearMsg( SwitchPMID(pstSWInfo->emSelPMID), CS1131_REQ ))
                    {
                        //DEBUG_CS1131( "PMHandle: SysClearMsg Failed, SelPMID-%d\n", emSelPMID );
                    }
