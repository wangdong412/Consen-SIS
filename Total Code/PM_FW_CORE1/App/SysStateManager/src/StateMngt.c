...
static Key_Switch_Type_t SysSwitchKeyTriVote(Key_Switch_Type_t emKS1,Key_Switch_Type_t emKS2,Key_Switch_Type_t emKS3,Key_Switch_Type_t emOldKS);

static Key_Switch_Type_t SysSwitchKeyDoulVote(Key_Switch_Type_t emKS1,Key_Switch_Type_t emKS2,Key_Switch_Type_t emOldKS);
...
void SysStateMngt(void)
{
    ...
    Task_ID_t emTaskID = MAX_TASK_NUM;
...
//    printf("  SysStateMngt  \n");
    ...
    /*log*/
        ...
        LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_CHANGED);
        switch(uiStateCode)
        {
            case S_O_W:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_1);
            }
            break;
            case S_W_O:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_2);
            }
            break;
            case S_W_W:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_3);
            }
            break;
            case W_O_S:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_4);
            }
            break;
            case W_S_O:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_5);
            }
            break;
            case W_S_W:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_6);
            }
            break;
            case W_W_S:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_7);
            }
            break;
            case S_S_W:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_8);
            }
            break;
            case S_W_S:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_9);
            }
            break;
            case W_S_S:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_10);
            }
            break;
            case S_O_O:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_11);
            }
            break;
            case S_O_S:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_12);
            }
            break;
            case S_S_O:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_13);
            }
            break;
            case S_S_S:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_14);
            }
            break;
            case W_O_O:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_15);
            }
            break;
            case W_O_W:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_16);
            }
            break;
            case W_W_O:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_17);
            }
            break;
            case W_W_W:
            {
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_STAT_18);
            }
            break;
            default:
            {
                LogWrite(uiStateCode);
            }
            break;
        }
...
//                DEBUG_STATMNGT("State %x\n",uiStateCode);
    ...
    {/* 若系统状态处于稳定状态 */
        ...
        if(SYS_STATE_CHANGED == SysStateChange(&s_uiOldState[PMA], &uiState[PMA]))
        {
            ...
            {
                if(PF_PAUSE == SharedGetPauseFlag(emTaskID))
                {
...
void SysUpdateState(void)
{
    ...
    static uint16_t s_usOldState = 0U;
    PMController_t emLocalPMId = SysGetLocalPMID();                                     /* 本PMID号           */
    PMController_t emNextPMId = SysGetNextPMID(emLocalPMId);                            /* 下一个PMID号     */
    PMController_t emPrePMId = SysGetPrePMID(emLocalPMId);                              /* 前一个PMID号     */
    ...
    Key_Switch_Type_t emKeySwitch[NUM_OF_PM];
    Key_Switch_Type_t emKeySwitchVote;
    static download_flag_t s_emLastDownloadFlg;
    static uint32_t s_uiDownloadFlgCount = 0U;
    static uint32_t s_uiSyncStatusCount = 0U;
    static uint8_t s_ucKeyPosDiffCount = 0U;
...
//    SysSetSWRuninOK(SW_RUNING_OK_VAR);
    ...
    if((pstLocalPMState != NULL) && (pstNextPMState != NULL) && (pstPrePMState != NULL))
    {
        if(pstCtrlStationInfo != NULL)
        {
            if(HAS_BE_CONFIGURED == SysGetIsConfigured())
            {
                ...
                {/*当下装配置生效，将下装配置信息清空*/
                    ...
                    pstLocalPMState->stDLPrjInfo.ucIsConfirmed = NOT_HAS_BE_CONFIRMED;
                    ...
                    pstNextPMState->stDLPrjInfo.ucIsConfirmed = NOT_HAS_BE_CONFIRMED;
                    ...
                    pstPrePMState->stDLPrjInfo.ucIsConfirmed = NOT_HAS_BE_CONFIRMED;
        ...
        else
        {
            ...
            s_uiSyncStatusCount = 0;
        }
        if(s_uiSyncStatusCount > ((3*60*1000*10)/TASK_PERIOD))
        {
            pstLocalPMState->emSyncStatus = SS_NOSYNC;
            /* 清空工程同步队列 */
            ...
            s_uiSyncStatusCount = 0;
            LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_SYNC_TO);
        }
        
        
        if((s_emLastDownloadFlg == SharedGetDownloadFlg()) && (s_emLastDownloadFlg != NOT_DOWNLOAD))
        {
            s_uiDownloadFlgCount++;
        }
        else
        {
            s_uiDownloadFlgCount = 0;
        }
        if(s_uiDownloadFlgCount > ((4*60*1000*10)/TASK_PERIOD))
        {
            ...
            s_uiDownloadFlgCount = 0;
            LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_DOWNLOAD_TO);
        }
        
        pstLocalPMState->emDownloadFlg = SharedGetDownloadFlg();
        s_emLastDownloadFlg = SharedGetDownloadFlg();
        pstLocalPMState->emSwitchKeys = SharedGetSwitchPosTmp();
    ...
    if(pstAppend != NULL)
    {
        ...
        {
//            pstAppend->stPMState.stPMFirmwareState.ucNextPMBUSState = 0xFF;

            pstAppend->stPMState.stPMFirmwareState.ucNextPMBUSState = 
                    pstAppend->stPMState.stPMFirmwareState.ucNextPMBUSState | 0x01;
            if(SysGetWorkStatus(emNextPMId) != WS_OFFLINE)
            {
                /*log,PM离线*/
                LogWrite(LOG1_ID_USER_L0_NEXT_PM_OFFLINE);
            ...
            if(pstNextPMState != NULL)
            {
                ...
                pstNextPMState->emSwitchKeys = KS_INVALID_TYPE;
            }
        ...
        {
//            pstAppend->stPMState.stPMFirmwareState.ucNextPMBUSState = 0;

            pstAppend->stPMState.stPMFirmwareState.ucNextPMBUSState = 
                    pstAppend->stPMState.stPMFirmwareState.ucNextPMBUSState & 0xfe;

            if(!(s_usOldState & BITMASK(0)))
            {
                /*log,PM在线*/
                LogWrite(LOG1_ID_USER_L0_NEXT_PM_ONLINE);
            }
        ...
        {
//            pstAppend->stPMState.stPMFirmwareState.ucPrePMBUSState = 0xFF;
            
            pstAppend->stPMState.stPMFirmwareState.ucPrePMBUSState = 
                    pstAppend->stPMState.stPMFirmwareState.ucPrePMBUSState | 0x01;
            
            if(SysGetWorkStatus(emPrePMId) != WS_OFFLINE)
            {
                /*log,前一系PM离线*/
                LogWrite(LOG1_ID_USER_L0_PRE_PM_OFFLINE);
            ...
            if(pstPrePMState != NULL)
            {
                ...
                pstPrePMState->emSwitchKeys = KS_INVALID_TYPE;
            }
        ...
        {
//            pstAppend->stPMState.stPMFirmwareState.ucPrePMBUSState = 0;
            
            pstAppend->stPMState.stPMFirmwareState.ucPrePMBUSState = 
                    pstAppend->stPMState.stPMFirmwareState.ucPrePMBUSState & 0xfe;
            
            if(!(s_usOldState & BITMASK(1)))
            {
                /*log,PM在线*/
                LogWrite(LOG1_ID_USER_L0_PRE_PM_ONLINE);
            }
        }
        ...
        else if((0 != (pstAppend->stPMState.stPMFirmwareState.ucPrePMBUSState & 0x03)) &&
                (0 != (pstAppend->stPMState.stPMFirmwareState.ucNextPMBUSState & 0x03)))
        {
            SetLEDState(LED_NAME_PMBUS, LED_OFF);
        }
        else
    ...
    s_usOldState = usState;
    
    /*钥匙开关表决*/
    emKeySwitchVote = SharedGetSwitchPos();
    if((pstLocalPMState != NULL) && (pstNextPMState != NULL) && (pstPrePMState != NULL) && (pstAppend != NULL))
    {
        emKeySwitch[emLocalPMId] = pstLocalPMState->emSwitchKeys;
        emKeySwitch[emPrePMId] = pstPrePMState->emSwitchKeys;
        emKeySwitch[emNextPMId] = pstNextPMState->emSwitchKeys;
        
        if((WS_OFFLINE != SysGetWorkStatus(emNextPMId)) && (WS_OFFLINE != SysGetWorkStatus(emPrePMId)))
        {
            emKeySwitchVote = SysSwitchKeyTriVote(emKeySwitch[emLocalPMId],emKeySwitch[emPrePMId],emKeySwitch[emNextPMId],SharedGetSwitchPos());
            
            if((emKeySwitch[emPrePMId] != emKeySwitch[emLocalPMId]) || (emKeySwitch[emNextPMId] != emKeySwitch[emLocalPMId]) ||
               (emKeySwitch[emPrePMId] != emKeySwitch[emNextPMId]))
            {
//                pstAppend->stPMState.stPMFirmwareState.ucKeyPosFlg = 0x01;
                if(s_ucKeyPosDiffCount <= 5)
                {
                    s_ucKeyPosDiffCount++;
                }
            }
            ...
            {
//                pstAppend->stPMState.stPMFirmwareState.ucKeyPosFlg = 0x00;
                s_ucKeyPosDiffCount = 0;
            }
            
        }
        else if((WS_OFFLINE != SysGetWorkStatus(emNextPMId)) && (WS_OFFLINE == SysGetWorkStatus(emPrePMId)))
        {
            emKeySwitchVote = SysSwitchKeyDoulVote(emKeySwitch[emLocalPMId],emKeySwitch[emNextPMId],SharedGetSwitchPos());
            
            if(emKeySwitch[emNextPMId] != emKeySwitch[emLocalPMId])
            {
//                pstAppend->stPMState.stPMFirmwareState.ucKeyPosFlg = 0x01;
                if(s_ucKeyPosDiffCount <= 5)
                {
...
//                pstAppend->stPMState.stPMFirmwareState.ucKeyPosFlg = 0x00;
        ...
        {
            emKeySwitchVote = SysSwitchKeyDoulVote(emKeySwitch[emLocalPMId],emKeySwitch[emPrePMId],SharedGetSwitchPos());
            
            if(emKeySwitch[emPrePMId] != emKeySwitch[emLocalPMId])
            {
//                pstAppend->stPMState.stPMFirmwareState.ucKeyPosFlg = 0x01;
                if(s_ucKeyPosDiffCount <= 5)
                {
...
//                pstAppend->stPMState.stPMFirmwareState.ucKeyPosFlg = 0x00;
        ...
        {
            if((emKeySwitch[emLocalPMId] & 0xfffffff9) == 0)
            {
                emKeySwitchVote = emKeySwitch[emLocalPMId];
            }
...
//            pstAppend->stPMState.stPMFirmwareState.ucKeyPosFlg = 0x00;
        ...
        if((emKeySwitchVote & 0xfffffff9) == 0)
        {
            SharedSetSwitchPos(emKeySwitchVote);
    //        printf("V K %x %x %x %x\n",emKeySwitchVote,emKeySwitch[emLocalPMId],emKeySwitch[emPrePMId],emKeySwitch[emNextPMId]);
        }
...
void SysStableStateHandle(uint32_t uiStateCode)
{
    ...
    static uint32_t s_uiTestCount = 0U;
    ...
    if((pstLocalPMState != NULL) && (pstNextPMState != NULL) && (pstPrePMState != NULL) && (pstUP1CtrlSrvInfo != NULL) \
       &&(pstSyncSrvQueue != NULL) && (pstSyncPrjQueue != NULL))
    {
        switch(uiStateCode)
        {/* 根据状态码值调用相应处理函数 */
            case S_O_O:
            case W_O_O:
            {
                if((INIT_DOWNLOAD == SharedGetDownloadFlg()) || (ONLINE_DOWNLOAD == SharedGetDownloadFlg()))
                {
                    if((emLocalPMId == SharedGetDownloadPM()) && 
                       ...
                       (HAS_BE_CONFIRMED == pstLocalPMState->stDLPrjInfo.ucIsConfirmed) && 
                       (IPBUS_DEVICE_CONFIG_SLAVE == pstLocalPMState->stDLPrjInfo.ucIsIOConfigured))
                    {/*发送三系同步切换命令*/
                        ucSyncSwitchSrvFlg = 0x01;
                        LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_PRJ_SWITCH);
                    }
                }
            }
            break;
            ...
            case W_O_W:
            {
                ...
                {/*初始化下装时，产生同步*/
                    ...
                    if((HAS_BE_CONFIGURED == pstNextPMState->stDLPrjInfo.ucIsConfigured) && 
                       (HAS_BE_CONFIRMED == pstNextPMState->stDLPrjInfo.ucIsConfirmed) && 
            ...
            case W_W_O:
            {
                ...
                {/*初始化下装时，产生同步*/
                    ...
                    if((HAS_BE_CONFIGURED == pstPrePMState->stDLPrjInfo.ucIsConfigured) && 
                       (HAS_BE_CONFIRMED == pstPrePMState->stDLPrjInfo.ucIsConfirmed) && 
                       (NOT_HAS_BE_CONFIGURED == pstLocalPMState->stDLPrjInfo.ucIsConfigured))
                    {/*接收前一系工程*/
                        SysStateHandle8();
                    }
    ...
    //                        DEBUG_STATMNGT("DeSyncPrj\n");
                    ...
                    {/*发送三系同步切换命令*/
                        ...
                        else
                        {
                            printf("DEV %x %x %x\n",pstLocalPMState->stDLPrjInfo.ucIsIOConfigured,\
                                    pstPrePMState->stDLPrjInfo.ucIsIOConfigured,pstNextPMState->stDLPrjInfo.ucIsIOConfigured);
                            
                            s_uiTestCount++;
                            if(s_uiTestCount >= 200)
                            {
                                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_IPBUS_DEV_CFG_ERR);
                                s_uiTestCount = 0;
                            }
                        }
                    }
        ...
        if(0x01 == ucSyncSwitchSrvFlg)
        {
               ...
               (SRV_NEED_PROCESS != pstUP1CtrlSrvInfo->stSysSrvInfoHead.ucProcessFlag))
            {
                ...
                memset(&pstUP1CtrlSrvInfo->stSysSrvInfoHead, 0x00, sizeof(SysSrvInfoHead_t));
       ...
       (pstUP2CtrlSrvInfo != NULL) && (pstPMStateInfo != NULL))
    {
        ...
        {/*本机工程文件无效，或者工程信息有任何不一致，需要同步工程文件*/
            ...
            {/* 若同步任务队列无同步工程文件任务 */
                ...
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_RECVPRJ);
            ...
            if(SYS_QUEUE_NOEXIST == SysSyncPrjQueueExist(SYS_RECV_REALDATA))
            {
                ...
                {/* 同步工程数据区，从G区*/
                    if(SYS_QUEUE_NOEXIST == SysSyncPrjQueueExist(SYS_RECV_REALDATA))
                    {
                        ...
                        stSyncTask.emUPTaskId = TASK_UP1;
                        SysEnSyncPrjQueue(pstSyncPrjQueue, &stSyncTask);
                        LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_UP1_RECVDATA);
                    }
                }
            }
            else
                ...
                {/*同步到UP2的实时数据完成*/
                    ...
                    {/*若任务状态未恢复，添加同步任务恢复状态*/
                           ...
                           (SRV_NEED_PROCESS != pstUP2CtrlSrvInfo->stSysSrvInfoHead.ucProcessFlag))
                        {
                            ...
                            uiBuffIndex = 0;
                            memset(&stMSGHead, 0x00, sizeof(MSGHead_t));
                            memset(&pstUP2CtrlSrvInfo->stSysSrvInfoHead, 0x00, sizeof(SysSrvInfoHead_t));
                    ...
                    {/*同步到UP1的实时数据完成*/
                        ...
                        {/*UP1数据同步完成并恢复，开始同步UP2数据*/
                            ...
                            {/*UP2暂停之后， 同步UP2的工程数据区，从G区*/
                                ...
                                SysEnSyncPrjQueue(pstSyncPrjQueue, &stSyncTask);
                                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_UP2_RECVDATA);
                            }
                        }
                    }
                }
...
*
* 功能描述: 非稳态处理函数2，处理状态为：S_W_O、S_W_W。
*
* 输入参数: 无
*
* 输出参数: 无
...
*
* 功能描述: 非稳态处理函数3，处理状态为：W_O_S、W_W_S。
*
* 输入参数: 无
*
* 输出参数: 无
    ...
    if((pstLocalPrjInfo != NULL) && (pstNextPrjInfo != NULL) && (pstSyncQueue != NULL))
    {
        ...
        {/*后一系工程文件无效，或者工程信息与本机有任何不一致，需要同步工程文件*/
            ...
            {/* 若同步任务队列无同步工程文件任务 */
                ...
                LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_SENDPRJ);
            }
        }
        else 
        {
            ...
            if(SYS_QUEUE_NOEXIST == SysSyncPrjQueueExist(SYS_SEND_REALDATA))
            {
                ...
                {/* 同步工程数据前，若系统处于暂停状态，则开始同步工程数据区，从G区开始 */
                    if(IEC_END == SysGetIECShedCtrlFlg(TASK_UP1))
                    {
                        ...
                        stSyncTask.emUPTaskId = TASK_UP1;
                        SysEnSyncPrjQueue(pstSyncQueue, &stSyncTask);
                        LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_UP1_SENDDATA);
                ...
                {/*同步到UP1的实时数据完成，开始同步UP2的数据*/
                    ...
                    {/* 同步工程数据前，若系统处于暂停状态，则开始同步工程数据区，从G区开始 */
                        if(IEC_END == SysGetIECShedCtrlFlg(TASK_UP2))
                        {
                            ...
                            SysEnSyncPrjQueue(pstSyncQueue, &stSyncTask);
                            LogWrite(LOG1_ID_APP_SYS_STATE_MNG_LV0_UP2_SENDDATA);
...
static Key_Switch_Type_t SysSwitchKeyTriVote(Key_Switch_Type_t emKS1,Key_Switch_Type_t emKS2,Key_Switch_Type_t emKS3,Key_Switch_Type_t emOldKS)
{
    Key_Switch_Type_t emKeySwitchVote = KS_INVALID_TYPE;
    
    if(((emKS1 & 0xFFFFFFF9) == 0) && ((emKS2 & 0xFFFFFFF9) == 0) && ((emKS3 & 0xFFFFFFF9) == 0))
    {
        if((emKS1 == emKS2))
        {
            emKeySwitchVote = emKS1;
        }
        else if((emKS2 == emKS3))
        {
            emKeySwitchVote = emKS2;
        }
        else if((emKS1 == emKS3))
        {
            emKeySwitchVote = emKS1;
        ...
        {
            emKeySwitchVote = emOldKS;
        }
    }
    else if(((emKS1 & 0xFFFFFFF9) == 0) && ((emKS2 & 0xFFFFFFF9) == 0))
    {
        if((emKS1 == emKS2))
        {
            emKeySwitchVote = emKS1;
    ...
    else if(((emKS1 & 0xFFFFFFF9) == 0) && ((emKS3 & 0xFFFFFFF9) == 0))
    {
        if((emKS1 == emKS3))
        {
            emKeySwitchVote = emKS1;
    ...
    else if(((emKS2 & 0xFFFFFFF9) == 0) && ((emKS3 & 0xFFFFFFF9) == 0))
    {
        if((emKS2 == emKS3))
        {
            emKeySwitchVote = emKS2;
    ...
    else if((emKS1 & 0xFFFFFFF9) == 0)
    {
        emKeySwitchVote = emKS1;
    }
    else if((emKS2 & 0xFFFFFFF9) == 0)
    {
        emKeySwitchVote = emKS2;
    }
    else if((emKS3 & 0xFFFFFFF9) == 0)
    {
        emKeySwitchVote = emKS3;
    }
    else
    {
        emKeySwitchVote = emOldKS;
...
static Key_Switch_Type_t SysSwitchKeyDoulVote(Key_Switch_Type_t emKS1,Key_Switch_Type_t emKS2,Key_Switch_Type_t emOldKS)
{
    ...
    if(((emKS1 & 0xFFFFFFF9) == 0) && ((emKS2 & 0xFFFFFFF9) == 0))
    {
        if((emKS1 == emKS2))
        {
            emKeySwitchVote = emKS1;
