...
static int32_t s_iReqSem;
static int32_t s_iRespSem;
...
void SysFileManagerInit(void)
{
    ...
    s_iReqSem = os_sem_create("COMMReq", 0, &uiErr);
    if(uiErr != OS_ERR_NONE)
    {
       printf("COMMReq create failed.\n");
    }

    s_iRespSem = os_sem_create("FMResp", 0, &uiErr);
    if(uiErr != OS_ERR_NONE)
    {
       printf("FMResp create failed.\n");
    }
...
bool_t SysWriteFile(int32_t iFd, uint8_t ucContent[], uint16_t usLen)
{
    ...
    os_sem_post(s_iReqSem, OS_OPT_POST_1|OS_OPT_POST_NO_SCHED, &uiErr);
    return bRet;
}
...
bool_t SysSendAck(SysFMAck_t *pstAck)
{
    ...
    os_sem_post(s_iRespSem, OS_OPT_POST_1|OS_OPT_POST_NO_SCHED, &uiErr);
...
bool_t SysReadFMReqMsg(SysFMReq_t *pstSysFMReq)
{
    ...
    iPend = os_sem_pend(s_iReqSem, 0, OS_OPT_PEND_NON_BLOCKING, &uiErr);
...
bool_t SysRecvAckHeader(SysFMAckHeader_t *pstAckHeader)
{
    ...
    iPend = os_sem_pend(s_iRespSem, 0, OS_OPT_PEND_NON_BLOCKING, &uiErr);
...
int32_t SysEraseNorflash(int iBlockIndex, int iBlockNum)
{
    ...
    if((iBlockIndex >= 0) && (iBlockNum > 0))
    {
        iIndex = iBlockIndex;
        for(uiCount = 0; uiCount < iBlockNum; uiCount++)
        {
            iResult = lld_erase(iIndex, 1);
...
bool_t SysEraseFirmware(uint32_t uiCwd)
{
    ...
    if((FM_MSGBOX_UNLOCKED == pstReqMsgBox->bLock) && (FM_MSGBOX_EMPTY == pstReqMsgBox->bStatus))
    {
        ...
        /* 控制字 */
        pstReqMsgBox->stReqMsg.uiCtrlWord = uiCwd;
