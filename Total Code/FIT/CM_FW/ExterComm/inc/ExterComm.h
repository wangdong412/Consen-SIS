...
#define CONN_TIMEOT_CYCLE_CNT (80/8U)    /* TCP连接超时拍数：80ms */
#define ETH_CHK_INTL_CYC_CNT  (20000/32) /* Eth Check Interval Cycle Count 每隔20s检查网卡开关状态 */

/* Network Default Value */
#define DEF_IPADDR_NET1 (0xC0A80101U)  /* 网口1 IP地址默认值：192.168.1.1 */
#define DEF_IPADDR_NET2 (0xC0A80201U)  /* 网口2 IP地址默认值：192.168.2.1 */
...
#if 0
    SockInfo_t stNTPSock;       /* NTPTCP Socket         校时套接字，只NET2有效，
                                                                                                                          用于接收校时服务器的校时命令/向从站发送校时命令，端口号123 */
#endif
}NetworkSockInfo_t;
...
#if 0
/* NTP/TCP Info.  NTP/TCP校时配置参数信息结构体定义 */
typedef struct NTPInfoTag
{
    uint8_t ucIndex;                                    /* Index 当前使用的配置参数的索引值 */
    uint8_t ucReserve[3];                               /* Reserve 预留 */
    uint32_t uiCMCfgCRC;                                /* CM配置文件CRC */
    uint32_t uiCMCfgVer;                                /* CM配置文件的版本 */
    NTPTCPConfigInfo_t stNTPConfig[LOCAL_CFG_VER_CNT];  /* NTPTCP Configuration 校时配置参数 */
}NTPInfo_t;
#endif
