...
/* FPGA Update */
static uint8_t s_ucFPGAData[FPGA_FILE_SIZE];
...
/* FPGA Load */
static int32_t GetFPGAPrgIndex( void );
static bool_t LoadFPGAPrg( int32_t iSPIFD, uint32_t uiFPGAIndex, uint8_t ucPrgBuff[], uint32_t uiBuffLen );
...
void LoadFPGA( void )
{
    ...
    /* Get FPGA program index */
    iFPGAIndex = GetFPGAPrgIndex();
    if( iFPGAIndex > 0 )
    {
        /* Open SPI device */
        iSPIFD = open( "/dev/spi_xfer", O_RDWR | O_SYNC );
        if( iSPIFD < 0 )
        {
            DEBUG_RTS( "spi_xfer open fail: %d\n", errno );
        }
        else
        {
            /* Load current version */
            bResult = LoadFPGAPrg( iSPIFD, iFPGAIndex, &s_ucFPGAData[0], sizeof(s_ucFPGAData));
            if( !bResult )
            {
                DEBUG_RTS( "Load current FPGA version failed, Index %d\n", iFPGAIndex );
                ...
                /* Try load older version */
                bResult = LoadFPGAPrg( iSPIFD, iFPGAIndex, &s_ucFPGAData[0], sizeof(s_ucFPGAData));
                if( !bResult )
                {
                    DEBUG_RTS( "Load older FPGA version failed, Index %d\n", iFPGAIndex );
                }
                else
                {
                    DEBUG_RTS( "Load older FPGA version OK, Index %d\n", iFPGAIndex );
                }
            }
            else
            {
                DEBUG_RTS( "Load current FPGA version OK, Index %d\n", iFPGAIndex );
            }
        }
    }
    else
    {
        DEBUG_RTS( "GetFPGAPrgIndex failed\n" );
    }
    ...
    if( !bResult )
    {
        ...
        while(1)
        {
            usleep( 1000 );  /* Wait watchdog reset */
        }
#else
        DEBUG_RTS( "Load FPGA Failed: Wait watchdog reset\n" );
#endif
    }
    else
    {
        ...
        DEBUG_FU( "LoadFPGA: Start: %d %d; Fin: %d %d\n", stPreTime.tv_sec, stPreTime.tv_usec, \
...
{
    int32_t iIndex = -1;
    int32_t iFPGAInfoFD = (int32_t)(INVALID_FD);
    char_t cIndex[4] = { 0 };
    int32_t iLen = 0;

    /* Open "FPGAInfo" file: Read/Write; flush after write */
    iFPGAInfoFD = open( "/home/root/FPGAInfo", O_RDWR|O_SYNC );
    if((int32_t)(INVALID_FD) == iFPGAInfoFD )
    {
        DEBUG_RTS( "Open FPGAInfo file failed, errno: %d!\n", errno );
    }
    else
    {
        /* Read FPGA program index */
        iLen = ReadFile( iFPGAInfoFD, 0U, &cIndex[0], sizeof(cIndex));
        if( iLen >= 0 )
        {
            if( iLen > 0 )
            {
                /* String to int */
                iIndex = (uint32_t)strtoul( &cIndex[0], NULL, 0 );
                DEBUG_RTS( "GetFPGAPrgIndex: FPGAIndex %d\n", iIndex );
            }
            else
            {
                iIndex = 1U;
                DEBUG_RTS( "GetFPGAPrgIndex: FPGAInfo file is empty\n" );
            }
        }
        else
        {
            DEBUG_RTS( "GetFPGAPrgIndex: Read FPGAInfo file failed\n" );
        }
...
*
* 输入参数: iSPIFD：SPI设备FD；
*          uiFPGAIndex：FPGA程序索引值；
*          ucPrgBuff：FPGA程序缓冲区；
*          uiBuffLen：FPGA程序缓冲区长度。
*
* 输出参数: 无
*
* 返 回 值: 是否加载成功
...
static bool_t LoadFPGAPrg( int32_t iSPIFD, uint32_t uiFPGAIndex, uint8_t ucPrgBuff[], uint32_t uiBuffLen )
{
    ...
    int32_t iFPGAFD = -1;
    int32_t iRet = -1;
    char_t cPathName[255] = { 0 };

    if( ucPrgBuff != NULL )
    {
        if( 2U == uiFPGAIndex )
        {
            sprintf( cPathName, "/home/root/FPGA%d", uiFPGAIndex );
        }
        else
        {
            uiFPGAIndex = 1U;
        ...
        iFPGAFD = open( cPathName, O_RDWR|O_SYNC );
        if((int32_t)(INVALID_FD) == iFPGAFD )
        {
            DEBUG_RTS( "Open %s failed, errno: %d!\n", cPathName, errno );
        }
        else
        {
            /* Read FPGA Data */
            iFPGALen = ReadFile( iFPGAFD, 0U, &ucPrgBuff[0], uiBuffLen );
            if( iFPGALen > 0 )
            {
                iRet = sysCycloneFlush( iSPIFD, &s_ucFPGAData[0], iFPGALen );
                if( iRet < 0 )
                {
                    bResult = false;
                    DEBUG_RTS( "sysCycloneFlush failed: %d\n", iRet );
                }
                else
                {
                    bResult = true;
            ...
            {
                DEBUG_RTS( "ReadFPGAData failed, index %d\n", uiFPGAIndex );
            }
        }
    }
