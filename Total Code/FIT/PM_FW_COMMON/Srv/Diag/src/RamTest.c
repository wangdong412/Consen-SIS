...
void RamTestInit(void)
{
    ...
    printf("zone=%d step=%d\n", s_stRamMessage.emCurUsedZone,s_stRamMessage.uiAddrStep);
...
puint32_t RamTestGetCurDiagAddr(const RamDiagInfo_t * pstThis)
{
    ...
    if(NULL != pstThis)
    {
        ...
        {
            if((pstThis->uiCurRamSn * pstThis->uiAddrStep) >= (CFG_DATA_SIZE * 3))
            {
                /*地址连续，根据自检序号计算自检分片起始地址*/
                uiStartAddr = CFG_FOUR_DATABASE + pstThis->uiCurRamSn * pstThis->uiAddrStep - (CFG_DATA_SIZE * 3);
            }
            else if((pstThis->uiCurRamSn * pstThis->uiAddrStep) >= (CFG_DATA_SIZE * 2))
            {
                /*地址连续，根据自检序号计算自检分片起始地址*/
                uiStartAddr = CFG_THI_DATABASE + pstThis->uiCurRamSn * pstThis->uiAddrStep - (CFG_DATA_SIZE * 2);
            }
            else if(pstThis->uiCurRamSn * pstThis->uiAddrStep >= CFG_DATA_SIZE)
            {
                /*地址连续，根据自检序号计算自检分片起始地址*/
                uiStartAddr = CFG_SEC_DATABASE + pstThis->uiCurRamSn * pstThis->uiAddrStep - (CFG_DATA_SIZE);
            }
            else
            {
                /*地址连续，根据自检序号计算自检分片起始地址*/
                uiStartAddr = CFG_FIR_DATABASE + pstThis->uiCurRamSn * pstThis->uiAddrStep;
            }
        ...
        else
        {
            if(pstThis->uiCurRamSn * pstThis->uiAddrStep >= CFG_DATA_SIZE * 3)
            {
                uiStartAddr = CFG_FIR_DATABASE + CFG_DATA_SIZE;
                /*地址连续，根据自检序号计算自检分片起始地址*/
                uiStartAddr = uiStartAddr - (pstThis->uiCurRamSn * pstThis->uiAddrStep - CFG_DATA_SIZE * 3);
            }
            else if(pstThis->uiCurRamSn * pstThis->uiAddrStep >= CFG_DATA_SIZE * 2)
            {
                uiStartAddr = CFG_SEC_DATABASE + CFG_DATA_SIZE;
                /*地址连续，根据自检序号计算自检分片起始地址*/
                uiStartAddr = uiStartAddr - (pstThis->uiCurRamSn * pstThis->uiAddrStep - CFG_DATA_SIZE * 2);
            }
            else if(pstThis->uiCurRamSn * pstThis->uiAddrStep >= CFG_DATA_SIZE)
            {
                uiStartAddr = CFG_THI_DATABASE + CFG_DATA_SIZE;
                /*地址连续，根据自检序号计算自检分片起始地址*/
                uiStartAddr = uiStartAddr - (pstThis->uiCurRamSn * pstThis->uiAddrStep - CFG_DATA_SIZE);
            }
            else
            {
                uiStartAddr = CFG_FOUR_DATABASE + CFG_DATA_SIZE;
                /*地址连续，根据自检序号计算自检分片起始地址*/
