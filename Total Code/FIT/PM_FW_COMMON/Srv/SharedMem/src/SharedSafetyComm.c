...
void SharedSafetyCommInit( void )
{
    ...
    /* PM Safety */
    SharedPMSafetyCommInit();

    /* PM IO Safety */
    SharedPMIOSafetyCommInit();
...
{
    memset( &s_stSysP2PRecvInfo[0], 0, sizeof(SysP2PRecvReqCommInfo_t)*(uint32_t)(MAX_STATION_CNT));
    memset( &s_stSysP2PSendInfo[0], 0, sizeof(SysP2PSendReqCommInfo_t)*(uint32_t)(MAX_STATION_CNT));
}
...
{
    memset( &s_stSysPMRecvInfo[0], 0, sizeof(SysPMRecvInfo_t)*(uint32_t)(NUM_OF_PM));
    memset( &s_stSysPMSendInfo[0], 0, sizeof(SysPMSendInfo_t)*(uint32_t)(NUM_OF_PM));
}
...
void SharedPMIOSafetyCommInit( void )
{
    ...
    memset( &s_stSysPMIOCommInfo[0], 0, sizeof(SysPMIOCommInfo_t)*(uint32_t)(IPBUS_MODULE_MAX_NUM));
}
...
* 功能描述: 设置P2P接收容忍标志位
*
* 输入参数: ucSrcStaIndex：源（数据发送方）控制站索引值: 0-63；
*          bTolFlag：容忍标志位。
*
* 输出参数: 无
*
