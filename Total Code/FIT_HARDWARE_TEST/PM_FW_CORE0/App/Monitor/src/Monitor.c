...
uint32_t uiUP1AverageScanSurplus = 0;
uint32_t uiUP1AverageScanOverrun = 0;
...
uint32_t uiUP2AverageScanSurplus = 0;
uint32_t uiUP2AverageScanOverrun = 0;
        ...
        if( HAS_BE_CONFIGURED == SysGetIsConfigured())
        {
            ...
            if((pstP2PBlock != NULL) && (ACTIVE == pstP2PBlock->emActive))
            {
                if( pstRecvCtrl->bHasRecv )
                {
                    ...
                    if( uiRecvTimeout < (uint32_t)(P2P_TIMEOUT_TIME))
                    {
                    ...
                    {/*输入轮询完成，若需要开始新一轮轮询*/
                        ...
                        {/*任务状态为运行和单步时，进行IEC运算*/
                            ...
                            else
                            {
                                ...
                                if(ALLOW_IEC_RUN == s_ucIECCtrlFlg[ucTaskIndex])
                                {
            ...
            {/*根据服务号，处理Ctrl服务*/
               ...
               case SYS_PRJ_SWITCH:
               {
                   if(INIT_DOWNLOAD == SharedGetDownloadFlg())
                   {
                       ...
                       /* 临时清bUP1DeltaFlag、bUP2DeltaFlag标志 */
                       bUP1DeltaFlag = false;
                       bUP2DeltaFlag = false;
                   ...
                   if(ONLINE_DOWNLOAD == SharedGetDownloadFlg())
                   {
                       ...
                       for(emTaskIndex = TASK_UP1; emTaskIndex < MAX_TASK_NUM; emTaskIndex++)
                       {
                           DataProcessSoftSOEInit(emTaskIndex);
                       }
