...
void CommCycle(void)
{
	...
	if((ucRTDataBackupCount % 100) == 0)//liqi 5ms
	#endif
...
static void CommCS1131Cycle(void)
{
    ...
    static ucSendACKFlg[NUM_OF_CM] = {0,0,0,0};
...
static uint16_t CommOSCGetVar(uint8_t ucCS1131Data[], uint8_t ucACKData[])
{
    ...
    else
    {
        ...
        {
            if((uiStartPos <= pstOSCPCtrl->uiWRIndex) && (uiStartPos < 2*1024))
            {
...
static void CommP2PInit( void )
{
    ...
    memset( &s_stP2PSendCtrl[0], 0, sizeof(P2PSendCtrl_t)*(uint32_t)(MAX_P2P_BLOCK_NUM));
        ...
        {/* 由Core1来处理 */
               ...
               (P2P_RES_FRAME == pstSysSrvInfo->stSysSrvData.usSrvNo))
            {
                ...
                if( pstP2PBlock != NULL )
                {
                    ...
                    if( NOT_WAIT_ACK == s_stP2PSendCtrl[ucBlkIndex].emWaitACKFlag )
                    {
                        bDiscard = true;
                    }
                    ...
                    if( !bDiscard )
                    {
                                           ...
                                           pstRespHead->usTotalLen, &stResDecodeResultInfo))
                        {
                            ...
                            else
                            {
                                ...
                                if( s_stP2PSendCtrl[ucBlkIndex].usWaitACKCount > (uint16_t)uiRecvTOCycleCnt )
                                {/* 等待ACK超时 */
                                    //printf( "PERspTO\n" );
...
static void CommP2PSend( void )
{
        ...
        if( ACTIVE == SharedGetP2PBlockSendActive( ucBlockIndex ))
        {
            ...
            if( GetP2PSendStartFlag( (uint8_t)(pstP2PBlock->stP2PBlockHead.uiBlockID)))
            {
                ...
                {/* 在等待来自对端控制站的ACK */
                    ...
                    if( s_stP2PSendCtrl[ucBlockIndex].usWaitACKCount > (uint16_t)uiRecvTOCycleCnt )
                    {/* 等待ACK超时 */
                        //printf( "PERspTO\n" );
    ...
    if( uiP2PSendBlockNum > 0U )
    {
        ...
        if((WAIT_SEND == s_stP2PSendCtrl[ucSendIndex].emWaitSendFlag) &&
           (NOT_WAIT_ACK == s_stP2PSendCtrl[ucSendIndex].emWaitACKFlag))  /* Timeout / Received ACK */
        {
            /* packing P2P data */
                        ...
                        case AMS_DWHARTCMD:
                        {
                            ...
                            else
                            {
                                ...
                                if(((pstAppend->stSlaveState[(ucModAddr/2)*2].stSlaveDiag.ucOnlineDiag & 0x01) != 0) &&
                                   ((pstAppend->stSlaveState[(ucModAddr/2)*2].stSlaveDiag.ucOnlineDiag & 0x10) != 0))
                                {
                                    ucModAddr = (ucModAddr/2)*2;
                                }
                                else if(((pstAppend->stSlaveState[(ucModAddr/2)*2 + 1].stSlaveDiag.ucOnlineDiag & 0x01) != 0) &&
                                        ((pstAppend->stSlaveState[(ucModAddr/2)*2 + 1].stSlaveDiag.ucOnlineDiag & 0x10) != 0))
                                {
                                    ...
                                    if((pstIoModInfo != NULL) && (pstAppend != NULL))
                                    {
                                        for(ucModIndex = 0U; ucModIndex < 120; ucModIndex++)
                                        {
                                            if((((MODTAG_AI == pstIoModInfo->stSysIoSlaveInfo.stDevInfo[ucModIndex*2].usModTag) ||
                                               ...
                                               ((pstAppend->stSlaveState[ucModIndex*2].stSlaveDiag.ucOnlineDiag & 0x10) != 0)) ||
                                               (((MODTAG_AI == pstIoModInfo->stSysIoSlaveInfo.stDevInfo[ucModIndex*2+1].usModTag) ||
                                                ...
                                                ((pstAppend->stSlaveState[ucModIndex*2+1].stSlaveDiag.ucOnlineDiag & 0x10) != 0)))
                                            {
...
//                        s_stSlaveFWInfo.ucSlotID = 9;
                        ...
                        if(IPBUS_RET_OK == ucMailRet)
                        {
                            ...
                            printf("SEND WR_ST OK %d %d\n",s_stSlaveFWInfo.ucSlotID,stMailHeader.usMailLN);
                        }
                        else
                        {
                ...
                if(HAS_IO_FW_UPDATE == s_uiFWUpdateFlg)
                {
                       ...
                       (s_stSlaveFWInfo.uiFileReadIndex <= s_stSlaveFWInfo.uiIOFWLen))
                    {
                        ...
                        if(IPBUS_RET_OK == ucMailRet)
                        {
                            ...
                            printf("SEND WR_CONT OK %d %d\n",stMailHeader.usMailLN,s_stSlaveFWInfo.uiFileReadIndex);
                        }
                        else
                        {
                            s_emUpdateIOStag = UPIO_END;
...
//                    printf("SEND RD_STAT OK %d\n",s_stSlaveFWInfo.ucSlotID);
                }
                else
                {
                    s_emUpdateIOStag = UPIO_END;
...
static void CommAlarmCycle(void)
{
    ...
    switch(emReadAlarmStag)
    {
        case RA_RD:
        {
            ...
            if((pstAppend != NULL) && (AH_FREE == emAperiodicHandleFlg) && (s_usCounter >= 1000))
            {
...
//                printf("BI%d ModT %x\n",ucBISlotID,pstAppend->stBIVern[ucBISlotID].stBIFirmwareVern.usModuleType);
            }
...
//                printf("HDV %x ",pstAppend->stBIVern[ucBISlotID].stBIFirmwareVern.ucHardwareVern);
//                printf("SFV %x\n",pstAppend->stBIVern[ucBISlotID].stBIFirmwareVern.ucSoftwareVern);
            }
...
//                printf("ChipID %x ",pstAppend->stBIVern[ucBISlotID].stBIFirmwareVern.usChipID);
//                printf("V %x\n",pstAppend->stBIVern[ucBISlotID].stBIFirmwareVern.ucChipFWVern);
            }
            ...
            memset((puint8_t)&pstAppend->stBIState[ucBISlotID].stBIDiagInfo.usBITemperature, 0xff, 4*sizeof(uint16_t));
            if((ucBIPWInfoData[(99 + (3/8))] & (0x01 << (3 % 8))) != 0)
            {
                memcpy(&pstAppend->stBIState[ucBISlotID].stBIDiagInfo.usBITemperature, &ucBIPWInfoData[uiBuffIndex], sizeof(uint16_t));
                uiBuffIndex += sizeof(uint16_t);
//                printf("T %d",pstAppend->stBIState[ucBISlotID].stBIDiagInfo.usBITemperature);
            }
            
            if((ucBIPWInfoData[(99 + (4/8))] & (0x01 << (4 % 8))) != 0)
            {
                memcpy(&pstAppend->stBIState[ucBISlotID].stBIDiagInfo.usSEP1Temperature, &ucBIPWInfoData[uiBuffIndex], sizeof(uint16_t));
                uiBuffIndex += sizeof(uint16_t);
//                printf("T1 %d",pstAppend->stBIState[ucBISlotID].stBIDiagInfo.usSEP1Temperature);
            }
            
            if((ucBIPWInfoData[(99 + (5/8))] & (0x01 << (5 % 8))) != 0)
            {
                memcpy(&pstAppend->stBIState[ucBISlotID].stBIDiagInfo.usSEP2Temperature, &ucBIPWInfoData[uiBuffIndex], sizeof(uint16_t));
                uiBuffIndex += sizeof(uint16_t);
//                printf("T2 %d",pstAppend->stBIState[ucBISlotID].stBIDiagInfo.usSEP2Temperature);
            }
            
            if((ucBIPWInfoData[(99 + (6/8))] & (0x01 << (6 % 8))) != 0)
            {
                memcpy(&pstAppend->stBIState[ucBISlotID].stBIDiagInfo.usSEP3Temperature, &ucBIPWInfoData[uiBuffIndex], sizeof(uint16_t));
                uiBuffIndex += sizeof(uint16_t);
//                printf("T3 %d\n",pstAppend->stBIState[ucBISlotID].stBIDiagInfo.usSEP3Temperature);
            }
...
//                printf("PW1_AA01_DT1 %x ",pstAppend->stPWState[ucPWSlotID].stPWDiagInfo.ucPW1_AA01_DT1);
            }
...
//                printf("PW1_AA02_DT1 %x\n",pstAppend->stPWState[ucPWSlotID].stPWDiagInfo.ucPW1_AA02_DT1);
            }
            ...
            {
                memcpy(&pstAppend->stPWState[ucPWSlotID].stPWDiagInfo.ucPW1_AB01_DT1, &ucBIPWInfoData[uiBuffIndex], sizeof(uint8_t));
                uiBuffIndex += sizeof(uint8_t);
            }
            ...
            {
                memcpy(&pstAppend->stPWState[ucPWSlotID].stPWDiagInfo.ucPW1_AB02_DT1, &ucBIPWInfoData[uiBuffIndex], sizeof(uint8_t));
                uiBuffIndex += sizeof(uint8_t);
            }
...
//                printf("PW1_AF04_DT1 %x ",pstAppend->stPWState[ucPWSlotID].stPWDiagInfo.ucPW1_AF04_DT1);
            }
...
//                printf("PW2_AA01_DT1 %x ",pstAppend->stPWState[ucPWSlotID].stPWDiagInfo.ucPW2_AA01_DT1);
            ...
            if((ucBIPWInfoData[(50 + (50/8))] & (0x01 << (50 % 8))) != 0)
            {
            ...
            {
                memcpy(&pstAppend->stPWState[ucPWSlotID].stPWDiagInfo.ucPW2_AB01_DT1, &ucBIPWInfoData[uiBuffIndex], sizeof(uint8_t));
                uiBuffIndex += sizeof(uint8_t);
            }
            ...
            {
                memcpy(&pstAppend->stPWState[ucPWSlotID].stPWDiagInfo.ucPW2_AB02_DT1, &ucBIPWInfoData[uiBuffIndex], sizeof(uint8_t));
                uiBuffIndex += sizeof(uint8_t);
            }
            ...
            if((ucBIPWInfoData[(99 + (65/8))] & (0x01 << (65 % 8))) != 0)
            {
                ...
                uiBuffIndex += 3*sizeof(uint8_t);
//                printf("PW2_AF04_DT1 %x ",pstAppend->stPWState[ucPWSlotID].stPWDiagInfo.ucPW2_AF04_DT1);
            }
...
FileOptRest_t CommEraseFlash(NorFlashOptType_t emNorFlashOptType,uint32_t uiFWUsedArea)
{
    ...
    switch(emProcessStag)
    {
        ...
        {/*等待应答，若收到应答，解析应答结果，并返回；若超时，返回超时*/
            ...
            {
                ucTimeCount++;
                if(ucTimeCount >= NORFLASH_OPT_TIMEOUT )
                {/*超时处理；返回失败*/
                    emFileOptRest = FAIL;
                    emProcessStag = FOS_START;
...
static void CommPMUpdate(void)
{
    ...
    static uint8_t ucTest2 = 0;
    
//    if((NOT_HAS_FW_UPDATE == s_uiFWUpdateFlg) && (0xaa == ss_ucTest))
//    {
//        uiCount++;
//        if(uiCount >= 1000)
//        {
...
//    ucTest2++;
//    if((ucTest2 > 100) && (0xaa == ss_ucTest))
//    {
//        ucTest2 = 0;
//        printf("FMUpdateStag %d %d\n",s_emFMUpdateStag,s_uiFWUpdateFlg);
//    }
...
//                s_emFMUpdateStag = FM_UPDATE_END;
        ...
        case FM_UPDATE_ERASE_FW:
        {
            ...
            {/*擦除操作成功，进行下一步*/
                ...
                s_stSlaveFWInfo.uiFileReadIndex = 0;
                printf("ERASE_FW SUCCESS \n");
                
//                puint8_t pucTT = 0xec0fff60;
//                
...
//                }
            ...
            if((HAS_RTS_UPDATE == s_uiFWUpdateFlg) || (HAS_FPGA_UPDATE == s_uiFWUpdateFlg))
            {
                switch(s_emFWType)
                {
                    ...
                    case FW_PMCORE0RTS:
                    {
                        ...
                        if(0 == s_stSlaveFWInfo.uiFileReadIndex)
                        {
                            printf("WR CORE0 RTS\n");
                        }
                    }
                ...
                if((pucSrcAddr != NULL) && (pucDestAddr != NULL))
                {
                    ...
                    printf("N %d %d\n",s_emFWType,s_stSlaveFWInfo.uiFileReadIndex);
                    ...
                    {
                        s_emFMUpdateStag = FM_UPDATE_END;
                        printf("WR Nor Err %d %d %x %d\n",iReadFWFileLen,iWriteFWFileLen,pucDestAddr,nor_flash_errno);
                        
                        for(uint8_t y = 0; y < 10; y++)
                        {
                            printf("%x ",*pucDestAddr);
                            pucDestAddr++;
                        }
                        printf("\n");
                    }
        ...
        case FM_UPDATE_ERASE_FLG:
        {
            ...
            {/*擦除操作成功，进行下一步*/
                ...
                printf("ERASE_FLG SUCCESS\n");
            }
            if(FAIL == emOptRst)
            {/*操作失败，并应答错误信息*/
                s_emFMUpdateStag = FM_UPDATE_END;
                
                printf("ERASE_FLG FAIL\n");
            }
        }
        break;
