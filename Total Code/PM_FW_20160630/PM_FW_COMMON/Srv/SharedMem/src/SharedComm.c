...
***********************************************************************************************************************
*/
static SMMsgBox_t  s_stSMMsgBox[NUM_OF_SM_MSGTYPE];
...
{
    /* 初始化 */
    memset((puint8_t)&s_stSMMsgBox[0], 0, sizeof(SMMsgBox_t)*NUM_OF_SM_MSGTYPE);
...
bool_t SysGetSMMsg (SMMsgType_t emMsgType,CMController_t emCMIndex, puint8_t pucContent, puint16_t pusLen)
{
    ...
    /* 输入输出参数有效性检查 */
    if((emMsgType < NUM_OF_SM_MSGTYPE) && (pucContent != NULL) && (pusLen != NULL))
    {
        /* 找到待读出信息的信箱 */
        pstMsgBox = &(s_stSMMsgBox[emMsgType]);

        /* 原子操作：读锁标志，判断是否已加锁，若未加锁，则加锁，返回成功标志；若已加锁，则返回失败标志 */
        bResult = (bool_t) atomic32Cas((puint32_t)&(pstMsgBox->bLock), \
        ...
        if(true == bResult)
        {
            ...
            /* 原子操作：解除锁标志 */
            atomic32Set((puint32_t)&(pstMsgBox->bLock), (uint32_t)SM_MSGBOX_UNLOCKED);
        }
        else
        {
...
bool_t SysSetSMMsg (SMMsgType_t emMsgType ,CMController_t emCMIndex, puint8_t pucContent, uint16_t usLen)
{
    ...
    /* 输入输出参数有效性检查 */
    if((emMsgType < NUM_OF_SM_MSGTYPE) && (pucContent != NULL) && (usLen != 0))
    {
        /* 找到待写入信息的信箱 */
        pstMsgBox = &(s_stSMMsgBox[emMsgType]);
