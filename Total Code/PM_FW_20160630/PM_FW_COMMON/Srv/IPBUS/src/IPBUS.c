...
void IPBUSDualRamInit(void)
{
    ...
    /* 清从站周期轮询列表 */
    IPBUSSetSlaveRollingList(&ucInit256Buf[0], IPBUS_SLAVE_ROLLINGLIST_SIZE);
    /* 清从站使能标志（buffer1） */
...
void IPBUSSetSlaveRollingList(uint8_t ucData[], uint32_t uiSize)
{
    ...
    {
        usDataTemp[i] = ((uint16_t)ucData[2*i]<<8) + (uint16_t)ucData[2*i+1];
    }

    IPBUSMemCopyByLocalbus(REG_IPBUS_SLAVE_ROLLINGLIST, &usDataTemp[0], uiSizeTemp);
...
void IPBUSSetSlaveEnableBuf(uint8_t ucData[], uint8_t ucBufID)
{
    ...
    if (ucBufID == IPBUS_SLAVE_ENABLE_BUF0)
    {
        IPBUSMemCopyByLocalbus(REG_IPBUS_SLAVE_ENABLE_BUF0, &usDataTemp[0], uiSizeTemp);
    }
    else if (ucBufID == IPBUS_SLAVE_ENABLE_BUF1)
    {
        IPBUSMemCopyByLocalbus(REG_IPBUS_SLAVE_ENABLE_BUF1, &usDataTemp[0], uiSizeTemp);
    }
    else
    {
        ;
...
void IPBUSSetSlavePRMChange(uint8_t ucData[])
{
    ...
    for(i = 0; i< uiSizeTemp; i++)
    {
        usDataTemp[i] = ((uint16_t)ucData[2*i]<<16) + (uint16_t)ucData[2*i+1];
    }
...
static void IPBUSSlaveInputAddrInit(void)
{
    ...
    for(usSlaveNo = 0; usSlaveNo <= IPBUS_SLAVE_NUMBER; usSlaveNo++)
    {
