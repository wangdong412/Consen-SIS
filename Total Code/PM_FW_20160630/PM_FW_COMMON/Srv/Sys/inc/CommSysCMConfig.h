...
#ifndef COMMSYSCMCONFIG_H_
#define COMMSYSCMCONFIG_H_
...
***********************************************************************************************************************
*/
#define MAX_CM_NUM                          (2U)//4
#define MAX_PORT_NUM                        (2U)//6
#define MAX_COMPORT_NUM                     (1U)//4
#define MAX_NETPORT_NUM                     (1U)//2
#define MAX_ACCESS_CTRL_NUM                 (16U)

#define MAX_MODBUS_VAR_NUM                  (100U)
#define MAX_MODBUS_RGN_VAR_NUM              (100U)//10000          /* Added：每个Modbus数据区最多包含9999个变量 */
#define MAX_MODBUS_BLOCK_NUM                (2U)//64             /*undefined*/
#define MAX_MODBUS_AREA_NUM                 (1U)              /*undefined*/
#define MAX_MODBUS_SLAVE_NUM                (64U)             /*undefined*/

#define MAX_OPC_NUM                         (100U)//10000          /*undefined*/
...
#define MAX_TASK_EXVAR_NUM                  (500U)            /*待定*/
...
/*实时数据不需要同步*/
#define NOT_NEED_SYNC                       (0x00U)
/*实时数据需要同步*/
#define NEED_SYNC                           (0x01U)
...
typedef enum
{
    FP_ORDER_H = 0x00,       /*0:High 16 Bits First*/
    FP_ORDER_L,              /*1:Low 16 Bits First*/
    FP_ORDER_INVALID
}FPOrder_t;
...
typedef enum
{
    NW_NET1 = 0x01,  /*网口号：0-NET1 / 1-NET2 / 2- NET1 & NET2*/
    NW_NET2,
    NW_NET_ALL
}Network_t;
...
typedef enum
{
    VM_READ = 0x0000,  /* Read  读 */
    VM_WRITE           /* Write  写 */
}VisitMode_t;


/* Modbus Region  Modbus数据区枚举定义 */
typedef enum
{
    MR_COIL = 0x0000,    /* Coil  线圈 */
    MR_DISCRETE_INPUTS,  /* Discrete Inputs  离散量输入*/
    MR_INPUT_REGS,       /* Input Registers  输入寄存器 */
    MR_HOLD_REGS         /* Holding Registers  保持寄存器 */
}ModbusRgn_t;
...
typedef enum
{
    PMSN_READONLY = 0x0000,             /* Read only  只读 */
    PMSN_READWRITE                      /* Read/Write 可读写 */
}Permission_t;
...
/*Modbus Slave站信息头结构体定义*/
typedef struct  ModbusSlaveHeadTag
{
    uint32_t uiBlockNum;
}ModbusSlaveHead_t;/*4 byte*/
...
{
    uint32_t uiBlockID;
    ModbusRgn_t emModbusRgn;            /* Changed: 线圈、离散量输入、输入寄存器、保持寄存器 */
    Permission_t emPermission;          /* Changed: 可读写 / 只读 */
...
typedef struct ModbusSlaveBlockTag
{
    ...
    ModbusVar_t stModbusVar[MAX_MODBUS_RGN_VAR_NUM];
...
typedef struct ModbusSlaveInfoTag
{
    uint32_t uiCRC;
    uint32_t uiFileVern;
    ModbusSlaveHead_t stModbusSlaveHead;
...
/*CM上每个端口Modbus Master站信息头结构体定义*/
typedef struct  ModbusMasterPortHeadTag
{
    uint32_t uiBlockNum;
}ModbusMasterPortHead_t;/*4 byte*/

/*Modbus master站块信息头结构体定义*/
typedef struct ModbusMasterBlockHeadTag
{
    uint32_t uiBlockID;
    VisitMode_t emVisitMode;            /* Changed: 读 / 写 */
    ModbusRgn_t emModbusRgn;            /* Changed: 线圈、离散量输入、输入寄存器、保持寄存器 */
...
typedef struct ModbusMasterPortTag
{
    ...
    ModbusMasterPortHead_t stModbusMasterPortHead;
...
typedef struct COMConfigInfoTag
{
    ...
    uint8_t ucHandshake;                /*0:None,1:Hardware，默认值0 (None)。*/
    uint8_t ucTermiOptions;             /*0:None,1:Resistor, 2: Pull Up,3:Down,4:All，默认值0 (None)。*/
    ...
    uint8_t ucReserve1[2];              /*保留*/
    uint8_t ucReserve2[4];              /*保留*/
...
typedef struct SectionTableTag
{
    ...
    uint32_t uiSize;
    uint32_t uiMsg;
    uint8_t uiSameWithLastDownload;
