...
static PMRecvInfo_t s_stPMRecvInfo[PM_NUM];  /* 本系PM作为接收方时，维护的接收信息：索引号为发送方PMID */
static PMSendInfo_t s_stPMSendInfo[PM_NUM];  /* 本系PM作为发送方时，维护的发送信息：索引号为接收方PMID */
...
static bool_t DecodePMSafetyFra( uint8_t const pucSFra[], uint32_t uiSFraLen,
                                 SysPMID_t emSrcPMID, SysPMID_t emDstPMID,
...
bool_t PMHandleRecvTO( SysPMID_t emSendPMID )
{
    bool_t bResult = false;
    PMRecvInfo_t *pstRecvInfo = NULL;

    if( emSendPMID < PM_NUM )
    {
        bResult = true;
...
bool_t PMDecodeFra( SysPMID_t emSendPMID, uint8_t const pucFra[], uint16_t usFraLen,
        ...
        *pusDataLen = 0U;             /* Reset */
        ...
        if( usFraLen > (uint16_t)uiHeadLen )
        {
            ...
            if( usFraLen >= (uint16_t)uiSFraLen )
            {
                ...
                /* Decode Safety Frame */
                if( DecodePMSafetyFra( pucFra, uiSFraLen, emSendPMID, s_emLocalPMID, pstRecvInfo, s_ucTolThr ))
                {
                    /* Copy Data */
...
bool_t PMHandleSendTO( SysPMID_t emRecvPMID )
{
    bool_t bResult = false;

    if( emRecvPMID < PM_NUM )
    {
        bResult = true;

        /* Set Timeout Flag */
        SysSetPMSendTimeoutFlag( emRecvPMID, true );
    }
...
bool_t PMMakeFra( SysPMID_t emRecvPMID, uint8_t const pucData[], uint16_t usDataLen,
       ...
       ( pusFraLen != NULL ))
    {
        ...
        *pusFraLen = 0U;            /* Reset Output Length */
        ...
        if( usFraBuffLen >= usSFraLen )
        {
            ...
            /* Make Response Head */
            usIndex = 0U;
            stHead.ucSrcPMID = (uint8_t)s_emLocalPMID;
                                 ...
                                 PMRecvInfo_t *pstRecvInfo, uint8_t ucTolThr )
{
       ...
       ( pstRecvInfo != NULL ))
    {
        ...
        if( bValid )
        {
            ...
            {
                /* Sequence */
                if( stHead.usSeq != (pstRecvInfo->usRecvSeqNum + 1U))
                {
                    bValid = false;
                }
                else
