...
void SyncMngt(void)
{
    ...
    PMController_t emPrePMId = SysGetNextPMID(emLocalPMId);          /* 前一个PM ID号     */
    ...
    {/*当一包工程文件同步数据未处理完时，不再收发同步数据*/
        ...
        {/* 若同步任务队列不为空 */
            ...
            DEBUG_SYNC("  SyncSrvQueue is not empty; stSyncTask.emTaskWait is : %x \n",stSyncTask.emTaskWait);
...
void SyncRecvSyncData(SyncData_t* pstSyncData)
{
    ...
    PMController_t emPrePMId = SysGetNextPMID(emLocalPMId);               /* 前一个 PM ID号     */
    ...
    {/* 校验接收缓存指针不为空 */
        ...
        {/* 接收长度小于零，接收数据错误 */
            ...
            pstSyncData->sLinkASrvErr = sRetLen;
            
            DEBUG_SYNC("  RECV NEXT PM SRV DATA ERR ! \n");
            
        }
        else if(SYNC_RECV_SIZE_0 == sRetLen)
        {/* 若接收到数据长度为0 */
            pstSyncData->sLinkASrvLen = 0;
        ...
        sRetLen = PMBUS_Receive(PM_BUS_B, PM_RECV_BUFF_PRJ, (puint8_t)&pstSyncData->stSyncMSGA.stSyncPrjData);
        
        if(sRetLen < 0)
        {/* 接收长度小于零，接收数据错误 */
            pstSyncData->sLinkAPrjLen = 0;
            pstSyncData->sLinkAPrjErr = sRetLen;
            
            DEBUG_SYNC("  RECV NEXT PM PRJ DATA ERR ! \n");
        ...
        {/* 接收长度小于零，接收数据错误 */
            ...
            pstSyncData->sLinkBSrvErr = sRetLen;
            
            DEBUG_SYNC("  RECV PRE PM SRV DATA ERR ! \n");
        ...
        {/* 接收长度小于零，接收数据错误 */
            ...
            pstSyncData->sLinkBPrjErr = sRetLen;
            
            DEBUG_SYNC("  RECV PRE PM PRJ DATA ERR ! \n");
...
***********************************************************************************************************************
*/
uint32_t SyncHandleCtrlData(const SysPMStateInfo_t* pstOneStatData, PMController_t emSrcPMId,
                            int16_t sDataLen)
{
...
void SyncBroadCastCtrlData(ack_flag_t emAckFlag)
{
    ...
    PMController_t emPrePMId = SysGetNextPMID(emLocalPMId);             /* 前一个PMID号     */
...
void SyncRecvRealData(SysSyncPrjQueue_t* pstSyncPrjQueue)
{
    ...
    PMController_t emPrePMId = SysGetNextPMID(emLocalPMId);                 /* 本机PMID号           */
    SysSyncTask_t stSyncTask;                                   /* 同步任务                 */
...
void SyncRecvFileData(SysSyncPrjQueue_t* pstSyncPrjQueue)
{
    ...
    SyncStaticAck_t stSyncFileAck;                                  /* 接收文件应答 */
    ...
    if(pstSyncPrjData->uiSyncPrjDataLen > 0U)
    {
        do
        {
            pstSyncRecvFile = (SyncFile_t*)(pstSyncPrjData->ucPrjData);
            ...
            {/* 不是接收到新文件 */
                ...
                if(stCfgFileState.bWriteable == CFG_FILE_WR_DISABLE)
                {
                    break;
                }

                SysSetCfgFileInfo(pstSyncRecvFile->stFileHead.emCfgFileType,pstSyncRecvFile->ucData, uiDataLen);
...
//                    SyncWriteFileEnd(s_stSyncFileStatus.ucFileName);
        ...
        /* 打包应答数据 */
        stSyncFileAck.stSyncHead.emSrvType = SYNC_FILE;
        stSyncFileAck.ucData[0] = uiRecvAck;
        /* 计算发送数据长度 */
        iSendLen = sizeof(stSyncFileAck.stSyncHead) + sizeof(stSyncFileAck.ucData[0]);

        if(emNextPMId == emSrcPMId)
        {
            sRetSend = PMBUS_Send(PM_BUS_A, PM_RECV_BUFF_PRJ, (puint8_t)&stSyncFileAck, iSendLen, PMBUS_SEND_TIMEOUT);
        ...
        {
            sRetSend = PMBUS_Send(PM_BUS_B, PM_RECV_BUFF_PRJ, (puint8_t)&stSyncFileAck, iSendLen, PMBUS_SEND_TIMEOUT);
...
void SyncSendFileData(SysSyncPrjQueue_t* pstSyncPrjQueue)
{
    ...
    do
    {
        ...
        {/* 发送新文件 */
            ...
            {/* 文件发送完成，更新同步任务 */
                SysSyncPrjQueueUpdate(TASK_FILE_COMPLETE);
                break;
            }
            /* 新文件已发送数据长度为0 */
            s_stSyncFileStatus.uiSendSize = 0U;
        ...
        if(emNextPMId == emDstPMId)
        {
            sRetSend = PMBUS_Send(PM_BUS_A, PM_RECV_BUFF_PRJ, (puint8_t)&stSyncPrjData, iSendLen, SYNC_DATA);
            if(sRetSend != RET_OK)
            {/* 发送错误重新启动文件传输 */
                /*log*/
                SyncInitFileStatus();
        ...
        {
            sRetSend = PMBUS_Send(PM_BUS_B, PM_RECV_BUFF_PRJ, (puint8_t)&stSyncPrjData, iSendLen, SYNC_DATA);
            if(sRetSend != RET_OK)
            {/* 发送错误重新启动文件传输 */
                /*log*/
                SyncInitFileStatus();
