...
bool_t SysOpenFile(uint8_t ucName[])
{
    ...
    if(ucName != NULL)
    {
        ...
        if((FM_MSGBOX_UNLOCKED == pstReqMsgBox->bLock) && (FM_MSGBOX_EMPTY == pstReqMsgBox->bStatus))
        {
            ...
            {
                memcpy((puint8_t)&(pstReqMsgBox->stReqMsg.ucFileName[0]), ucName, ucNameLen);
...
bool_t SysRenameFile(uint8_t ucOldName[], uint8_t ucNewName[])
{
    ...
    if((ucOldName != NULL) && (ucNewName != NULL))
    {
        ...
        if((FM_MSGBOX_UNLOCKED == pstReqMsgBox->bLock) && (FM_MSGBOX_EMPTY == pstReqMsgBox->bStatus))
        {
            ...
            {
                memcpy((puint8_t)&(pstReqMsgBox->stReqMsg.ucFileName[0]), ucOldName, ucOldNameLen);
                memcpy((puint8_t)&(pstReqMsgBox->stReqMsg.ucContent[0]), ucNewName, ucNewNameLen);
