...
#define MAX_RTN_SOE_RECORD_NUM                 (10000U)
...
typedef struct P2PConfigInfoTag
{
    ...
    uint32_t uiP2PRecvBlockTabl[MAX_P2P_BLOCK_NUM + 1];
    uint32_t uiP2PSendBlockTabl[MAX_P2P_BLOCK_NUM + 1];
...
{
    ActiveFlag_t emActive;
    uint16_t usUpdateFlag;                  /*更新标志，0：SOE块配置信息未变化；1：SOE块配置信息变化；2：SOE块配置信息为新增*/
    uint16_t usBlockType;               /* SOE块的类型，取值UNASSIGNED、EXTERNAL、FIRST OUT、HISTORY、OPC等*/
    uint8_t ucProperty;                 /* SOE块属性：其中最低位表示是否掉电保持（0：不保持，1：保持）*/
    uint8_t ucReserved;
    uint32_t uiMaxRecordCount;          /*单个SOE块保存的SOE记录条数上限*/
