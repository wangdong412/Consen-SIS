...
static uint32_t s_uiSwitchPosTmp;
...
static CMSlotIdTabl_t s_stOldCMSlotIdTabl;
static uint32_t s_uiDownloadStartFlag;
...
//    memset(&s_stOPCCfg, 0x00, sizeof(OPCCfg_t));
    ...
    memset(&s_stOldCMSlotIdTabl, 0, sizeof(CMSlotIdTabl_t));
    s_uiDownloadStartFlag = 0;
    ...
    atomic32Set(&s_uiSwitchPos, KS_INVALID_TYPE);
    
    atomic32Set(&s_uiSwitchPosTmp, KS_INVALID_TYPE);
    
    atomic32Set(&s_emLocalID, 0);
...
void SharedConfigAreaInit(void)
{
    ...
    atomic32Set(&s_stVoteCfg.uiTmpAvailable[TASK_UP1], INVALID_BUFF_ID);
    atomic32Set(&s_stVoteCfg.uiTmpAvailable[TASK_UP2], INVALID_BUFF_ID);
...
***********************************************************************************************************************
*/
VOTEConfigInfo_t* SharedGetVOTEConfigInfo(Task_ID_t emTaskID)
{
    uint32_t uiAvailable = INVALID_BUFF_ID;        /*乒乓buffer中可读的配置信息片区ID*/
    ...
    if(emTaskID < MAX_TASK_NUM)
    {
        if((OS_SWITCH_START == SharedGetOnlineDownLoadSwitchFlg(emTaskID)) || \
           (OS_SLAVE_ENABLE == SharedGetOnlineDownLoadSwitchFlg(emTaskID)))
        {
            uiAvailable = atomic32Get(&s_stVoteCfg.uiTmpAvailable[emTaskID]);
        }
        else
        {
...
***********************************************************************************************************************
*/
CMSlotIdTabl_t* SharedGetOldCMSlotIdTabl(void)
{
    CMSlotIdTabl_t* pstCMSlotIdTabl = NULL;

    pstCMSlotIdTabl = &s_stOldCMSlotIdTabl;
...
/*
***********************************************************************************************************************
* 函数名称: SharedBackupOldCMSlotIdTabl
*
* 功能描述: 备份CM表
*
* 输入参数: 无
...
***********************************************************************************************************************
*/
void SharedBackupOldCMSlotIdTabl(void)
{
    uint32_t uiAvailable = INVALID_BUFF_ID;        /*乒乓buffer中可读的配置信息片区ID*/
    CMSlotIdTabl_t* pstCMSlotIdTabl = NULL;
    ...
    if(FIRST_PIECE == uiAvailable || SECOND_PIECE == uiAvailable)
    {
        ...
        memcpy(&s_stOldCMSlotIdTabl, pstCMSlotIdTabl, sizeof(CMSlotIdTabl_t));
    }
...
/*
***********************************************************************************************************************
* 函数名称: SysGetDownloadAllStartFlag
*
* 功能描述: 获取全下装开始的标识
*
* 输入参数: 无
...
***********************************************************************************************************************
*/
uint32_t SysGetDownloadAllStartFlag(void)
{
    uint32_t uiFlag = 0;

    uiFlag = atomic32Get(&s_uiDownloadStartFlag);

    return uiFlag;
}
/*
***********************************************************************************************************************
* 函数名称: SysSetDownloadAllStartFlag
*
* 功能描述: 设置全下装开始的标识
*
* 输入参数: 无
...
***********************************************************************************************************************
*/
void SysSetDownloadAllStartFlag(void)
{
    atomic32Set(&s_uiDownloadStartFlag, CS1131_DOWNLOAD_ALL_START);
    return;
}
/*
***********************************************************************************************************************
* 函数名称: SysClearDownloadAllStartFlag
*
* 功能描述: 清除全下装开始的标识
*
* 输入参数: 无
...
***********************************************************************************************************************
*/
void SysClearDownloadAllStartFlag(void)
{
    atomic32Set(&s_uiDownloadStartFlag, CS1131_DOWNLOAD_ALL_NOT_START);
...
***********************************************************************************************************************
*/
uint32_t SharedGetSwitchPosTmp(void)
{
    uint32_t uiSwitchPos;
    
    uiSwitchPos = atomic32Get(&s_uiSwitchPosTmp);
...
/*
***********************************************************************************************************************
* 函数名称: SharedSetSwitchPosTmp
*
* 功能描述: 设置直接从寄存器读到的钥匙开关
*
* 输入参数: 无
*
* 输出参数: 无
...
***********************************************************************************************************************
*/
void SharedSetSwitchPosTmp(uint32_t uiSwitchPos)
{
    atomic32Set(&s_uiSwitchPosTmp, uiSwitchPos);
...
/*
***********************************************************************************************************************
* 函数名称: SharedGetTaskActive
*
* 功能描述: 获取指定Task的Active标志
*
* 输入参数: 无
*
* 输出参数: 无
*
* 返 回 值  : 指定Task的Active标志
*
* 注意事项: 无
***********************************************************************************************************************
*/
ActiveFlag_t SharedGetTaskActive(Task_ID_t emTaskID)
{
    ...
    if(emTaskID < MAX_TASK_NUM)
    {
        ...
        if(pstCtrlStationConfigInfo != NULL)
        {
            emActive = pstCtrlStationConfigInfo->stTaskConfigInfo[emTaskID].emActiveFlag;
        }
    }
...
***********************************************************************************************************************
*/
VoteModInfo_t* SharedGetVoteInfo(Task_ID_t emTaskID,uint8_t ucModID)
{
    VOTEConfigInfo_t* pstVOTEInfo = NULL;
    VoteModInfo_t* pstVoteModInfo = NULL;
    
    pstVOTEInfo = SharedGetVOTEConfigInfo(emTaskID);
...
***********************************************************************************************************************
*/
uint8_t SharedGetVoteTolerate(void)
{
    ...
    {
        ucTolerate = (uint8_t)pstCtrlStationInfo->stStationConfigInfo.ucVoteTolerate;
    }
...
***********************************************************************************************************************
*/
uint8_t SharedGetP2PBlockIDSend(uint8_t ucP2PBlockIndex)
{
    uint8_t uiBlkID = 0U;
    P2PConfigInfo_t* pstP2PConfigInfo = NULL;
    ...
    if((pstP2PConfigInfo != NULL) && (ucP2PBlockIndex <= MAX_P2P_BLOCK_NUM))
    {
        uiBlkID = pstP2PConfigInfo->uiP2PSendBlockTabl[ucP2PBlockIndex];
    }
...
***********************************************************************************************************************
*/
uint32_t SharedGetCommTolCnt(void)
{
    ...
    if(pstCtrlStationConfigInfo != NULL)
    {
        uiTolCnt = pstCtrlStationConfigInfo->stStationConfigInfo.ucCommTolerate;
    }
...
void SharedRecordTmpAvailable(Task_ID_t emTaskID)
{
    ...
    if(emTaskID < MAX_TASK_NUM)
    {
        ...
        atomic32Set(&s_stVoteCfg.uiTmpAvailable[emTaskID], atomic32Get(&s_stVoteCfg.uiAvailable));
    }
...
/*
***********************************************************************************************************************
* 函数名称: SharedClearVoteTmpAvailable
*
* 功能描述: 清除表决信息的临时记录标志
*
* 输入参数: 无
...
***********************************************************************************************************************
*/
void SharedClearVoteTmpAvailable(Task_ID_t emTaskID)
{
    
    if(emTaskID < MAX_TASK_NUM)
    {
        atomic32Set(&s_stVoteCfg.uiTmpAvailable[emTaskID], INVALID_BUFF_ID);
    }
