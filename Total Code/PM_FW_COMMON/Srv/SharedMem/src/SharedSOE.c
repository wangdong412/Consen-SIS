        ...
        SysTime_t* pstTimestamp)
{
    ...
    if((ucQueueIndex < MAX_SOE_QUEUE_NUM) && (pstSOE != NULL))
    {
        ...
        if(true == bAtomicFlg)
        {
            if(SOE_STORAGE_SRAM == stSOEQueue[ucQueueIndex].ucStorage)
            {
                uiSoeIndex = SharedGetSRAMSOE(ucQueueIndex, pstSOE, usLogicPos, puiNextIndex, pstTimestamp);
            }
            else if(SOE_STORAGE_DDR == stSOEQueue[ucQueueIndex].ucStorage)
            {
                uiSoeIndex = SharedGetDDRSOE(ucQueueIndex, pstSOE, usLogicPos, puiNextIndex, pstTimestamp);
            }
            else
            {
...
bool_t SharedSetHardSOE(HardSOE_t stSOE)
{
    ...
    SOESRAMQueuesInfo_t* pstSOESRAMQueuesInfoStartAddr = (SOESRAMQueuesInfo_t*)SOE_SRAM_QUEUE_INFO_START_ADDR;
    uint16_t i = 0;
    puint16_t pusDst = NULL;
    puint16_t pusSrc = NULL;
            ...
            if(SOE_QUEUE_CONFIGED == stSOEQueue[ucQueueIndex].bConfiged)
            {
                ...
                else /* SOE_QUEUE_TYPE_CYCLE_FIFO */
                {
                    ...
                    if(uiOffsetInSoeArr <= SOE_RECORD_SEC3_MAX_INDEX)
                    {
                        ...
                        //memcpy(pucDst, (puint8_t)&stSOE, sizeof(HardSOE_t));
                        pusDst = (puint16_t)pucDst;
                        pusSrc = (puint16_t)&stSOE;
                        for(i = 0; i < sizeof(HardSOE_t)/2; i++)
                        {
                            pusDst[i] = pusSrc[i];
                        }
...
***********************************************************************************************************************
*/
int32_t SharedGetHardSOE(HardSOE_t* pstSOE, uint16_t usLogicPos, puint32_t puiNextIndex, SysTime_t* pstTimestamp)
{
    uint32_t uiSoeIndex = READ_SOE_QUEUE_FAILED;
    ...
    uint32_t uiOffsetInSoeArrPre = 0;
    puint8_t pucSrcPre = NULL;
    HardSOE_t stSOEPre;
                    ...
                    else /* SOE_QUEUE_TYPE_CYCLE_FIFO */
                    {
                        ...
                        else
                        {
                            ...
                            if(usLogicPos != READ_SOE_FROM_BEGINNING)
                            {
                                ...
                                {
                                    usPreIndex = stSOEQueue[ucQueueIndex].usSize -1;
                                }
                                uiOffsetInSoeArrPre = stSOEQueue[ucQueueIndex].uiBase + usPreIndex;
                            }
                            ...
                            if(uiOffsetInSoeArr <= SOE_RECORD_SEC3_MAX_INDEX)
                            {
                                ...
                                if(uiOffsetInSoeArrPre <= SOE_RECORD_SEC2_MAX_INDEX)
                                {
                                    pucSrcPre = (puint8_t)&pstSec2StartAddr[uiOffsetInSoeArrPre];
                                }
                                else
                                {
                                    uiOffsetInSoeArrPre -= SOE_RECORD_SEC3_START_INDEX;
                                    pucSrcPre = (puint8_t)&pstSec3StartAddr[uiOffsetInSoeArrPre];
                                }
                                ...
                                if(uiOffsetInSoeArr < (stSOEQueue[ucQueueIndex].uiBase + stSOEQueue[ucQueueIndex].usSize))
                                {
                                    ...
                                    {
                                        memcpy((puint8_t)pstSOE, pucSrc, sizeof(HardSOE_t));
                                        memcpy((puint8_t)&stSOEPre, pucSrcPre, sizeof(HardSOE_t));
                                        ...
                                        else if((stSOEPre.uiSec == pstTimestamp->uiSec) && \
                                                (stSOEPre.uiMicroSec == pstTimestamp->uiMicSec))
                                        {/* 未循环覆盖过一圈 */
                                            /* 没有新SOE产生 */
                                            if(usLogicPos == stSOEQueue[ucQueueIndex].usTail)
                                            {
                                                uiSoeIndex = READ_SOE_QUEUE_NO_NEW_RECORD;
                                            }
                                            else
            ...
            if(SOE_QUEUE_TYPE_HISTORY == stSOEQueue[ucQueueIndex].usType)
            {
                ...
                if(SOE_QUEUE_FULL != stSOEQueue[ucQueueIndex].ucQueueState)
                {
                    ...
                    if(uiOffsetInSoeArr <= SOE_RECORD_SEC1_MAX_INDEX)
                    {
                        ...
                        pusDst = (puint16_t)pucDst;
                        pusSrc = (puint16_t)&stSOE;
                        for(i = 0; i < sizeof(SoftSOE_t)/2; i++)
                        {
                            pusDst[i] = pusSrc[i];
                        }
                ...
                else /* SOE_QUEUE_TYPE_CYCLE_FIFO */
                {
                    ...
                    else
                    {
                        ...
                        if(uiOffsetInSoeArr < (stSOEQueue[ucQueueIndex].uiBase + stSOEQueue[ucQueueIndex].usSize))
                        {
                            ...
                            {
                                pucSrc = (puint8_t)&stSoftSOEArray[uiOffsetInSoeArr];
                                pucSrcPre = (puint8_t)&stSoftSOEArray[uiOffsetInSoeArrPre];

                                memcpy((puint8_t)pstSOE, pucSrc, sizeof(SoftSOE_t));
                                memcpy((puint8_t)&stSOEPre, pucSrcPre, sizeof(SoftSOE_t));


                                /* 未循环覆盖过一圈 */
                                if((stSOEPre.uiSec == pstTimestamp->uiSec) && \
                                        (stSOEPre.uiMicroSec == pstTimestamp->uiMicSec))
                                {
                                    /* 没有新SOE产生 */
                                    if(usLogicPos == stSOEQueue[ucQueueIndex].usTail)
                ...
                else /* SOE_QUEUE_TYPE_CYCLE_FIFO */
                {
                    ...
                    else
                    {
                        ...
                        if(uiOffsetInSoeArr <= SOE_RECORD_SEC1_MAX_INDEX)
                        {
                            ...
                            {
                                pucSrc = (puint8_t)&pstSec0StartAddr[uiOffsetInSoeArr];
                                pucSrcPre = (puint8_t)&pstSec0StartAddr[uiOffsetInSoeArrPre];
                            }
                            else /*if(uiOffsetInSoeArr <= SOE_RECORD_SEC1_MAX_INDEX)*/
                            {
                                uiOffsetInSoeArr -= SOE_RECORD_SEC1_START_INDEX;
                                pucSrc = (puint8_t)&pstSec1StartAddr[uiOffsetInSoeArr];
                                pucSrcPre = (puint8_t)&pstSec1StartAddr[uiOffsetInSoeArrPre];
                            }
