...
void LogInitCore1(void)
{
    ...
    if(1==get_cpu_no())
    {
        ...
        s_pstQueueInfo = (LogQueue_t *)LOG_SRAM_QUEUE_INFO_SEC7_START_ADDR;
        ...
        if((s_pstQueueInfo->usMagic1 != LOG_MAGIC_1) && (s_pstQueueInfo->usMagic2 != LOG_MAGIC_2))
        {
            ...
            printf("magic1=0x%x\n", s_pstQueueInfo->usMagic1);
            printf("size=%d\n", s_pstQueueInfo->usSize);
            printf("Header=%x\n", s_pstQueueInfo->usHeader);
            printf("rheader=%d\n", s_pstQueueInfo->usRetainHeader);
            printf("tail=%d\n", s_pstQueueInfo->usTail);
            printf("state=%d\n", s_pstQueueInfo->usQueueState);
            printf("magic2=0x%x\n", s_pstQueueInfo->usMagic2);
        }
        else
        {
            /* 获取队列信息 */
        ...
        s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC7_START_ADDR;
        //s_puiLockFlag = (puint32_t)LOG_SRAM_LOCK_FLAG1_START_ADDR;
        //*s_puiLockFlag = (uint32_t)LOG_QUEUE_UNLOCK;

        s_uiFlagC1 = (uint32_t)LOG_QUEUE_UNLOCK;
        s_puiLockFlag = &s_uiFlagC1;
...
***********************************************************************************************************************
*/
void LogWriteCore1(uint32_t uiLogID)
{
    ...
    do
    {
        ...
        if(true == bAtomicFlg)
        {
            ...
            //printf("tail=%d\n", usTail);
            if(usTail <= LOG_INFO_SEC7_MAX_INDEX)
            {
                s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC7_START_ADDR;
            ...
            {
                s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC8_START_ADDR;
                s_pstLogInfo = &s_pstBaseLogInfo[usTail-LOG_INFO_SEC8_START_INDEX];
            }
...
***********************************************************************************************************************
*/
int32_t LogReadCore1(LogInfo_t *pstLogInfo, uint16_t usLogicPos, puint32_t puiNextIndex, SysTime_t* pstTimestamp)
{
        ...
        do
        {
            ...
            if(true == bAtomicFlg)
            {
                ...
                if(LOG_QUEUE_EMPTY != s_stQueueInfo.usQueueState)
                {
                    ...
                    {/* 队列未满，逻辑位置超出最后一个位置 */
                        iLogIndex = (int32_t)READ_LOG_QUEUE_NO_NEW_RECORD;
                    }
                    else
                    {
                        /* 找到物理存储位置 */
                        ...
                        if(iLogIndex <= LOG_INFO_SEC7_MAX_INDEX)
                        {
                            s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC7_START_ADDR;
                        ...
                        {
                            s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC8_START_ADDR;
                            pstLogInfoTmp = &s_pstBaseLogInfo[iLogIndex-LOG_INFO_SEC8_START_INDEX];
                        }

                        if(usPreIndex <= LOG_INFO_SEC7_MAX_INDEX)
                        {
                            s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC7_START_ADDR;
                        ...
                        {
                            s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC8_START_ADDR;
                            pstLogInfoTmpPre = &s_pstBaseLogInfo[usPreIndex-LOG_INFO_SEC8_START_INDEX];
                        }
                        ...
                        if(usLogicPos != READ_LOG_FROM_BEGINNING)
                        {
                            ...
                            else if((stLogPre.uiSec == pstTimestamp->uiSec) && \
                            ...
                            else
                            {
                                ...
                                *puiNextIndex = (usLogicPos + 1) % MAX_LOG_NUM;
                                ...
                                if(usLogicPos <= LOG_INFO_SEC7_MAX_INDEX)
                                {
                                    s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC7_START_ADDR;
                                ...
                                {
                                    s_pstBaseLogInfo = (LogInfo_t *)LOG_SRAM_LOG_INFO_SEC8_START_ADDR;
                                    pstLogInfoTmp = &s_pstBaseLogInfo[usLogicPos-LOG_INFO_SEC8_START_INDEX];
                                }
                                /* 读取日志 */
                                memcpy((puint8_t)pstLogInfo, (puint8_t)pstLogInfoTmp, sizeof(LogInfo_t));
