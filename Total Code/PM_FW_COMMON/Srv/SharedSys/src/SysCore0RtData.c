...
bool_t SysCopyCore0RtDataToSharedRtDataUP1(void)
{
    ...
    //printf("res=%d\n", bResult);
    //printf("L=%dS=%dR=%d\n",g_stSharedRtData.bLock, g_stSharedRtData.bStatus, bResult);
    if(true == bResult)//liqi
    {
        /* 拷贝UP1 G区数据 */
        ...
        if(pstTaskConfigInfo!=NULL)
        {
            ...
            //printf("%x%x%x%x\n",s_stPRGRtData.ucG[uiOffset+3],s_stPRGRtData.ucG[uiOffset+2],s_stPRGRtData.ucG[uiOffset+1],s_stPRGRtData.ucG[uiOffset+0]);
        }
            ...
            for(i = 0; i < uiNum; i++)
            {
                if(0 != pstCfg->stTaskDataExInfo.stTaskDataExVar[i].usSize)
                {
                    ...
                    if(0 == pstCfg->stTaskDataExInfo.stTaskDataExVar[i].ucDestRefId)
                    {
                        ...
                        pucDst2 = (puint8_t) &(s_stPRGRtData.ucG[uiDstOff]);
                    }
                    else if(1 == pstCfg->stTaskDataExInfo.stTaskDataExVar[i].ucDestRefId)
                    {
                        ...
                        pucDst2 = (puint8_t) &(s_stPRGRtData.ucI[uiDstOff]);
                    }
                    else
                    {
                        //pucDst = (puint8_t)&(s_stBackupUP1Var.ucQVar[i].ucVar[0]);
                        //s_stBackupUP1Var.ucQVar[i].ucVarSize = usVarSize;
                        pucDst2 = (puint8_t) &(s_stPRGRtData.ucQ[uiDstOff]);
                    }

                    ucRefId = pstCfg->stTaskDataExInfo.stTaskDataExVar[i].ucDestRefId;
                    pucDisableAreaAddr = SysGetRtDataDisableFlagStartAddr(ucRefId);

                    memcpy(&ucDisFlag,(puint8_t)(pucDisableAreaAddr + uiDstOff), sizeof(uint8_t));
                    if(ucDisFlag == 0)
                    {
                        memcpy(pucDst2, pucSrc, usVarSize);
                    }
