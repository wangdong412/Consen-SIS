...
#define BIT_CTRL_DATAAVALID (0x04U)   /* Bit2: Data_Avalid：    数据有效位，数据有效时，置1 */
#define BIT_CTRL_SLAVECONFIR (0x08U)  /* Bit3: Slave_Confirm：    从站重连确认位，已确认时，置1 */
...
bool_t PMIOMakeReq( uint8_t ucIOID, uint8_t const pucData[], uint8_t ucDataLen, bool_t bActiveFV, bool_t bEnableFlg,
                    bool_t bDataValid, bool_t bSlaveConfirm, uint8_t pucReq[], uint16_t *pusReqLen );
