...
static bool_t ExlibModbusStatus(uint8_t ucPortNo, uint8_t ucCMSlot);

static void ExlibSlotStatus(SlotStatustype_t *pstData);
...
static const ExtLibMathTable stMathTable[MAX_EXLIB_FUNC_NUM]={
                                            ...
                                            {"TR_SLOT_STATUS",(fp)ExlibSlotStatus},
                                            {"GetModbusStatus",(fp)ExlibModbusStatus},

                                            {"null", NULL}
                                        };

static uint32_t s_uiUP1CheckErr1;
static uint32_t s_uiUP1CheckErr2;
static uint32_t s_uiUP2CheckErr1;
static uint32_t s_uiUP2CheckErr2;

extern const uint8_t s_ucPMVersion1[];
extern const uint8_t s_ucOSVersion1[];

extern uint8_t s_ucSN[14];

#if 0
static uint32_t uiUP1DeltaTime = 0;
static uint32_t uiUP2DeltaTime = 0;
...
void ExtLibInit(void)
{
    ...
    pstExLib = SharedGetExLibInfoWrit();
            ...
            for(j = 0; j < MAX_EXLIB_FUNC_NUM; j++)
            {
                ...
                if(0 == iRet)
                {
                    ...
                    if(pucGDataAreaAddr != NULL)
                    {
                        ...
                        *((puint32_t)pucGDataAreaAddr) = uiExFuncAddr + 0x4000000;
...
void ExtAppendDataInit(void)
{
    ...
    if(pstAppend != NULL)
    {
        ...
        /* 重新置SOE块状态 */
        pstSOEcfg = SharedGetSOEConfigInfoWrit();
        if(pstSOEcfg != NULL)
        {
        ...
        /* 重新置PTP块状态 */
        pstP2Pcfg = SharedGetP2PConfigInfoWrit();
        if(pstP2Pcfg != NULL)
        {
        ...
        /* 重新置Modbus块状态 */
        pstModbusMastercfg = SharedGetModbusMasterInfoWrit();
...
//        memset((puint8_t)&pstAppend->ucP2PCtrl[0], 0x01, sizeof(uint8_t)* MAX_P2P_CTRL_INFO);
        ...
        /* SN版本 */
        memcpy(&pstAppend->stPMVern.stPMFirmwareVern.ucModSN[0], &s_ucSN[0], 14);
        ...
        pstSysIoModInfo = SharedGetIoModInfoWrit(); /* 获取对应IO表 */
        ...
        if(pstChkErrDat->CI)
        {
            if(TASK_ID_UP1 == os_get_tno())
            {
                if(s_uiUP1CheckErr1 != 0)
                {
                    ...
                    if(CPU_EXP_DIV_ZERO == SharedGetCheckErr())
                    {
                        pstChkErrDat->DIVBYZERO = true;
                    }
                    else if(CPU_EXP_OVERFLOW == SharedGetCheckErr())
                    {
                        pstChkErrDat->OVERFLOW = true;
                    }
                    else  if(CPU_EXP_UNDERFLOW == SharedGetCheckErr())
                    {
                        pstChkErrDat->UNDERFLOW = true;
                    }
                    else  if(CPU_EXP_INVALID_INPUT == SharedGetCheckErr())
                    {
                        pstChkErrDat->BADPARAM = true;
                    }
                    else
                    {
                        pstChkErrDat->DIVBYZERO = false;
                        pstChkErrDat->OVERFLOW = false;
                        pstChkErrDat->UNDERFLOW = false;
                        pstChkErrDat->BADPARAM = false;
                    }
                }
                else
                {
...
static void ExlibClrErr(ClrErrtype_t *pstClrErr)
{
    ...
    {/* 非空 */
        if(pstClrErr->CI)
        {
            ...
            SharedSetCheckErr(CPU_EXP_NO_ERROR);
...
static float_t ExtEXP_REAL(float_t x)
{
    float_t fRet = (float_t)0.0;
    if((x <= 88) && (x >= -88))
    {

        fRet = (float_t)ExtEXP(x);
    }
    else
    {
        SharedSetCheckErr(1);
...
static void ExtEXP_LR(MATH_LRtype_t *pstDat)
{
    if(pstDat != NULL)
    {
        if((pstDat->dIN <= 709) && (pstDat->dIN >= -709))
        {
            #ifdef MATH_LIB
...
static void ExtEXP_R(MATH_Rtype_t *pstDat)
{
    if(pstDat != NULL)
    {
        if((pstDat->dIN <= 88) && (pstDat->dIN >= -88))
        {
            #ifdef MATH_LIB
            pstDat->dOUT = exp(pstDat->dIN);
            #endif
...
static void ExlibCTU(CTUtype_t *pstCTUDat)
{
    ...
    {/* 非空 */
        ...
        else if((pstCTUDat->CU == true) && (pstCTUDat->CV < MAX_INT))
        {/* trigger here, cv ++ */
            pstCTUDat->CV ++;
        }
        else
    ...
    if(pstDat != NULL)
    {
        ...
        else if (!pstDat->IN)
        {
            if (pstDat->Q)
            {
                if(TASK_ID_UP1 == os_get_tno())
                {
                    pstDat->ET = pstDat->ET + ExtGetDelta(1);
                }
                else if(TASK_ID_UP2 == os_get_tno())
                {
                    pstDat->ET = pstDat->ET + ExtGetDelta(2);
                }
                else
                {
                    ;
    ...
    if(pstDat != NULL)
    {
        ...
        else if (!pstDat->IN)
        {
            if (pstDat->Q)
            {
                if(TASK_ID_UP1 == os_get_tno())
                {
                    pstDat->ET = ((float_t)(pstDat->ST + ExtGetDelta(1)))/1000.0;
                    pstDat->ST= pstDat->ST + ExtGetDelta(1);
                }
                else if(TASK_ID_UP2 == os_get_tno())
                {
                    pstDat->ET = ((float_t)(pstDat->ST + ExtGetDelta(2)))/1000.0;
                    pstDat->ST= pstDat->ST + ExtGetDelta(2);
                }
                else
                {
                    ;
    ...
    {/* 非空 */
        ...
        {//IN 使能功能块
            ...
            else
            {
                if(TASK_ID_UP1 == os_get_tno())
                {
                    uiTon = pstDat->uiST + ExtGetDelta(1);
                    pstDat->uiST = pstDat->uiST + ExtGetDelta(1);
                }
                else if(TASK_ID_UP2 == os_get_tno())
                {
                    uiTon = pstDat->uiST + ExtGetDelta(2);
                    pstDat->uiST = pstDat->uiST + ExtGetDelta(2);
                }
                else
                {
                    ;
    ...
    {/* 非空 */
        ...
        {//IN 使能功能块
            ...
            else
            {
                if(TASK_ID_UP1 == os_get_tno())
                {
                    fTon = ((float_t)(pstDat->uiST + ExtGetDelta(1)))/1000.0;
                    pstDat->uiST = pstDat->uiST + ExtGetDelta(1);
                }
                else if(TASK_ID_UP2 == os_get_tno())
                {
                    fTon = ((float_t)(pstDat->uiST + ExtGetDelta(2)))/1000.0;
                    pstDat->uiST = pstDat->uiST + ExtGetDelta(2);
                }
                else
                {
    ...
    if(pstData != NULL)
    {
        ...
        else if(true == pstData->bIN)
        {
            ...
            if(pstData->uiET < pstData->uiPT)
            {
                if(TASK_ID_UP1 == os_get_tno())
                {
                    pstData->uiET = pstData->uiET + ExtGetDelta(1);
                }
                else if(TASK_ID_UP2 == os_get_tno())
                {
                    pstData->uiET = pstData->uiET + ExtGetDelta(2);
                }
                else
                {
                    ;
...
static void ExtTIMER_R(TIMER_Rtype_t *pstData)
{
    ...
    if(pstData != NULL)
    {
        if(true == pstData->bRESET)
        {
            ...
            pstData->uiDeltaT = (int32_t)(pstData->fIT*1000.0);
        }
        else if(true == pstData->bIN)
        {
            pstData->fET = ((float_t)pstData->uiDeltaT)/1000.0;//s
            ...
            if(pstData->fET < pstData->fPT)
            {
                if(TASK_ID_UP1 == os_get_tno())
                {
                    pstData->uiDeltaT = pstData->uiDeltaT + ExtGetDelta(1);
                }
                else if(TASK_ID_UP2 == os_get_tno())
                {
                    pstData->uiDeltaT = pstData->uiDeltaT + ExtGetDelta(2);
                }
                else
                {
                    ;
...
static int32_t ExtGetDelta(int32_t iTaskNo)
{
    ...
    if(iTaskNo == 1)
    {
        DeltaTime = (int32_t)SharedGetDelataTime(TASK_UP1);
    }
    else if(iTaskNo == 2)
    {
        DeltaTime = (int32_t)SharedGetDelataTime(TASK_UP2);
    }
    else
    {
        ;
    ...
    {/* 非空 */
        if (pstData->CI == true)
        {
            ...
            if(pstData->iTaskNo == 1)
            {
                ...
                if(true == SharedGetFirstScanFlag(TASK_UP1))
                {
                    pstData->bFirstScan = false;
                }
                else
                {
                    pstData->bFirstScan = true;
                }

                pstData->iKeySwitch = iSwitchPos;
                pstData->iScanDetra = (int32_t)SharedGetDelataTime(TASK_UP1);
                pstData->iScanOverrun = (int32_t)SharedGetAvgScanOverrun(TASK_UP1);
                pstData->iScanRequest = SysGetPrjShedTime(0);;
                pstData->iScanSurplus = (int32_t)SharedGetAvgScanSurplus(TASK_UP1);
                pstData->uiDeltaT = (int32_t)SharedGetDelataTime(TASK_UP1);
                pstData->CO = true;
            }
            else if(pstData->iTaskNo == 2)
            {
                ...
                if(true == SharedGetFirstScanFlag(TASK_UP2))
                {
                    pstData->bFirstScan = false;
                }
                else
                ...
                pstData->iKeySwitch = iSwitchPos;
                pstData->iScanDetra = (int32_t)SharedGetDelataTime(TASK_UP2);
                pstData->iScanOverrun = (int32_t)SharedGetAvgScanOverrun(TASK_UP2);
                pstData->iScanRequest = SysGetPrjShedTime(1);;
                pstData->iScanSurplus = (int32_t)SharedGetAvgScanSurplus(TASK_UP2);
                pstData->uiDeltaT = (int32_t)SharedGetDelataTime(TASK_UP2);
                pstData->CO = true;
            }
            else
            {
...
static bool_t ExlibModbusStatus(uint8_t ucPortNo, uint8_t ucCMSlot)
{
    ...
    {
        uiCMNo = SysCheckCMSlotID((uint32_t)(ucCMSlot), pstCMSlotIdTabl);
    }

    if((pstAppend != NULL) && (uiCMNo != INVALID_CM_SLOT_NUM))
    {
        if((ucPortNo <= 6) && (ucPortNo >= 1))
        {
            ucPortNo = ucPortNo - 1 ;
            ModbusStatus = (bool_t)pstAppend->stCMState[uiCMNo].stCMFirmwareState.ucModbusState[ucPortNo];
        }
    }

    return (bool_t)(ModbusStatus);
}

/*
***********************************************************************************************************************
* 函数名称: ExlibSlotStatus
*
* 功能描述:
*
* 输入参数:
...
static void ExlibSlotStatus(SlotStatustype_t *pstData)
{
    ...
    if(pstData != NULL)
    {
        pstData->CO = false;
        pstData->FAULT = false;
        pstData->ACTIVE = false;
        pstData->INSTALLED = false;

        pstAppend = (SharedAppend_t *) SysGetSharedMemAppendInfoStartAddr();
        if((pstData->CI == true) && (pstAppend != NULL))
        {
            if((pstData->ucChassis == 1) && (pstData->ucSlot >= 3) && (pstData->ucSlot <= 8))
            {
                uiSlaveNo = (pstData->ucSlot - 1) * 2;
                pstData->CO = true;
            }

            if((pstData->ucChassis > 1) && (pstData->ucChassis <= 15) && (pstData->ucSlot >= 1) && (pstData->ucSlot <= 8))
            {
                uiSlaveNo = ((pstData->ucChassis - 1) * 8 + pstData->ucSlot - 1) * 2;
                pstData->CO = true;
            }
            ...
            if(pstData->CO == true)
            {
                pstData->INSTALLED = (bool_t)pstAppend->stSlaveState[uiSlaveNo].ucActive;

                ucOnlineA = pstAppend->stSlaveState[uiSlaveNo].stSlaveDiag.ucOnlineDiag;
                ucOnlineB = pstAppend->stSlaveState[uiSlaveNo + 1].stSlaveDiag.ucOnlineDiag;
                if(((ucOnlineA | ucOnlineB) & 0x01) == 0x01)
                {
                    pstData->ACTIVE = true;
                }

                if((ucOnlineA & 0x01) == 0x01)
                {
                    if(pstAppend->stSlaveState[uiSlaveNo].stSlaveDiag.DeviceDiag[0] > 0)
                    {
                        ucFaultA = 1;
                    }
                    if(pstAppend->stSlaveState[uiSlaveNo].stSlaveDiag.DeviceDiag[1] > 0)
                    {
                        ucFaultA = 1;
                    }
                    if(pstAppend->stSlaveState[uiSlaveNo].stSlaveDiag.DeviceDiag[2] > 0)
                    {
                        ucFaultA = 1;
                    }
                }
                if((ucOnlineB & 0x01) == 0x01)
                {
                    if(pstAppend->stSlaveState[uiSlaveNo + 1].stSlaveDiag.DeviceDiag[0] > 0)
                    {
                        ucFaultB = 1;
                    }
                    if(pstAppend->stSlaveState[uiSlaveNo + 1].stSlaveDiag.DeviceDiag[1] > 0)
                    {
                        ucFaultB = 1;
                    }
                    if(pstAppend->stSlaveState[uiSlaveNo + 1].stSlaveDiag.DeviceDiag[2] > 0)
                    {
                        ucFaultB = 1;
                    }
                }
                if((ucFaultA == 1) || (ucFaultB == 1))
                {
                    pstData->FAULT = true;
                }
            }
