...
void SysStart(void)
{
    ...
    udelay(100000);/*延时100ms,等待core0初始化共享内存*/
    ...
    printf("%d  %d  T1time=%d\n", (uint32_t)ulStartTime, (uint32_t)ulEndTime, (uint32_t)(ulEndTime- ulStartTime));

    ulStartTime = SysGetFreeCounterValue();
    SharedSetSwitchPosTmp(uiSwitchPos1);
    ulEndTime = SysGetFreeCounterValue();
    printf("%d  %d  T2time=%d\n", (uint32_t)ulStartTime, (uint32_t)ulEndTime, (uint32_t)(ulEndTime- ulStartTime));
    
    SharedSetSwitchPos(uiSwitchPos1);
...
void TskInitTask(void)
{
    ...
    /* Create CFG task */
    iTaskId = os_create_task("CFG_task", TskCFGTask, NULL, TASK_PRIO_CFG, TASK_TIME_SLIDE_CFG, &uiErr);
    printf("Create CFG task.\n");
    if(iTaskId < 0)
    {
        printf("Create CFG task failed! %d %d\n", iTaskId, uiErr);
    }
...
void TskRTSTaskExe(void)
{
    ...
    if(ulEndTime - ulStartTime < 4000)
    {
        //printf("end=%d %d\n",uiEndTime,uiEndTime - uiStartTime);
        udelay(4000-(ulEndTime - ulStartTime));
    }
    #endif

    #ifdef CONTROL_CYCLE_5MS
    if(ulEndTime - ulStartTime < 800)
    {
        //printf("end=%d %d\n",uiEndTime,uiEndTime - uiStartTime);
        udelay(800-(ulEndTime - ulStartTime));
    }
    #endif
    //ConfigCycle();
    SyncMngt();
