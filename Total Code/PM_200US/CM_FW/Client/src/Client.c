...
static void DecodeReadPMFWUPStatus( ClientInfo_t *pstSWInfo, uint8_t const pucReq[], RespMsg_t *pstResp );
...
static uint16_t AddPMFWUPStatusData( uint8_t const pucReq[], uint8_t pucRespData[]);
...
static bool_t CheckPMHandle( uint8_t const pucReq[])
{
    ...
    if( pucReq != NULL )
    {
        ...
        if(((uint16_t)(CLI_LOGIN) == usAType) ||
           ...
           ((uint16_t)(CLI_READ_PMFWUP_STS) == usAType))
        {
            bPMHandle = false;
        }
       ...
       (pstResp != NULL))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else if((uint16_t)(CLI_READ_PMFWUP_STS) == usAType )
                {
                    usDataLen = AddPMFWUPStatusData( &pucReq[0], &pucResp[usIndex]);
                    if( usDataLen > 0U )
                    {
                        usLength = (uint16_t)(APP_MSG_HEADER_LEN) + usDataLen;
                    }
                    else
                    {
                        ...
                        DEBUG_CLI( "AddPMFWUPStatusData Failed\n" );
...
/*
***********************************************************************************************************************
* 函数名称: AddPMFWUPStatusData
*
* 功能描述: 添加PM I/O 固件升级状态服务
*
* 输入参数: pucReq：请求帧首地址；
*          pucRespData：应答帧数据域首字节地址。
*
...
static void DecodeCmd( ClientInfo_t *pstSWInfo, uint8_t const pucReq[], RespMsg_t *pstResp )
{
       ...
       (pstResp != NULL))
    {
        ...
        switch( usAType )
        {
            ...
            case (uint16_t)(CLI_READ_PMFWUP_STS):
            {
                DecodeReadPMFWUPStatus( pstSWInfo, pucReq, pstResp );
            }
            break;
...
static void DecodeReadPMFWUPStatus( ClientInfo_t *pstSWInfo, uint8_t const pucReq[], RespMsg_t *pstResp )
{
    ...
    {
        usAType = GetReqActualType( pucReq );
        if((uint16_t)(CLI_READ_PMFWUP_STS) == usAType )
        {
            if( LOGIN == pstSWInfo->emLog )
            {
