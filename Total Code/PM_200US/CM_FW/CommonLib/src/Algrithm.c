...
static uint16_t s_usCrc16Table[256] =
{
...
uint32_t SysCrc32Cal(uint32_t uiCrc, const puint8_t pucBuf, uint32_t uiCount)
{
    ...
    if (pucTmp != NULL)
    {
        ...
        while (uiCount--)
        {
            uiCrc = s_uiCrc32Table[((int)uiCrc ^ (*pucTmp++)) & 0xff] ^ (uiCrc >> 8);
        }
...
{
    uint16_t usCrc = 0x0, i = 0, usTmp = 0;
    puint8_t pucTemp = pucBuf;
    ...
    if (pucTemp != NULL)
    {
        ...
        {
            usTmp = usCrc;
            usCrc = s_usCrc16Table[(usTmp >> 8) ^ (*pucTemp)];
            usCrc = usCrc ^ (usTmp << 8);
            pucTemp++;
        }
    }
