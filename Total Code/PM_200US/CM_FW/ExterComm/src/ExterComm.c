...
void ExternalCommCycle( void )
{
    ...
    bool_t bSetNet1 = false;
    bool_t bSetNet2 = false;
    ...
    if( emNet != INVALID_NETWORK_PORT )
    {
        ...
        {
            DEBUG_RTS( "ExecSysCmd finished ok\n" );
    ...
    /* Set net after close */
    if( 0U == s_stNetCmdInfo[NET1].ucCmdCnt )
    {
        if( bSetNet1 )
        {
    ...
    if( 0U == s_stNetCmdInfo[NET2].ucCmdCnt )
    {
        if( bSetNet2 )
        {
...
static void InitFileUpTask( void )
{
    ...
    if( bInitRet )
    {
        ...
        {
            /* Child Process */
            DEBUG_RTS( "Fork write file process: Child process\n" );
