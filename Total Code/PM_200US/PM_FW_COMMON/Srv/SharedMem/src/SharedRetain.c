...
{
    bool_t bRet = false;
    puint16_t pusStartAddr = NULL;
    uint16_t i = 0;
    uint16_t usNum = 0;
    puint16_t pusBuf = NULL;

    if((pucBuf != NULL) && (usLen > 0) && (0 == (usLen % 2)) && (usOffset < RETAIN_SRAM_INPUT_VAR_SIZE) && (0 == (usOffset % 2)))
    {
        pusStartAddr = (puint16_t)(RETAIN_SRAM_SEC4_START_ADDR + RETAIN_SRAM_INPUT_VAR_OFFSET + usOffset);
        usLen = usLen % RETAIN_SRAM_INPUT_VAR_SIZE;
        usNum = usLen / 2;
        pusBuf = (puint16_t)pucBuf;
        for(i = 0; i < usNum; i++)
        {
            pusStartAddr[i] = pusBuf[i];
        }
...
bool_t SharedGetInputModuleVar(puint8_t pucBuf, uint16_t usOffset, uint16_t usLen)
{
    ...
    if((pucBuf != NULL) && (usLen > 0) && (0 == (usLen % 2)) && (usOffset < RETAIN_SRAM_INPUT_VAR_SIZE) && (0 == (usOffset % 2)))
    {
        pucStartAddr = (puint8_t)(RETAIN_SRAM_SEC4_START_ADDR + RETAIN_SRAM_INPUT_VAR_OFFSET + usOffset);
        usLen = usLen % RETAIN_SRAM_INPUT_VAR_SIZE;
        memcpy(pucBuf, pucStartAddr, usLen);
        bRet = true;
    }
    return bRet;
}
...
bool_t SharedSetOutputModuleVar(puint8_t pucBuf, uint16_t usOffset, uint16_t usLen)
{
    ...
    if((pucBuf != NULL) && (usLen > 0) && (0 == (usLen % 2)) && (usOffset < RETAIN_SRAM_INPUT_VAR_SIZE) && (0 == (usOffset % 2)))
    {
        pusStartAddr = (puint16_t)(RETAIN_SRAM_SEC4_START_ADDR + RETAIN_SRAM_OUTPUT_VAR_OFFSET + usOffset);
        usLen = usLen % RETAIN_SRAM_OUTPUT_VAR_SIZE;
        usNum = usLen / 2;
        pusBuf = (puint16_t)pucBuf;
        for(i = 0; i < usNum; i++)
        {
...
bool_t SharedGetOutputModuleVar(puint8_t pucBuf, uint16_t usOffset, uint16_t usLen)
{
    ...
    if((pucBuf != NULL) && (usLen > 0) && (0 == (usLen % 2)) && (usOffset < RETAIN_SRAM_INPUT_VAR_SIZE) && (0 == (usOffset % 2)))
    {
        pucStartAddr = (puint8_t)(RETAIN_SRAM_SEC4_START_ADDR + RETAIN_SRAM_OUTPUT_VAR_OFFSET + usOffset);
        usLen = usLen % RETAIN_SRAM_OUTPUT_VAR_SIZE;
        memcpy(pucBuf, pucStartAddr, usLen);
        bRet = true;
    }
