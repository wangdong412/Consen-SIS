...
static uint16_t MonitorCS1131StartRun(uint8_t ucCS1131Data[], uint32_t usCS1131DataLen,uint8_t ucACKData[]);

static uint16_t MonitorCS1131StopRun(uint8_t ucCS1131Data[], uint32_t usCS1131DataLen,uint8_t ucACKData[]);

static uint16_t MonitorCS1131PauseRun(uint8_t ucCS1131Data[], uint32_t usCS1131DataLen,uint8_t ucACKData[]);

static uint16_t MonitorCS1131SingleStep(uint8_t ucCS1131Data[], uint32_t usCS1131DataLen,uint8_t ucACKData[]);

static uint16_t MonitorCS1131WriteVarPreHandle(uint8_t ucCS1131Data[], uint32_t usCS1131DataLen,uint8_t ucACKData[]);
...
static uint16_t MonitorOPCWriteVarPreHandle (uint8_t ucOPCData[], uint32_t usOPCDataLen,uint8_t ucACKData[]);

static void MonitorRetianVarHanlde (Task_ID_t emTaskID);

static void MonitorRecoveryRetianVar(void);
...
static void MonitorRecoveryDisableTabl(void);
...
void MonitorCycle(void)
{
    ...
    uint8_t ucP2PIndex = 0U;
    uint32_t uiRecvCycle = 0U;  /* ms */
    P2PBlock_t* pstP2PBlock = NULL;
    P2PRecvReqCtrl_t *pstRecvCtrl = NULL;
    
    /* P2P Receive Request Control */
    for( ucP2PIndex = 1U; ucP2PIndex <= (uint8_t)(MAX_P2P_BLOCK_NUM); ucP2PIndex++ )
    {
        pstP2PBlock = SharedGetP2PRecvBlockInfo( ucP2PIndex );
        if((pstP2PBlock != NULL) && (ACTIVE == pstP2PBlock->emActive))
        {
            pstRecvCtrl = &s_stP2PRecvReqCtrl[ucP2PIndex];
            if( pstRecvCtrl->bHasRecv )
            {
                /* Check if recv timeout */
                pstRecvCtrl->uiIntrCycCnt++;
                uiRecvCycle = pstP2PBlock->stP2PBlockHead.uiPeriod;  /* ms */
                if( pstRecvCtrl->uiIntrCycCnt > ((10*uiRecvCycle)/(uint32_t)(TASK_PERIOD)))
                {
                    /* Recv Timeout */
                    /*printf( "P2PRTO: STAID%d, CCnt%d, Cyc%d\n", ucP2PIndex, \
                            pstRecvCtrl->uiIntrCycCnt, uiRecvCycle );*/
                    P2PHandleRecvReqTO( ucP2PIndex );
                    pstRecvCtrl->bHasRecv = 0U;
    ...
    {
        MonitorRetianVarHanlde(TASK_UP1);
    }
    ...
    {
        MonitorRetianVarHanlde(TASK_UP2);
    }
...
//                printf("Set TASK %d STOP\n",ucTaskIndex);
        ...
        SharedSOEQueueCfg();
        DataProcessSoftSOEInit(TASK_UP1);
        DataProcessSoftSOEInit(TASK_UP2);
    ...
    for(ucTaskIndex = TASK_UP1; ucTaskIndex < MAX_TASK_NUM; ucTaskIndex++)
    {
           ...
           (TS_PAUSE == emTaskStat) || (TASK_SINGLE_STEP == uiTaskSingleStepFlg))
        {
            ...
            {
                uiCycle = uiUP1IECTime / 20000;
                if((uiUP1IECTime % 20000) > 4000)
                {
                    uiCycle = uiCycle + 1;
                }
                pstAppend->stPMState.stPMFirmwareState.stSharedUPState[ucTaskIndex].uiUPIECTime = uiUP1IECTime - uiCycle * 16000;
                pstAppend->stPMState.stPMFirmwareState.stSharedUPState[ucTaskIndex].uiUPScanSurplus = uiShedTime * 1000 - (uiCycle + 1) * 20000;

//                printf("S%d\n",pstAppend->stPMState.stPMFirmwareState.stSharedUPState[ucTaskIndex].uiUPScanTime);
//                printf("I%d\n",pstAppend->stPMState.stPMFirmwareState.stSharedUPState[ucTaskIndex].uiUPIECTime);
//                printf("L%d\n",pstAppend->stPMState.stPMFirmwareState.stSharedUPState[ucTaskIndex].uiUPScanSurplus);
            }
            if(ucTaskIndex == TASK_UP2)
            {
                uiCycle = uiUP2IECTime / 20000;
                if((uiUP2IECTime % 20000) > 2000)
                {
                    uiCycle = uiCycle + 1;
                }
                pstAppend->stPMState.stPMFirmwareState.stSharedUPState[ucTaskIndex].uiUPIECTime = uiUP2IECTime - uiCycle * 18000;
                pstAppend->stPMState.stPMFirmwareState.stSharedUPState[ucTaskIndex].uiUPScanSurplus = uiShedTime * 1000 - (uiCycle + 1) * 20000;
            }
            ...
            if((uiShedTime != 0U ) && (0U == (uiShedTime % CFG_MINIMUM_BEAT_TIME)) && (HAS_BE_CONFIGURED == SysGetIsConfigured()))
            {
                ...
                {
                    if(TS_RUN == emTaskStat)
                    {
//                        DEBUG_MONITOR(" CurTime : %d ",unCurTime.stTimeStruct.uiLow32b);
//                        printf("  uiShedTime : %d ",uiShedTime);
//                        printf(" NextTime : %d \n",unNextTime.stTimeStruct.uiLow32b);
            ...
            {
//                printf("ShedTime :%d %d\n",SysGetPrjShedTime(ucTaskIndex),SysGetIsConfigured());
            }
        }
...
//                    printf("M:%d;%d\n",usWriteVarNum,stModbusMasterHead.uiBlockID);
...
//                        printf("%d;%d\n",pstModbusMasterBlock->stModbusVar[usCount].uiModbusIndex, pstModbusMasterBlock->stModbusMasterBlockHead.uiTargetBeginAddr);
                        if(ucTaskID < MAX_TASK_NUM)
                        {
                            ...
                            {/* 寄存器 */
//                                printf("MR:%d;%d\n",usSize,ucModbusData[usBuffIndex + sizeof(uint16_t) * uiModbusIndex]);
                                memcpy(&s_stWriteVarArray[ucTaskID].ucWriteVar[s_stWriteVarArray[ucTaskID].uiWriteIndex],
...
//                    printf("S#5:%d;%d;%d\n",stModbusSlaveHead.usElementNum,stModbusSlaveHead.usBeginAddr,stModbusSlaveHead.uiBlockID);
                    /* 获取modbus从站的块配置信息地址 */
...
//        DEBUG_MONITOR("  bRet : %d  usCS1131DataLen : %d \n",bRet,usCS1131DataLen);
...
//            uint8_t x = 0U;
//            DEBUG_MONITOR("SRV DATA  %d ： ",usCS1131DataLen);
//            for(x = 0U; x < usCS1131DataLen; x++)
//            {
//                printf(" %x ",ucCS1131Data[x]);
//            }
//            printf(" \n ");
            ...
            else
            {
                ...
                {/*根据服务号，处理1131服务*/
                   ...
                   {/*处理开始运行服务*/
                       usACKDataLen = MonitorCS1131StartRun(&ucCS1131Data[0], usCS1131DataLen, &ucACKData[0]);
                   }
                   ...
                   {/*处理停止运行服务*/
                       usACKDataLen = MonitorCS1131StopRun(&ucCS1131Data[0], usCS1131DataLen, &ucACKData[0]);
                   }
                   ...
                   {/*处理暂停运行服务*/
                       usACKDataLen = MonitorCS1131PauseRun(&ucCS1131Data[0], usCS1131DataLen, &ucACKData[0]);
                   }
                   ...
                   {/*处理写变量服务*/
                       usACKDataLen = MonitorCS1131WriteVarPreHandle(&ucCS1131Data[0], usCS1131DataLen, &ucACKData[0]);
                   }
                   ...
                   {/*处理单步运行服务*/
                       usACKDataLen = MonitorCS1131SingleStep(&ucCS1131Data[0], usCS1131DataLen, &ucACKData[0]);
                   }
                   ...
                   {/*处理Disable变量服务*/
                       usACKDataLen = MonitorCS1131DisableVarPreHandle (&ucCS1131Data[0], usCS1131DataLen, &ucACKData[0]);
                   }
...
//            DEBUG_MONITOR("OPC SRV DATA  %d ：\n ",usOPCDataLen);
//            uint8_t x = 0U;
//            for(x = 0U; x < usOPCDataLen; x++)
//            {
//                printf(" %x ",ucOPCData[x]);
//            }
//            printf(" \n ");
...
//            DEBUG_MONITOR("SRV DATA  %d \n",usOPCDataLen);
            ...
            else
            {
                ...
                {/*根据服务号，处理OPC服务*/
                   ...
                   case OPC_WRITE:
                   {/*处理写变量服务*/
                       usACKDataLen = MonitorOPCWriteVarPreHandle (&ucOPCData[0], usOPCDataLen, &ucACKData[0]);
                   }
                   break;
...
static void MonitorRetianVarHanlde (Task_ID_t emTaskID)
{
    ...
    puint8_t pucDataAreaAddr = NULL;
    uint32_t uiRetianVarCount = 0U;
    uint8_t ucRetainBuff[1024];
    ...
    if((HAS_BE_CONFIGURED == SysGetIsConfigured()) && (pstRetainVarInfo != NULL) )
    {
        ...
        {
            memset(&ucRetainBuff[0], 0x00, 1024);

//            printf("UPVarNum: %x \n",pstRetainVarInfo->uiUPVarNum[emTaskID]);

            for(uiRetianVarCount = 0U; uiRetianVarCount < pstRetainVarInfo->uiUPVarNum[emTaskID]; uiRetianVarCount++)
            {
                pstRetainVar = (RetainVar_t*)&pstRetainVarInfo->stRetainVar[emTaskID][uiRetianVarCount];
                if(TS_RUN == SysGetTaskStatus(emLocalPMId,emTaskID))
                {
//                    printf("%d TaskID: %x ",uiRetianVarCount,pstRetainVar->ucTaskID);
...
//                    printf("Offset: %x \n",pstRetainVar->uiOffset);
                ...
                if(pucDataAreaAddr != NULL)
                {
                    if((uiBuffIndex + pstRetainVar->usVarSize) <= 1024)
                    {
...
//                                printf("Value: ");
//                                uint8_t x = 0U;
//                                for(x = 0U; x < pstRetainVar->usVarSize; x++)
//                                {
//                                    printf("%x ",ucRetainBuff[uiBuffIndex+x]);
//                                }
//                                printf("\n");
                            }
...
//                printf("\n");
//                uint8_t x = 0U;.
//                for(x = 0U; x < uiBuffIndex; x++)
//                {
//                    printf("%x ",ucRetainBuff[x]);
//                }
//                printf("\n");
//
//                ucTest = 0;
...
static void MonitorRecoveryRetianVar(void)
{
    ...
    DEBUG_MONITOR("RecoveryRetianVar \n");
            ...
            if((pstRetainVarInfo->uiUPVarNum[emTaskID] <= MAX_RETAIN_NUM))
            {
                ...
                if(TASK_UP2 == emTaskID)
                {
                    ...
                    else
                    {
                        ...
                        printf("UP2: ");
                        uint16_t y = 0U;
                        for(y = 0U; y < uiDataLen; y++)
                        {
                            printf("%x ",ucRetainBuff[y]);
                        }
                        printf("\n");
                    }
                    ...
                    {/*将变量拷贝放入数据区*/
                        if((uiBuffIndex + pstRetainVar->usVarSize) <= 1024)
                        {
                            if(pstRetainVar->usVarSize != 0)
                            {
                                ...
                                printf("$$Value: ");
                                uint8_t z = 0U;
                                for(z = 0U; z < pstRetainVar->usVarSize; z++)
                                {
                                    printf("%x ",ucRetainBuff[uiBuffIndex + z]);
                                }
                                printf("\n");
...
//    printf("CTRL %d\n ",emTaskID);
    ...
    /*从共享内存上获取Ctrl服务数据*/
    bRet = SysGetSMMsg(SM_CTRLSRV_REQ, emTaskID, &ucCtrlSrvData[0], &usCtrlDataLen);
            ...
            {/*根据服务号，处理Ctrl服务*/
                   ...
                   if(emTaskID == emReadTaskID)
                   {
                       ...
                       if(pstTaskConfigInfo != NULL)
                       {
                           ...
                           if((pucSrcAddr != NULL) && (pucDstAddr != NULL))
                           {
                               ...
                               {
                                   printf("AreaID: %d Offset: %d, UsedSize：%d \n",RT_DATA_TYPE_G, \
                                           uiAreaOffset,uiAreaUsedSize);
                               }
                           }
                           ...
                           if((pucSrcAddr != NULL) && (pucDstAddr != NULL))
                           {
                               ...
                               {
                                   uiAreaOffset = pstTaskConfigInfo->uiOOffset - \
                                           (MAX_RTDATA_G_AREA_LEN + MAX_RTDATA_I_AREA_LEN);
                               }
               ...
               case SYS_PRJ_SWITCH:
               {
                   if(INIT_DOWNLOAD == SharedGetDownloadFlg())
                   {
                       ...
                       {
                           /*调用UP初始化*/
                           GlobalInitPtr = (GlobaInit)SharedGetTaskGlobalInitEntry(emTaskIndex);
                           if((pucDataAreaAddr != NULL) && (GlobalInitPtr != NULL))
                           {
                               printf("UP%d Init %x %x\n", \
                   ...
                   if(ONLINE_DOWNLOAD == SharedGetDownloadFlg())
                   {
                       ...
                       {
                           /*调用UP增量初始化*/
                           ChangeGlobalInitPtr = (GlobaInit)SharedGetTaskChangeGlobalInitEntry(emTaskIndex);
                           if((pucDataAreaAddr != NULL) && (ChangeGlobalInitPtr != NULL))
                           {
                               
                               printf("UP%d CInit %x %x\n", \
                                       emTaskIndex,ChangeGlobalInitPtr,pucDataAreaAddr);
    
                               ChangeGlobalInitPtr((uint32_t)pucDataAreaAddr);
...
static void MonitorP2PPreprocess(void)
{
    ...
    for(emCMID = CM1; emCMID < NUM_OF_CM; emCMID++)
    {
        ...
        //DEBUG_MONITOR("bRet : %d  P2PDataLen : %d \n",bRet,usP2PDataLen);
        
        if((true == bRet) && (usP2PDataLen != 0U) && (usP2PDataLen <= MAX_SM_MSG_LEN))
                                       ...
                                       CMController_t emCMID)
{
    ...
    uint8_t ucDataBuff[1024];
    uint8_t usDataLen = 0U;
    uint32_t uiP2PVarConut = 0U;
    P2PRecvReqCtrl_t *pstRecvCtrl = NULL;

#if 0
    uint16_t usIndex = 0U;  /* print */
#endif
    
    /*DEBUG_MONITOR("P2PREQPreH\n");*/
                    ...
                    else
                    {
                        ...
                        else
                        {
                               ...
                               (stReqDRInfo.usDataLen > 0U))
                            {
                                ...
                                memcpy(&ucDataBuff[0], &stReqDRInfo.ucBuff[0], usDataLen);

                                /* Make write variable config info. and its value */
                                uiBuffIndex = 0U;
                                for(uiP2PVarConut = 0U; uiP2PVarConut < pstP2PBlock->stP2PBlockHead.uiP2PVarNum; uiP2PVarConut++)
                                {
                                    ...
                                    {/*组织写变量包*/
                                        ...
                                        {
                                            memcpy(&s_stWriteVarArray[emTaskID].ucWriteVar[s_stWriteVarArray[emTaskID].uiWriteIndex], \
                                                    &ucDataBuff[uiBuffIndex], stWriteVar.usSize);

#if 0
                                            uiTestIndex = s_stWriteVarArray[emTaskID].uiWriteIndex;
                                            DEBUG_MONITOR("Value: ");
                                            uint8_t x = 0U;
                                            for(x = 0U; x< stWriteVar.usSize; x++)
                                            {
                                                printf("%x ",s_stWriteVarArray[emTaskID].ucWriteVar[uiTestIndex]);
                                                uiTestIndex++;
                                            }
                                        ...
                                        {
                                            memcpy(&s_stWriteVarArray[emTaskID].ucWriteVar[s_stWriteVarArray[emTaskID].uiWriteIndex], \
                                                    &ucDataBuff[uiBuffIndex], sizeof(uint8_t));
                        ...
                        else
                        {
                            //printf( "PPRsp\n" );
#if 0
                            printf( "MPMP2PRsp: L-%d\n", usACKDataLen );
                            printf( "Data(0x): " );
                            for( usIndex = 0U; usIndex < usACKDataLen; usIndex++ )
                            {
                                printf( "%02X ", ucACKData[usIndex]);
                            }
       ...
       ((emCMID >= CM1) && (emCMID < NUM_OF_CM)))
    {
        ...
        if( pstP2PRecvBlk != NULL )
        {
            ...
            else
            {
                ...
                pstPMP2PRspHead->ucSrcStaID = SharedGetStationID();
                ...
                if( bRedundant && (pstP2PRecvBlk->stP2PBlockHead.ucCMID1 == pstP2PRecvBlk->stP2PBlockHead.ucCMID2))
                {
                    if( pstP2PRecvBlk->stP2PBlockHead.ucCMID1 == ucCMSlotID )
                    {
                        if( 1U == pstP2PReqHead->ucLinkID )
                        {
                            ...
                            /* Result */
                            *pusLen = pstPMP2PRspHead->uiTotalLen;
                            bResult = true;
                        }
...
/*
***********************************************************************************************************************
* 函数名称: MonitorRecoveryDisableTabl
*
* 功能描述: 完成disable表在disable区的数据恢复.
*
* 输入参数: 无
*
* 输出参数: 无
           ...
           (stDisableTablHead.uiSize == (sizeof(DisableTablHead_t) + (stDisableTablHead.uiItemNum * sizeof(ForceItem_t)))))
        {
            ...
            do
            {
                ...
                if((pucRefAreaAddr != NULL) && (pstForceItem->usSize <= 8))
                {
                    if(pstForceItem->usSize != 0)
                    {
//                        printf("%x ",pstForceItem->ucTaskID);
//                        printf("%x ",pstForceItem->ucRefID);
//                        printf("%x ",pstForceItem->usSize);
//                        printf("%x ",pstForceItem->uiOffSet);
//                        printf("\n");
