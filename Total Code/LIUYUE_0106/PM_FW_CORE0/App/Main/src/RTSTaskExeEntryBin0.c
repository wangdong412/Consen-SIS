...
{
    /* 拷贝全局变量  */
    memcpy((puint8_t)RTS1_DATA_BASE, (puint8_t)RTS0_DATA_BASE, RTS_DATA_SIZE);

    /* 拷贝堆栈 */
    memcpy((puint8_t)RTS1_RTS_STACKF_BOTTOM, (puint8_t)RTS0_RTS_STACKF_BOTTOM, RTS_STACKF_SIZE);
    memcpy((puint8_t)RTS1_RTS_STACKI_BOTTOM, (puint8_t)RTS0_RTS_STACKI_BOTTOM, RTS_STACKI_SIZE);
    ...
    *s_puiTCBStartAddr = s_uiSp + 0x05000000;//RTS1_UP2_STACKI_BOTTOM; //新空间sp的内容
    ...
    //------------------------------------------------------------
    os_set_vec(0x05100000);             //新空间的中断向量
    ...
    /* 改sp和lr */
    ImageSwitchRTS0ToRTS1();            //任务到新空间运行
