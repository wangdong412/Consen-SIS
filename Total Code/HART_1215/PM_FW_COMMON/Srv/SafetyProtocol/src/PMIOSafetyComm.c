...
void PMIOCommInit( void )
{
    ...
    for( ucIndex = 0U; ucIndex < IPBUS_MODULE_MAX_NUM; ucIndex++ )
    {
...
bool_t PMIOHandleSendTO( uint8_t ucIOID, bool_t bTimeout )
{
    ...
    if( ucIOID <= (uint8_t)(IPBUS_MAX_SLAVE_NO))
    {
        bResult = true;
       ...
       (pusReqLen != NULL))
    {
        ...
        *pusReqLen = 0U;            /* Reset Output Length */
        ...
        if( usReqBuffLen >= usReqLen )
        {
            ...
            stReqHead.usPMID = (uint16_t)IPBUS_MASTER_ADDR;
        ...
        *pucDataLen = 0U;             /* Reset */
        ...
        if( usRespLen > (uint16_t)uiSHeadLen )
        {
            ...
            if( usRespLen >= uiSFraLen )
            {
                ...
                /* Decode */
                if( DecodeIOResp((uint16_t)IPBUS_MASTER_ADDR, (uint16_t)ucIOID, pstPMIOInfo, pucResp, \
                            ...
                            uint32_t uiSRespLen, uint8_t ucTolThr )
{
    ...
    if(( usIOID <= (uint16_t)(IPBUS_MAX_SLAVE_NO)) &&
       ( pstPMIOInfo != NULL ) &&
       ( pucSResp != NULL ) &&
       ( uiSRespLen > 0U ))
    {
