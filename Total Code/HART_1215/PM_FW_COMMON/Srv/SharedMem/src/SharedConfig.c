...
static OnlineDownLoad_Switch_Flg_t s_emOnlineDownLoadSwitchFlg[NUM_OF_PM];
...
PMController_t SharedGetLocalPMID(void)
{
    ...
    emLocalID = (PMController_t)atomic32Get(&s_emLocalID);
...
task_status_t SysGetTaskStatus(PMController_t emPMId, Task_ID_t emTaskID)
{
    ...
    task_status_t emtask_status = TS_INIT;
    if((emPMId < NUM_OF_PM) && (emTaskID < MAX_TASK_NUM))
    {
        emtask_status = (task_status_t)atomic32Get(&s_emTaskStatus[emPMId][emTaskID]);
    }
    return emtask_status;
}
...
SOEType_t SharedGetSOEType(uint8_t ucSOEBlockID)
{
    ...
    pstSOEConfigInfo = SharedGetSOEConfigInfo();
