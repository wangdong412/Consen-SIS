...
void SharedSOEInit(void)
{
    ...
    pstSOESRAMQueuesInfoStartAddr->stSOESRAMQueue[ucBlockIndex].ucStorage = stSOEQueue[ucBlockIndex].ucStorage;
    printf("Hard SOE storage=%d\n", pstSOESRAMQueuesInfoStartAddr->stSOESRAMQueue[ucBlockIndex].ucStorage);
...
void SharedSOEQueueCfg(void)
{
    ...
    if(pstSOEcfg != NULL)
    {
            ...
            for(i = 0; i < ucBlockNum; i++)
            {
                ...
                if(ucBlockIndex < MAX_SOE_QUEUE_NUM)
                {
                    if(ACTIVE == pstSOEcfg->stSoftSOEBlock[ucBlockIndex].emActive)
                    {
                        stSOEQueue[ucBlockIndex].bConfiged = SOE_QUEUE_CONFIGED;
                        stSOEQueue[ucBlockIndex].ucQueueState = SOE_QUEUE_EMPTY;
                        stSOEQueue[ucBlockIndex].ucStorage = SOE_STORAGE_DDR;
                        pstSOESRAMQueuesInfoStartAddr->stSOESRAMQueue[ucBlockIndex].ucStorage = stSOEQueue[ucBlockIndex].ucStorage;
                        printf("storage=%d\n", pstSOESRAMQueuesInfoStartAddr->stSOESRAMQueue[ucBlockIndex].ucStorage);
        ...
        if(SOE_SRAM_QUEUE_INFO_VALID_FLAG == pstSOESRAMQueuesInfoStartAddr->uiFlag)
        {
            ...
            {
                if(SOE_STORAGE_SRAM == pstSOESRAMQueuesInfoStartAddr->stSOESRAMQueue[i].ucStorage)
                {
                    stSOEQueue[i].ucQueueState = pstSOESRAMQueuesInfoStartAddr->stSOESRAMQueue[i].ucQueueState;
    ...
    if(ucQueueIndex < MAX_SOE_QUEUE_NUM)
    {
        ...
        if(true == bAtomicFlg)
        {
            if(SOE_STORAGE_SRAM == stSOEQueue[ucQueueIndex].ucStorage)
            {
                ...
                pstSOESRAMQueuesInfoStartAddr->stSOESRAMQueue[ucQueueIndex].ucQueueState = stSOEQueue[ucQueueIndex].ucQueueState;
