...
void ExtAppendDataInit(void)
{
    ...
    /* 重新置SOE块状态 */
    pstSOEcfg = SharedGetSOEConfigInfo();
    if(pstSOEcfg != NULL)
    {
    ...
    /* 默认启动modbus和ptp */
    memset((puint8_t)&pstAppend->ucModBusCtrl[0], 0xFF, sizeof(uint8_t)* MAX_MODBUS_CTRL_INFO);
    memset((puint8_t)&pstAppend->ucP2PCtrl[0], 0xFF, sizeof(uint8_t)* MAX_P2P_CTRL_INFO);
...
//#if 0
    ...
    /* PM版本 test */
    pstAppend->stPMVern.stPMFirmwareVern.ucRTSVern[0] = 1;
    pstAppend->stPMVern.stPMFirmwareVern.ucRTSVern[0] = 1;
    pstAppend->stPMVern.stPMFirmwareVern.ucRTSVern[0] = 0;
    pstAppend->stPMVern.stPMFirmwareVern.ucRTSVern[0] = 15;
    /* OS版本 test */
    pstAppend->stPMVern.stPMFirmwareVern.ucOSVern[0] = 1;
    pstAppend->stPMVern.stPMFirmwareVern.ucOSVern[0] = 0;
    pstAppend->stPMVern.stPMFirmwareVern.ucOSVern[0] = 1;
    pstAppend->stPMVern.stPMFirmwareVern.ucOSVern[0] = 10;
    /* FPGA版本 */
    ...
    {/* 非空 */
        if(pstTimeAdjDat->EN)
        {
            if(!pstTimeAdjDat->M)
            {
                ...
                iAdjTime = pstTimeAdjDat->MS;
                if(iAdjTime >= 0)
                {
                    ulAdjustTime = ((uint64_t)iAdjTime) * 1000;//us
                    ulTime += ulAdjustTime;
                }
                else
                {
                    ulAdjustTime = ((uint64_t)(iAdjTime*(-1))) * 1000;//us
    ...
    {/* 非空 */
                ...
                && (pstMBMasterDat->uiBlockIndex <= MAX_MODBUS_BLOCK_ID))
        {
            if(!pstMBMasterDat->M)
            {
                ...
                pstAppend = (SharedAppend_t *) SysGetSharedMemAppendInfoStartAddr();
                (pstAppend->ucModBusCtrl[uiBlockIndex/8]) |= (1<<(uiBlockIndex%8));
...
static void ExlibSOEClear(SOEtype_t *pstSOEData)
{
    ...
    if((pstSOEData != NULL) && (pstSOEData->CI))
    {/* 非空且CI为true */
...
static void ExlibP2PStop(P2Ptype_t *pstP2PData)
{
    ...
    if(pstP2PData != NULL)
    {/* 非空 */
        if((pstP2PData->En) && (pstP2PData->uiBlockIndex >= 1) \
                && (pstP2PData->uiBlockIndex <= 128))
        {
            if(!pstP2PData->M)
            {
                uiBlockIndex = pstP2PData->uiBlockIndex - 1;
                pstAppend = (SharedAppend_t *) SysGetSharedMemAppendInfoStartAddr();
                (pstAppend->ucP2PCtrl[uiBlockIndex/8]) &= ~(1<<(uiBlockIndex%8));
                pstP2PData->M = TRUE;
                pstP2PData->Q = TRUE;
            }
        }
    ...
    if(pstData != NULL)
    {
        if(false == pstData->bRESET)
        {
            ...
            else if(true == pstData->bIN)
            {
                ...
                pstData->uiET = uiDeltaT + pstData->uiMIT;
    ...
    if(pstData != NULL)
    {
        if(false == pstData->bRESET)
        {
            ...
            else if(true == pstData->bIN)
            {
                ...
                fTmp = (float_t)(iDeltaT + pstData->iMIT); //ms
