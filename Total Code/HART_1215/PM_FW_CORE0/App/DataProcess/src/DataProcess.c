...
//static uint8_t ucRollingSlaveNo[DATAPROCESS_SLAVE_POLL_NUM_TASK1 * 2 + DATAPROCESS_SLAVE_POLL_NUM_TASK2 * 2]; /* 记录每周期轮询的从站号 */
static uint8_t ucUP1RollingSlaveNo[DATAPROCESS_SLAVE_POLL_NUM_TASK1 * 2]; /* 记录每周期UP1轮询的从站号 */
static uint8_t ucUP2RollingSlaveNo[DATAPROCESS_SLAVE_POLL_NUM_TASK2 * 2]; /* 记录每周期UP2轮询的从站号 */
//static uint32_t uiRollingSlaveNum = 0U;
...
/* 标志位，用于控制SOE、Q区拷贝等只执行一次 */
static uint32_t uiOutputTask1Flag = 0U;
static uint32_t uiOutputTask2Flag = 0U;
...
static void DataProcessGenerateSoftSOERecord(uint8_t ucSOEBlockNo);

static void DataProcessSoftSOEHandle(Task_ID_t uiTaskID);
...
void DataProcessOutputHandle(void)
{
    ...
    uint8_t ucDataExBuf[DATAPROCESS_AO_SAFETYDATA_LEN * DATAPROCESS_SLAVE_POLL_NUM_TASK1] = {0}; /* 数据交换区buf */
    uint8_t ucPreDataExBuf[DATAPROCESS_AO_SAFETYDATA_LEN * DATAPROCESS_SLAVE_POLL_NUM_TASK1] = {0}; /* 数据交换区buf */
    uint8_t ucNextDataExBuf[DATAPROCESS_AO_SAFETYDATA_LEN * DATAPROCESS_SLAVE_POLL_NUM_TASK1] = {0}; /* 数据交换区buf */
    ...
    /* 清除记录每周期轮询的从站号 */
    memset(&ucUP1RollingSlaveNo[0], 0, DATAPROCESS_SLAVE_POLL_NUM_TASK1 * 2);
    memset(&ucUP2RollingSlaveNo[0], 0, DATAPROCESS_SLAVE_POLL_NUM_TASK2 * 2);
    ...
    if((DataProcessCheckDownloadIOConfigFinish() == IPBUS_DEVICE_CONFIG_SLAVE) && (pstUP1IoModInfo != NULL) && (pstUP2IoModInfo != NULL))
    {
        ...
        if(IO_POLL_START == SysGetIOPollFlg(TASK_UP1))
        {
            ...
            {/* 已经经过一次IEC运算，可以进行输出数据处理 */
                ...
                if(uiOutputTask1Flag == 0)
                {
                    DataProcessSoftSOEHandle(TASK_UP1); /* 处理软SOE数据 */
                    DataProcessOutputDataCopy(TASK_UP1); /* 输出数据从运算数据区Q区拷贝到临时数据区Q区 */
                    uiOutputTask1Flag = 1;
                }
            }
        }
        ...
        if(IO_POLL_START == SysGetIOPollFlg(TASK_UP2))
        {
            ...
            {/* 已经经过一次IEC运算，可以进行输出数据处理 */
                /* 每运算一次，只处理一次SOE，拷贝一次数据 */
                if(uiOutputTask2Flag == 0)
                {
                    DataProcessSoftSOEHandle(TASK_UP2); /* 处理软SOE数据 */
                    DataProcessOutputDataCopy(TASK_UP2); /* 输出数据从运算数据区Q区拷贝到共享数据区Q区 */
                    uiOutputTask2Flag = 1;
                }
            }
        }
        ...
        if(IO_POLL_START == SysGetIOPollFlg(TASK_UP1))
        {
            ...
            {/* 已经经过一次IEC运算，可以进行输出数据处理 */
                ...
                /* Q区交换，每周期处理固定数量*/
                for(i = 0; i < DATAPROCESS_SLAVE_POLL_NUM_TASK1; i++)
                {
        ...
        if(IO_POLL_START == SysGetIOPollFlg(TASK_UP2))
        {
            ...
            {/* 已经经过一次IEC运算，可以进行输出数据处理 */
                ...
                /* Q区交换，每周期处理固定数量*/
                for(i = 0; i < DATAPROCESS_SLAVE_POLL_NUM_TASK2; i++)
                {
                    /* 处理输入模块的输出数据，每次处理一对模块 */
        ...
        {/* 获取权限，向双口拷贝数据 */
            ...
            if(IO_POLL_START == SysGetIOPollFlg(TASK_UP1))
            {
                for(i = 0; i < 2 * DATAPROCESS_SLAVE_POLL_NUM_TASK1; i++)
                {
                    /* 每次处理单个模块 */
                    ...
                    if(uiSlaveCount < (uint32_t)pstUP1IoModInfo->stSysIoSlaveInfo.usOutDevNum[TASK_UP1])
                    {
                        ...
                        ucUP1RollingSlaveNo[uiRollingListBufIndex] = ucSlaveNo;
                        uiRollingListBufIndex = uiRollingListBufIndex + 1;
                        uiUP1RollingSlaveNum = uiRollingListBufIndex;
                    }
            ...
            if(IO_POLL_START == SysGetIOPollFlg(TASK_UP2))
            {
                for(i = 0; i < 2 * DATAPROCESS_SLAVE_POLL_NUM_TASK2; i++)
                {
                    /* 每次处理单个模块 */
                    ...
                    if(uiSlaveCount < (uint32_t)pstUP2IoModInfo->stSysIoSlaveInfo.usOutDevNum[TASK_UP2])
                    {
                        ...
                        ucRollingListBuf[uiRollingListBufIndex] = ucSlaveNo;
                        ucUP2RollingSlaveNo[uiRollingListBufIndex] = ucSlaveNo;
                        uiRollingListBufIndex = uiRollingListBufIndex + 1;
                        uiUP2RollingSlaveNum = uiRollingListBufIndex;
                    }
            ...
            /*根据配置信息填充从站周期轮询列表*/
            IPBUSSetSlaveRollingList(&ucRollingListBuf[0], 2 * (DATAPROCESS_SLAVE_POLL_NUM_TASK1 + DATAPROCESS_SLAVE_POLL_NUM_TASK1));
        ...
        if(IO_POLL_START == SysGetIOPollFlg(TASK_UP1))
        {
            uiTask1SlaveCountIndex = uiTask1SlaveCountIndex + 2 * DATAPROCESS_SLAVE_POLL_NUM_TASK1; /* 每次处理一对模块 */
        ...
        if(IO_POLL_START == SysGetIOPollFlg(TASK_UP2))
        {
            uiTask2SlaveCountIndex = uiTask2SlaveCountIndex + 2 * DATAPROCESS_SLAVE_POLL_NUM_TASK2; /* 每次处理一对模块 */
    ...
    {
        printf("NULL %d %d\n",pstUP1IoModInfo,pstUP2IoModInfo);
    }
...
void DataProcessInputHandle(void)
{
    ...
    uint8_t ucDataExBuf[DATAPROCESS_AI_SAFETYDATA_LEN * DATAPROCESS_SLAVE_POLL_NUM_TASK1] = {0}; /* 数据交换区buf */
    uint8_t ucPreDataExBuf[DATAPROCESS_AI_SAFETYDATA_LEN * DATAPROCESS_SLAVE_POLL_NUM_TASK1] = {0}; /* 数据交换区buf */
    uint8_t ucNextDataExBuf[DATAPROCESS_AI_SAFETYDATA_LEN * DATAPROCESS_SLAVE_POLL_NUM_TASK1] = {0}; /* 数据交换区buf */
    uint32_t uiDataExBufIndex = 0U;
    uint8_t ucPreDataFlg = 0U;
    uint8_t ucNextDataFlg = 0U;
                ...
                if((ucSlaveNo <= IPBUS_MAX_SLAVE_NO) && (ucSlaveNo >= IPBUS_MIN_SLAVE_NO))
                {
                    ...
                    else
                    {
                        ...
                        ipbus_read(READ_IPBUS_INPUT,
                                ...
                                pstUP1IoModInfo->stSysIoSlaveInfo.stDevInfo[ucSlaveNo].usDualInputSize);
                        /*
                        usModTag = pstSysIoModInfo->stSysIoSlaveInfo.stDevInfo[ucSlaveNo].usModTag;
                        switch(usModTag)
                        {
                            case MODTAG_AI:
                            {
...
//                        printf("%d;%d\n", *((puint16_t)(&ucDataExBuf[uiDataExBufIndex + 56])), uiDataExBufIndex);
                        uiDataExBufIndex = uiDataExBufIndex + pstUP1IoModInfo->stSysIoSlaveInfo.stDevInfo[ucSlaveNo].usIecSize;
                        memcpy(&ucDataExBuf[uiDataExBufIndex], ucSlaveQuality[ucSlaveNo/2], DATAPROCESS_AI_QDATA_LEN);
//                        memset(&ucDataExBuf[uiDataExBufIndex], 0xFF, DATAPROCESS_AI_QDATA_LEN);
...
//                        memset(&ucDataExBuf[uiDataExBufIndex], 0xFF, DATAPROCESS_DI_QDATA_LEN);
...
//                        memset(&ucDataExBuf[uiDataExBufIndex], 0xFF, DATAPROCESS_OSP_QDATA_LEN);
...
//                        }
        ...
        if(IO_POLL_END == SysGetIOPollFlg(TASK_UP1))
        {
        ...
        if(IO_POLL_END == SysGetIOPollFlg(TASK_UP2))
        {
...
void DataProcessSoftSOEInit(Task_ID_t emTaskID)
{
    ...
    pstSOEConfigInfo = SharedGetSOEConfigInfo(); /* 获取SOE表 */
            ...
            switch(stSlotHead.ucSlotType)
            {
                ...
                {/* 安全数据slot */
                    ...
                    if(ucActiveState == 85)
                    {
                        ...
                        pstAppend->stSlaveState[ucSlaveNo].stSlaveDiag.ucOnlineDiag = pstAppend->stSlaveState[ucSlaveNo].stSlaveDiag.ucOnlineDiag | SLAVE_ACTIVE_MASK;
                    }
                    else if(ucActiveState == 170)
                    {
                        pstAppend->stSlaveState[ucSlaveNo].stSlaveDiag.ucOnlineDiag = pstAppend->stSlaveState[ucSlaveNo].stSlaveDiag.ucOnlineDiag & (~SLAVE_ACTIVE_MASK);
                    }
                    else
...
{
    uint8_t ucRet = DATAPROCESS_RET_FAIL;
    DataProcessOSPDiagDataOne_t stOSPDiagDataOne;
    DataProcessOSPDiagDataTwo_t stOSPDiagDataTwo;

    if(ucSlotLen == DATAPROCESS_OSP_DIAG_ONE_LEN)
    {
        memcpy(&stOSPDiagDataOne, &ucRecvSrc[0], sizeof(DataProcessOSPDiagDataOne_t));

//        if(ucSlaveNo == 5U)
//        {
//            printf("#%x;", stOSPDiagDataOne.uiPRPMA);
//            printf("%x;", stOSPDiagDataOne.uiPRPMB);
//            printf("%x;", stOSPDiagDataOne.uiPRPMC);
//            printf("%x;", stOSPDiagDataOne.uiPAccelA);
//            printf("%x;", stOSPDiagDataOne.uiPAccelB);
//            printf("%x;", stOSPDiagDataOne.uiPAccelC);
//            printf("%x;", stOSPDiagDataOne.uiAlarmPackA);
//            printf("%x;", stOSPDiagDataOne.uiAlarmPackB);
//            printf("%x;", stOSPDiagDataOne.uiAlarmPackC);
//            printf("%x;", stOSPDiagDataOne.ucDigPackA);
//            printf("%x;", stOSPDiagDataOne.ucDigPackB);
//            printf("%x\n", stOSPDiagDataOne.ucDigPackC);
//        }
        ucRet = DATAPROCESS_RET_OK;
    }
    else if(ucSlotLen == DATAPROCESS_OSP_DIAG_TWO_LEN)
    {
        memcpy(&stOSPDiagDataTwo, &ucRecvSrc[0], sizeof(DataProcessOSPDiagDataTwo_t));

//        if(ucSlaveNo == 5U)
//        {
//            printf("#%d;", stOSPDiagDataTwo.ucDigPackA);
//            printf("%d;", stOSPDiagDataTwo.ucDigPackB);
//            printf("%d\n", stOSPDiagDataTwo.ucDigPackC);
//        }
        ucRet = DATAPROCESS_RET_OK;
    }
    else
...
//        printf("C No: %d Value: %d\n",stSOE.ucChannelNo,stSOE.ucValue);
...
static void DataProcessSoftSOEHandle(Task_ID_t uiTaskID)
{
    ...
    if(pstSOEConfigInfo != NULL)
    {
        for(i = 0;i < pstSOEConfigInfo->ucSoftSOEBlockNum[(uint8_t)uiTaskID]; i++)
        {
            ...
            {/* 块状态为启动 */
                DataProcessGenerateSoftSOERecord(ucSOEBlockNo);
            }
        }
    }
