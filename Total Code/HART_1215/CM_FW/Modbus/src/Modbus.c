...
static void HandleModbusTCPSlave( CommPort_t emPort );
...
static void InitSerlCfgInfo( SerlCfgInfo_t *pstSerlCfgInfo )
{
    ...
    if( pstSerlCfgInfo != NULL )
    {
        ...
        for( ucVerIndex = 0U; ucVerIndex < (uint8_t)(LOCAL_CFG_VER_CNT); ucVerIndex++ )
        {
            ...
            pCOMCfg->ucMasterLogicalPort = 1U;
...
static bool_t CheckModbusTCPCfgInfo( ModbusTCPConfigInfo_t const *pstMBTCPCfgInfo )
{
    ...
    if( pstMBTCPCfgInfo != NULL )
    {
        if( ACTIVE == pstMBTCPCfgInfo->emActive )
        {
            ...
            /* Modbus Minimum & Maximum */
            if( bValid )
            {
                if( pstMBTCPCfgInfo->sModbusMin >= pstMBTCPCfgInfo->sModbusMax )
                {
                    bValid = false;
                }
                ...
                if( bValid )
                {
                    if((pstMBTCPCfgInfo->sModbusMin < (int16_t)(MODBUS_RNG_MIN)) ||
                       (pstMBTCPCfgInfo->sModbusMin > (int16_t)(MODBUS_RNG_MAX)))
                    {
                        bValid = false;
                    }
                }
                ...
                if( bValid )
                {
                    if((pstMBTCPCfgInfo->sModbusMax < (int16_t)(MODBUS_RNG_MIN)) ||
                       (pstMBTCPCfgInfo->sModbusMax > (int16_t)(MODBUS_RNG_MAX)))
                    {
                        bValid = false;
                    }
                }
...
static bool_t CheckCOMCfgInfo( COMConfigInfo_t const *pstCOMCfgInfo, SerialPortType_t emSerl )
{
       ...
       ((emSerl >= COM1) && (emSerl < NUM_OF_SERIAL_PORT)))
    {
        if( ACTIVE == pstCOMCfgInfo->emActive )
        {
            ...
            if( bValid )
            {
                if( pstCOMCfgInfo->sModbusMin >= pstCOMCfgInfo->sModbusMax )
                {
                    bValid = false;
                }
                ...
                if( bValid )
                {
                    if((pstCOMCfgInfo->sModbusMin < (int16_t)(MODBUS_RNG_MIN)) ||
                       (pstCOMCfgInfo->sModbusMin > (int16_t)(MODBUS_RNG_MAX)))
                    {
                        bValid = false;
                    }
                }
                ...
                if( bValid )
                {
                    if((pstCOMCfgInfo->sModbusMax < (int16_t)(MODBUS_RNG_MIN)) ||
                       (pstCOMCfgInfo->sModbusMax > (int16_t)(MODBUS_RNG_MAX)))
                    {
                        bValid = false;
                    }
                }
            ...
            if( bValid )
            {
                   ...
                   ((uint8_t)(MODBUS_SLAVE_ASCII) == pstCOMCfgInfo->ucProtocol))
                {
                       ...
                       (pstCOMCfgInfo->ucModbusSlaveAddr > (uint8_t)(MAX_MODBUS_SLAVE_ID)))
                    {
                        ...
                        ((uint8_t)(MODBUS_MASTER_ASCII) == pstCOMCfgInfo->ucProtocol))
                {
                    /* Master Logic Port */
                    if((pstCOMCfgInfo->ucMasterLogicalPort < (uint8_t)(MODBUS_MLP_MIN)) ||
                       (pstCOMCfgInfo->ucMasterLogicalPort > (uint8_t)(MODBUS_MLP_MAX)))
                    {
                        DEBUG_MB( "COM[%d]: MstrLPort-%d Invalid!\n", emSerl+1, pstCOMCfgInfo->ucMasterLogicalPort );
                        bValid = false;
                    }
                }
                else
