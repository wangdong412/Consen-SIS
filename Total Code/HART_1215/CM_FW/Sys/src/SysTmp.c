...
***********************************************************************************************************************
*/
bool_t SysGetPMDownloadStatus( PMController_t emPMID, uint8_t *pucStatus )
{
    bool_t bRet = false;
    uint16_t usPMState = 0;
    if((emPMID < NUM_OF_PM) && (pucStatus != NULL))
    {
        SysGetPMState(emPMID, &usPMState);
        *pucStatus = (uint8_t)(usPMState&0xFF);
        bRet = true;
    }
    return bRet;
}
