...
static void InitTimeSyncCfgInfo( TimeSyncCfgInfo_t *pstTimeSyncInfo );
...
/* Time Sync */
static bool_t HandleTimeSyncConfig( TimeSyncCfgInfo_t *pstTimeSyncInfo );
...
static NTPTCPConfigInfo_t const* GetCurTSyncCfg( TimeSyncCfgInfo_t const *pstTimeSyncInfo );
...
void GPSCycle( void )
{
    ...
    {
        /* s，ms -> us */
        llTime = stSysTime.uiSecond * 1000000;
        llTime += stSysTime.usMiliSecond*1000;
        DEBUG_GPS( "SysTimeFromPM: s %d, ms %d\n", stSysTime.uiSecond, stSysTime.usMiliSecond );
                ...
                if( SysGetNTPTCPConfigInfo( &stTSyncCfg ))
                {
                       ...
                       ( stTSyncCfg.bUpdate ))
                    {
                        ...
                        else
                        {
                            pstTimeSyncInfo->uiCMCfgCRC = uiCRC;
                            pstTimeSyncInfo->uiCMCfgVer = uiFileVer;
                            DEBUG_GPS( "TimeSync Cfg Invalid\n" );
                        }
                    }
                }
            }
                ...
                if( HandleGPSTimingPkt( s_ucOrigPktBuff, usPktLen, &s_stGPSTMsg ))
                {
                    ...
                    if( CheckPrimUTCTimingData( pstPrimUTCData ))
                    {
                           ...
                           ((uint8_t)(RECV_GOOD_1SV) == pstPrimUTCData->ucRecvStatus))
                        {
                            ...
                            /* Leap Second */
                            if((uint8_t)(LEAP_IN_PROGRESS) == pstPrimUTCData->ucUTCFlags )
                            {
                                DEBUG_GPS( "Leap in progress\n" );
#if 0
                                /* 闰秒处理机制待定 */
#endif
                            }
