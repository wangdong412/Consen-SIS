...
static void CMConfigCheck (void)
{
    ...
    ProjVerInfo_t stPMAPrjVerInfo;
    ProjVerInfo_t stPMBPrjVerInfo;
    ProjVerInfo_t stPMCPrjVerInfo;
    
    SysGetPMProjVersion(PMA,&stPMAPrjVerInfo);
    SysGetPMProjVersion(PMB,&stPMBPrjVerInfo);
    SysGetPMProjVersion(PMC,&stPMCPrjVerInfo);
    
    if((0U == stPMAPrjVerInfo.uiProjID) && 
       (0U == stPMAPrjVerInfo.uiMainVer) && 
       (0U == stPMAPrjVerInfo.uiMinorVer) && 
       (0U == stPMAPrjVerInfo.uiProjCRC) && 
       (0U == stPMBPrjVerInfo.uiProjID) && 
       (0U == stPMBPrjVerInfo.uiMainVer) && 
       (0U == stPMBPrjVerInfo.uiMinorVer) && 
       (0U == stPMBPrjVerInfo.uiProjCRC) && 
       (0U == stPMCPrjVerInfo.uiProjID) && 
       (0U == stPMCPrjVerInfo.uiMainVer) && 
       (0U == stPMCPrjVerInfo.uiMinorVer) && 
       (0U == stPMCPrjVerInfo.uiProjCRC))
    {/*三系PM版本信息皆为O，清除配置信息*/
        ConfigClearCfgInfo();
    }
        ...
        if((CONFIG_VERN_DIFFER == ucCMPrjInfoState) || (CONFIG_DOWNLOAD_VERN_DIFFER == ucCMPrjInfoState))
        {
            ...
            if(INTER_COMM_NORMAL == uiInterCommState)
            {
                ...
                if(usInterCMDLen > 0)
                {
                    ...
                    DEBUG_CONFIG("  @@@@SEND INTER CMD  emPMid is : %x\n",emPMid);
...
static int8_t ConfigTaskDataExSectionParse(uint8_t ucSectionAddr[],CtrlStationConfigInfo_t* pstCtrlStationInfo)
{
    ...
    {
        memcpy((puint8_t)&uiUP1ToUP2VarNum, &ucSectionAddr[uiBuffIndex], sizeof(uint32_t));
        uiBuffIndex += sizeof(uint32_t);
        pstCtrlStationInfo->stTaskDataExInfo.uiUP1ToUP2VarNum = uiUP1ToUP2VarNum;
        
//        memcpy((puint8_t)&uiUP2ToUP1VarNum, &ucSectionAddr[uiBuffIndex], sizeof(uint32_t));
//        uiBuffIndex += sizeof(uint32_t);
        ...
        if((uiUP1ToUP2VarNum + uiUP2ToUP1VarNum) > MAX_TASK_EXVAR_NUM)
        {
            
        }
        else
        {    
            for(uiVarCount = 0; uiVarCount < uiUP1ToUP2VarNum; uiVarCount++)
            {/*逐个记录任务间交换变量的信息*/
...
//                    pstCtrlStationInfo->stTaskDataExInfo.stTaskDataExVar[uiVarCount].ucDestRefId );
            ...
            for(uiVarCount = uiUP1ToUP2VarNum; uiVarCount < (uiUP1ToUP2VarNum + uiUP2ToUP1VarNum); uiVarCount++)
            {/*逐个记录任务间交换变量的信息*/
                memcpy((puint8_t)&pstCtrlStationInfo->stTaskDataExInfo.stTaskDataExVar[uiVarCount], \
                        &ucSectionAddr[uiBuffIndex], sizeof(TaskDataExVar_t));
                uiBuffIndex += sizeof(TaskDataExVar_t);
...
static void ConfigClearCfgInfo(void)
{
       ...
       (pstOPCVarInfo != NULL) && (pstCMConfigInfo != NULL) && (pstCMSlotIdTabl != NULL))
    {
        if((pstCtrlStationConfigInfo->uiCRC != 0U) || (pstCtrlStationConfigInfo->uiFileVern != 0U))
        {
            ...
            DEBUG_CONFIG("Clear CtrlStation Config Info .");
        }
           ...
           (pstCtrlStationConfigInfo->stPrjConfigInfo.uiLastSaveDate != 0U))
        {
            ...
            DEBUG_CONFIG("Clear Config Info Vern.");
        }
        ...
        if((pstModbusMasterInfo->uiCRC != 0U) || (pstModbusMasterInfo->uiFileVern != 0U))
        {
            ...
            DEBUG_CONFIG("Clear ModbusMaster Config Info .");
        }
        ...
        if((pstModbusSlaveInfo->uiCRC != 0U) || (pstModbusSlaveInfo->uiFileVern != 0U))
        {
            ...
            DEBUG_CONFIG("Clear ModbusSlave Config Info .");
        }
        ...
        if((pstOPCVarInfo->uiCRC != 0U) || (pstOPCVarInfo->uiFileVern != 0U))
        {
            ...
            DEBUG_CONFIG("Clear OPC Config Info .");
        }
           ...
           (pstCMConfigInfo->stCMSingleInfo[CM4].uiCRC != 0U) || (pstCMConfigInfo->stCMSingleInfo[CM4].uiFileVern != 0U))
        {
            ...
            DEBUG_CONFIG("Clear CM Config Info .");
        }

        memset(pstCMSlotIdTabl, 0x00, sizeof(CMSlotIdTabl_t));
        
        DEBUG_CONFIG("Clear CMSlotIdTabl Info\n");
