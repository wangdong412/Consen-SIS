...
#if 0
/* PM P2P Resp */
static void ReadPMP2PRespMsg( PMController_t emSelPMID, PMP2PRespMsgInfo_t *pstMsg );
static void SelPMP2PRespMsg( PMController_t emSelPMID, PMP2PRespMsgInfo_t *pstMsg );
static void HandlePMP2PRespWait( PMController_t emSelPMID, PMP2PRespMsgInfo_t *pstMsg );
#endif
...
void P2PCycle( void )
{
    ...
    HandleOtherStaResp();  /* Handle Other Station Response Message */
...
#if 0
    s_stPMP2PRespInfo.usCount = 0U;
    ReadPMP2PRespMsg( SwitchPMID( s_emSelPMID ), &s_stPMP2PRespInfo );
#endif
    ...
    if( usLen > 0U )
    {
       ...
       if( CheckStaRespData( s_ucP2PDataBuff, usLen ))
       {
           ...
           else
           {
               ...
               {
                   /* Record Error Info.: CM_BUS InValid */
...
/*
***********************************************************************************************************************
* 函数名称: ReadPMP2PRespMsg
*
* 功能描述: 读取三系PM发送的P2P应答帧，并挑选一系：优先使用请求帧被转发到的PM
*
* 输入参数: emSelPMID：请求帧被转发到的PM；
*          pstMsg：选取的应答帧。
*
* 输出参数: 无
*
...
static void ReadPMP2PRespMsg( PMController_t emSelPMID, PMP2PRespMsgInfo_t *pstMsg )
{
    if(((emSelPMID >= PMA) && (emSelPMID < NUM_OF_PM)) &&
       (pstMsg != NULL))
    {
        if( s_stRespInfo.bWaitFlag )
        {
            HandlePMP2PRespWait( emSelPMID, pstMsg );
            s_stRespInfo.bWaitFlag = false;
        }
        else
        {
            /* PMA */
            s_stRespInfo.stPMAResp.usCount = (uint16_t)sizeof(s_stRespInfo.stPMAResp.ucBuff);
            if( !SysGetP2PMsg( PMA, P2P_PM_RESP, s_stRespInfo.stPMAResp.ucBuff, &s_stRespInfo.stPMAResp.usCount ))
            {
                s_stRespInfo.stPMAResp.usCount = 0U;
            }

            /* PMB */
            s_stRespInfo.stPMBResp.usCount = (uint16_t)sizeof(s_stRespInfo.stPMBResp.ucBuff);
            if( !SysGetP2PMsg( PMB, P2P_PM_RESP, s_stRespInfo.stPMBResp.ucBuff, &s_stRespInfo.stPMBResp.usCount ))
            {
                s_stRespInfo.stPMBResp.usCount = 0U;
            }

            /* PMC */
            s_stRespInfo.stPMCResp.usCount = (uint16_t)sizeof(s_stRespInfo.stPMCResp.ucBuff);
            if( !SysGetP2PMsg( PMC, P2P_PM_RESP, s_stRespInfo.stPMCResp.ucBuff, &s_stRespInfo.stPMCResp.usCount ))
            {
                s_stRespInfo.stPMCResp.usCount = 0U;
            }

            if((s_stRespInfo.stPMAResp.usCount > 0U) ||
               (s_stRespInfo.stPMBResp.usCount > 0U) ||
               (s_stRespInfo.stPMCResp.usCount > 0U))
            {
                /* Wait Flag */
                if((0U == s_stRespInfo.stPMAResp.usCount) &&
                   ((uint16_t)(INTER_COMM_NORMAL) == SysGetLocalInterCommState( PMA )))
                {
                    s_stRespInfo.bWaitFlag = true;
                }
                else
                {
                    if((0U == s_stRespInfo.stPMBResp.usCount) &&
                       ((uint16_t)(INTER_COMM_NORMAL) == SysGetLocalInterCommState( PMB )))
                    {
                        s_stRespInfo.bWaitFlag = true;
                    }
                    ...
                    {
                        if((0U == s_stRespInfo.stPMCResp.usCount) &&
                           ((uint16_t)(INTER_COMM_NORMAL) == SysGetLocalInterCommState( PMC )))
                        {
                            s_stRespInfo.bWaitFlag = true;
                        }
                ...
                if( !s_stRespInfo.bWaitFlag )
                {
                    ...
                    SelPMP2PRespMsg( emSelPMID, pstMsg );
                }
            }
        }
    }
...
/*
***********************************************************************************************************************
* 函数名称: SelPMP2PRespMsg
*
* 功能描述: 挑选一系PM发送的P2P应答帧：优先使用请求帧被转发到的PM
*
* 输入参数: emSelPMID：请求帧被转发到的PM；
*          pstMsg：选取的应答帧。
*
...
static void SelPMP2PRespMsg( PMController_t emSelPMID, PMP2PRespMsgInfo_t *pstMsg )
{
    PMController_t emUsePMID = INVALID_PM_ID;

    /* print */
    bool_t bPMAExist = false;
    bool_t bPMBExist = false;
    bool_t bPMCExist = false;
       ...
       (pstMsg != NULL))
    {
        ...
        if( PMA == emSelPMID )
        {
            if( s_stRespInfo.stPMAResp.usCount > 0U )
            {
                memcpy( pstMsg, &s_stRespInfo.stPMAResp, sizeof(PMP2PRespMsgInfo_t));
            }
            else if( s_stRespInfo.stPMBResp.usCount > 0U )
            {
                emUsePMID = PMB;
                memcpy( pstMsg, &s_stRespInfo.stPMBResp, sizeof(PMP2PRespMsgInfo_t));
            }
            else if( s_stRespInfo.stPMCResp.usCount > 0U )
            {
                emUsePMID = PMC;
                memcpy( pstMsg, &s_stRespInfo.stPMCResp, sizeof(PMP2PRespMsgInfo_t));
            }
            else
            {
                emUsePMID = INVALID_PM_ID;
                pstMsg->usCount = 0U;
            }
        }
        else if( PMB == emSelPMID )
        {
            if( s_stRespInfo.stPMBResp.usCount > 0U )
            {
                memcpy( pstMsg, &s_stRespInfo.stPMBResp, sizeof(PMP2PRespMsgInfo_t));
            }
            else if( s_stRespInfo.stPMAResp.usCount > 0U )
            {
                emUsePMID = PMA;
                memcpy( pstMsg, &s_stRespInfo.stPMAResp, sizeof(PMP2PRespMsgInfo_t));
            }
        ...
        {
            if( s_stRespInfo.stPMCResp.usCount > 0U )
            {
                memcpy( pstMsg, &s_stRespInfo.stPMCResp, sizeof(PMP2PRespMsgInfo_t));
            }
            else if( s_stRespInfo.stPMAResp.usCount > 0U )
        ...
        DEBUG_P2P( "PMP2PResp Exist: A-%d B-%d C-%d\n", bPMAExist, bPMBExist, bPMCExist );
        DEBUG_P2P( "PMP2PResp: SelPMID: %d, UsePMID: %d\n", emSelPMID, emUsePMID );
    }
...
static bool_t CheckPMRespData( uint8_t const pucResp[], uint16_t usLen, uint8_t ucExtReqStaID )
{
       ...
       ((ucExtReqStaID >= (uint8_t)(MIN_STA_ID)) && (ucExtReqStaID <= (uint8_t)(MAX_STA_ID))))
    {
        ...
        if( bValid )
        {
            ...
            {
                if(((uint8_t)(SRC_NETALL) == ucSrcNetID) &&
                   (0U == pstRespAddl->uiDstIP2))
                {
                    bValid = false;
                    DEBUG_P2P( "PMP2PResp: Redundant, uiDstIP2 is 0\n" );
                }
            }
        }
...
*          pusLen：数据缓冲区长度。
*
* 输出参数: pusLen：打包后的请求帧长度；
*          pucL1NetID：同CM双网口冗余时，链路1的网口ID。
*
* 返 回 值: 无
*
* 注意事项: 无
...
static void MakeRespMsg( PMP2PRespMsgInfo_t const *pstPMP2PRespInfo, uint8_t pucBuff[], puint16_t pusLen )
{
       ...
       (pusLen != NULL))
    {
        ...
        if( *pusLen >= usTotalLen )
        {
            ...
            /* FCS */
            uiCRC = SysCrc32Cal( 0U, pucBuff, usIndex );
            pucBuff[usIndex++] = (uint8_t)((uiCRC >> 24U) & 0xFF);  /* Highest Byte */
            pucBuff[usIndex++] = (uint8_t)((uiCRC >> 16U) & 0xFF);
            pucBuff[usIndex++] = (uint8_t)((uiCRC >> 8U) & 0xFF);
            pucBuff[usIndex++] = (uint8_t)(uiCRC & 0xFF);
...
static void HandleWait( void )
{
    ...
    if( s_stPMP2PReqItem.bWaitFlag )
    {
        ...
        if((s_stPMP2PReqItem.stPMAReq.usCount > 0U) ||
           ...
           (s_stPMP2PReqItem.stPMCReq.usCount > 0U))
        {
            /* Has Request Message: Check if valid */
            DEBUG_P2P( "HandleWait: Has Req Msg\n" );
