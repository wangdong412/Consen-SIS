...
static uint16_t CommLOGRead(uint8_t ucLOGData[], uint32_t usLOGDataLen,uint8_t ucACKData[]);
...
static uint16_t CommDownloadStart (uint8_t ucCS1131Data[], uint16_t usCS1131DataLen,uint8_t ucACKData[]);
...
FileOptRest_t CommDetermineFileExist(uint8_t  ucFileName[])
{
    ...
    switch(emProcessStag)
    {
        ...
        {/*等待应答，若收到应答，解析应答结果，并返回；若超时，返回超时*/
            ...
            {/*获取操作应答*/
                printf("uiStatusWord=0x%x\n",stAckHeader.uiStatusWord);
                printf("uiErrCode=0x%x\n",stAckHeader.uiErrCode);
                ...
                else if((FM_IS_FILE_EXIST_ACK != (stAckHeader.uiStatusWord & FM_IS_FILE_EXIST_ACK)))
                {/*操作成功，且文件不存在；返回结果*/
...
//            DEBUG_COMM("AMS HAS ACK\n");
            ...
            uint8_t ucTest1 = 0;
            printf("%d AMS RESP ",emCMIndex);
            for(ucTest1 = 0; ucTest1 < usACKDataLen; ucTest1++)
            {
                printf("%x ",ucACKData[ucTest1]);
            }
            printf("\n");
                ...
                else
                {
                    switch(pstMSGHead->usActualType)
                    {
                        case LOG_READ_RECORD:
                        {
                            /*process read SOE record service*/
                            printf("Log Read !\n");
                            usACKDataLen = CommLOGRead(&pstSysSrvInfo->stSysSrvData.ucSrvData[0], \
                                                        pstMSGHead->usLength, &ucACKData[0]);
                        }
                        break;
    ...
    for(emTaskIndex = TASK_UP1; emTaskIndex < MAX_TASK_NUM; emTaskIndex++)
    {
        ...
        {/*当服务处理标志为SRV_NEED_PROCESS时，处理服务数据*/
            ...
            {/*当服务应答标志为NEED_ACK时，等待来自Core0的应答数据*/
                ...
                SysGetSMMsg(SM_CTRLSRV_RESP, emTaskIndex, &ucACKData[0], &usACKDataLen);
...
//                    s_ucTimeout++;
                    ...
                    bRet = SysSetSMMsg(SM_CTRLSRV_REQ, emTaskIndex, &pstSysSrvInfo->stSysSrvData.ucSrvData[0], \
                            pstSysSrvInfo->stSysSrvData.uiSrvLen);
                    s_ucTimeout++;
