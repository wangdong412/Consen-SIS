...
//    uint8_t *pBuf = NULL;
    ...
    if((emCMID >= CM1) && (emCMID <= CM4))
    {
        ...
        emPMID = (PMController_t)GetLocalPMAddr();
    ...
    if((pstSendBuf != NULL) && (emCMID < NUM_OF_CM) && (emMsgType < NUM_OF_MSGTYPE))
    {
        ...
        if((usMsgLen > 0) && ((usOffset + usMsgLen + DATA_BLOCK_HEADER_TAIL_SIZE) < PM_SEND_BUFF_BODY_SIZE))
        {
            ...
            *pusBuf = usMsgLen;
            ...
            if(AMS_RESP == emMsgType)
            {
                printf("AMS M %d %x %x %x\n",usMsgLen,pucBuf[DATA_BLOCK_MSG_CONTENT_OFFSET],pucBuf[DATA_BLOCK_MSG_CONTENT_OFFSET + 1],pucBuf[DATA_BLOCK_MSG_CONTENT_OFFSET + 2]);
            }
            ...
            *puiBuf = uiCRC32calc;
            ...
            else if(AMS_RESP == emMsgType)
            {
                ...
                printf("AMS P %d %d\n",pstSendBuf->stSendHeader.stAMSResp.usLen,pstSendBuf->stSendHeader.stAMSResp.usOffset);
            }
...
static void CMBusProtocolPackRTData (PMSendBuffer_t *pstSendBuf, CMController_t emCMID)
{
    ...
    if((pstSendBuf != NULL) && (emCMID < NUM_OF_CM))
    {
        ...
        *pusBuf = (uint16_t)emDataArea;
        ...
        bRet = SysReadRTData(emDataArea, pucBufContent, usMsgLen, &usRdMsgLen);

        //printf("type=%d len=%d\n", emDataArea, usRdMsgLen);
        if(bRet == true)
        {
#if 0
            /* test */
